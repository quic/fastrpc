...
bail:
  if (nErr) {
    FARF(ALWAYS, "Error 0x%x: %s: failed for queue %p", nErr, __func__, q);
  }
  return nErr;
}
...
AEEResult dspqueue_export(dspqueue_t queue, uint64_t *queue_id) {
  ...
  if (q->mdq.is_mdq) {
    FARF(ALWAYS,
         "Warning: %s not supported for multi-domain queue, already exported "
         "during create",
         __func__);
...
static int dspqueue_multidomain_create(dspqueue_create_req *create) {
  ...
  VERIFY(AEE_SUCCESS == (nErr = fastrpc_context_get_domains(
                             create->ctx, &effec_domain_ids, &num_domain_ids)));

  // Validate output parameter pointers
  VERIFYC(create->ids && !create->priority && !create->flags, AEE_EBADPARM);
  ...
  VERIFYC(AEE_SUCCESS ==
              (nErr = pthread_once(&queues_once, init_process_queues_once)),
          AEE_ENOTINITIALIZED);
  ...
  VERIFYC(NULL != (mdq->effec_domain_ids = calloc(1, size)), AEE_ENOMEMORY);
  std_memscpy(mdq->effec_domain_ids, size, effec_domain_ids, size);

  VERIFYC(NULL !=
              (mdq->queues = calloc(num_domain_ids, sizeof(*(mdq->queues)))),
          AEE_ENOMEMORY);
  ...
  for (unsigned int ii = 0; ii < num_domain_ids; ii++) {
    VERIFY(AEE_SUCCESS == (nErr = dspqueue_create(
    ...
    VERIFY(AEE_SUCCESS ==
           (nErr = dspqueue_export(mdq->queues[ii], &mdq->dsp_ids[ii])));
  }
...
int dspqueue_request(dspqueue_request_payload *req) {
  ...
  case DSPQUEUE_CREATE: {
    VERIFY(AEE_SUCCESS == (nErr = dspqueue_multidomain_create(&req->create)));
    break;
  }
  default:
    nErr = AEE_EUNSUPPORTED;
...
static int dspqueue_multidomain_write(struct dspqueue *q, uint32_t flags,
  ...
  for (unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
    if (block) {
       ...
       */
      VERIFY(AEE_SUCCESS == (nErr = dspqueue_write(
                                 mdq->queues[ii], flags, num_buffers, buffers,
                                 message_length, message, timeout_us)));
    } else {
      VERIFY(AEE_SUCCESS ==
             (nErr = dspqueue_write_noblock(mdq->queues[ii], flags, num_buffers,
...
bail:
  ...
  if (nErr) {
    ...
    nErr = AEE_EBADSTATE;
    FARF(ERROR,
         "Error 0x%x: %s (block %d): failed for queue %p, flags 0x%x, num bufs "
         "%u, msg len %u, timeout %u",
         nErr, __func__, block, q, flags, num_buffers, message_length,
         timeout_us);
  }
  return nErr;
}
...
AEEResult dspqueue_write_early_wakeup_noblock(dspqueue_t queue,
  ...
  if (q->mdq.is_mdq) {
    // Recursively call 'dspqueue_write_noblock' on individual queues
    return dspqueue_multidomain_write(q, flags, 0, NULL, msg_len, msg, 0,
                                      false);
  }
 ...
 */
static int dspqueue_multidomain_read(
    struct dspqueue *q, uint32_t *flags, uint32_t max_buffers,
  ...
  for (unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
    ...
    if (read) {
      if (block) {
        nErr = dspqueue_read(cq, flags, max_buffers, num_buffers, buffers,
                             max_message_length, message_length, message,
                             timeout_us);
      } else {
        nErr =
            dspqueue_read_noblock(cq, flags, max_buffers, num_buffers, buffers,
                                  max_message_length, message_length, message);
      }
    } else {
      if (block) {
        nErr =
            dspqueue_peek(cq, flags, num_buffers, message_length, timeout_us);
      } else {
        nErr = dspqueue_peek_noblock(cq, flags, num_buffers, message_length);
      }
    }
    if (!nErr) {
...
bail:
  ...
  if (nErr && nErr != AEE_EWOULDBLOCK) {
    FARF(ALWAYS,
         "Error 0x%x: %s (read %d, block %d): failed for queue %p, max bufs "
         "%u, max msg len %u",
...
AEEResult dspqueue_peek_noblock(dspqueue_t queue, uint32_t *flags,
  ...
  if (q->mdq.is_mdq) {
    ...
    return dspqueue_multidomain_read(q, flags, 0, num_buffers, NULL, 0,
                                     message_length, NULL, 0, false, false);
  }
...
AEEResult dspqueue_peek(dspqueue_t queue, uint32_t *flags,
  ...
  if (q->mdq.is_mdq) {
    ...
    return dspqueue_multidomain_read(q, flags, 0, num_buffers, NULL, 0,
                                     message_length, NULL, timeout_us, false,
                                     true);
  }
...
AEEResult dspqueue_read_noblock(dspqueue_t queue, uint32_t *flags,
  ...
  if (q->mdq.is_mdq) {
    // Recursively call 'dspqueue_read_noblock' on individual queues
    return dspqueue_multidomain_read(q, flags, max_buffers, num_buffers,
...
AEEResult dspqueue_read(dspqueue_t queue, uint32_t *flags, uint32_t max_buffers,
  ...
  if (q->mdq.is_mdq) {
    ...
    return dspqueue_multidomain_read(
        q, flags, max_buffers, num_buffers, buffers, max_message_length,
...
bail:
  ...
  if (nErr) {
    FARF(ALWAYS, "Error 0x%x: %s: failed for queue %p, stat %d", nErr, __func__,
         q, stat);
  }
  return nErr;
}
