...
static inline void
free_skel_uri(remote_rpc_get_uri_t *dspqueue_skel)
{
...
struct dspqueue_process_queues {
	pthread_mutex_t
	    mutex; // Hold this to manipulate domain_queues or
...
#define QUEUE_SIGNAL(queue_id, signal_no)                                     \
	((DSPQUEUE_NUM_SIGNALS * queue_id) + signal_no                        \
	 + DSPSIGNAL_DSPQUEUE_MIN)
...
static AEEResult
init_domain_queues_locked(int domain)
{
	...
	dspqueue_skel.domain_name_len = (dom == CDSP1_DOMAIN_ID)
	                                    ? strlen(CDSP1_DOMAIN_NAME) + 1
	                                    : strlen(CDSP_DOMAIN_NAME) + 1;
	VERIFYC((dspqueue_skel.domain_name
	         = (char *)calloc(dspqueue_skel.domain_name_len, sizeof(char)))
	            != NULL,
	        AEE_ENOMEMORY);
	...
	VERIFYC((dspqueue_skel.module_uri
	         = (char *)calloc(dspqueue_skel.module_uri_len, sizeof(char)))
	            != NULL,
	        AEE_ENOMEMORY);
	strlcpy(dspqueue_skel.module_uri, dspqueue_rpc_URI,
	        dspqueue_skel.module_uri_len);

	/* One extra character for NULL termination is already part of
	 * module_uri_len
	 */
	dspqueue_skel.uri_len
	    = dspqueue_skel.module_uri_len + FASTRPC_URI_BUF_LEN;
	VERIFYC((dspqueue_skel.uri
	         = (char *)calloc(dspqueue_skel.uri_len, sizeof(char)))
	            != NULL,
	        AEE_ENOMEMORY);
	if((nErr
	    = remote_session_control(FASTRPC_GET_URI, (void *)&dspqueue_skel,
	                             sizeof(dspqueue_skel)))
	   != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s: obtaining session URI for %s "
		     "on domain %d, session "
		     "%u failed\n",
		     nErr, __func__, dspqueue_rpc_URI, domain,
		     dspqueue_skel.session_id);
		/* In case of failure due to SSR, return corresponding
		 * error to client */
	...
	if((nErr = dspqueue_rpc_open(dspqueue_skel.uri, &dq->dsp_handle))
	   != 0) {
		FARF(ERROR,
		     "dspqueue_rpc_open failed with %x on domain %d "
	...
	if(!queues->notif_registered[domain]) {
		// Register for process exit notifications. Only do
		// this once for the lifetime of the process to avoid
		// multiple registrations and leaks.
		remote_rpc_notif_register_t reg
		    = { .context = queues,
			.domain = domain,
			.notifier_fn = dspqueue_notif_callback };
		nErr = remote_session_control(
		    FASTRPC_REGISTER_STATUS_NOTIFICATIONS, (void *)&reg,
		    sizeof(reg));
		if(nErr == AEE_EUNSUPPORTED) {
			FARF(ERROR,
			     "Warning 0x%x: %s: DSP doesn't "
	...
	VERIFY(
	    (nErr = fastrpc_mmap(domain, dq->state_fd, dq->state, 0,
	                         sizeof(struct dspqueue_process_queue_state),
	                         FASTRPC_MAP_FD))
	    == 0);
	state_mapped = 1;
	...
	if((nErr != 0) || (cap == 0)) {
		...
		if((nErr != 0) || (cap == 0)) {
			FARF(HIGH, "dspqueue: No driver signaling "
			           "support in CPU driver");
		} else {
			FARF(HIGH, "dspqueue: Optimized driver "
			           "signaling supported");
			dq->have_dspsignal = 1;
		}
	}
	...
	if(!dq->have_dspsignal) {
		// Create thread and resources to send signals to the
		// DSP
		VERIFY(
		    (nErr = pthread_mutex_init(&dq->send_signal_mutex, NULL))
		    == 0);
		sendmutex = 1;
		VERIFY((nErr = pthread_cond_init(&dq->send_signal_cond, NULL))
		       == 0);
		sendcond = 1;
		...
		VERIFY((nErr = pthread_create(&dq->send_signal_thread, &tattr,
		                              dspqueue_send_signal_thread, dq))
		       == 0);
		sendthread = 1;
		...
		pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_JOINABLE);
		VERIFY(
		    (nErr = pthread_create(&dq->receive_signal_thread, &tattr,
		                           dspqueue_receive_signal_thread, dq))
		    == 0);
		recvthread = 1;
	}
...
bail:
	...
	free_skel_uri(&dspqueue_skel);
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d) errno %s", nErr,
...
// Must hold queues->mutex.
static AEEResult
destroy_domain_queues_locked(int domain)
{
	...
	if(!dq->have_dspsignal) {
		if(dq->dsp_error) {
			// Ignore errors if the DSP process
			// died
			dspqueue_rpc_cancel_wait_signal(dq->dsp_handle);
		} else {
			VERIFY((nErr = dspqueue_rpc_cancel_wait_signal(
				    dq->dsp_handle))
			       == 0);
		}
		FARF(MEDIUM, "Join receive signal thread");
		VERIFY((nErr = pthread_join(dq->receive_signal_thread, &ret))
		       == 0);
		...
		VERIFY((nErr = pthread_join(dq->send_signal_thread, &ret))
		       == 0);
		if(!dq->dsp_error) {
			VERIFY(((uintptr_t)ret) == 0);
		}
...
AEEResult
dspqueue_create(int domain, uint32_t flags, uint32_t req_queue_size,
                uint32_t resp_queue_size, dspqueue_callback_t packet_callback,
                dspqueue_callback_t error_callback, void *callback_context,
                dspqueue_t *queue)
{
	...
	*queue = NULL;
	...
	// Initialize process-level and per-domain queue structures and
	// signaling
	...
	if((dq && (dq->num_queues >= DSPQUEUE_MAX_PROCESS_QUEUES))) {
		FARF(ERROR, "Too many queues in process for domain %d",
		     domain);
		pthread_mutex_unlock(&queues->mutex);
		return AEE_EBADPARM;
	}
	...
	// contains:
	// - Queue headers
	// - Read and write states for both request and response queues (four
	// total)
	// - Request and response queues
	// All are aligned to QUEUE_CACHE_ALIGN.
	q->user_queue_size = CACHE_ALIGN_SIZE(sizeof(struct dspqueue_header))
	                     + 4
	                           * CACHE_ALIGN_SIZE(sizeof(
				       struct dspqueue_packet_queue_state))
	                     + CACHE_ALIGN_SIZE(req_queue_size)
	                     + CACHE_ALIGN_SIZE(resp_queue_size);

	// Allocate queue shared memory and map to DSP
	VERIFYC((q->user_queue = rpcmem_alloc_internal(RPCMEM_HEAP_ID_SYSTEM,
	...
	VERIFYC((q->user_queue_fd = rpcmem_to_fd(q->user_queue)) > 0,
	        AEE_ERPC);
	VERIFY((nErr = fastrpc_mmap(domain, q->user_queue_fd, q->user_queue, 0,
	                            q->user_queue_size, FASTRPC_MAP_FD))
	       == 0);
	queue_mapped = 1;
	...
	if(dq->have_dspsignal) {
		...
		for(signals = 0; signals < DSPQUEUE_NUM_SIGNALS; signals++) {
			VERIFY((nErr = dspsignal_create(
				    domain, QUEUE_SIGNAL(id, signals), 0))
			       == AEE_SUCCESS);
		}
	}

	// First attempt to create the queue with an invalid version. If the
	// call succeeds we know the DSP side is ignoring the version and flags
	// and does not support wait counts in the header, driver signaling, or
	// any other post-v1 features. Unfortunately the initial DSP codebase
	// ignores the version and flags...
	q->header->version = UINT32_MAX;
	nErr = dspqueue_rpc_create_queue(
	    dq->dsp_handle, q->id, q->user_queue_fd, queue_count, &q->dsp_id);
	if((nErr == AEE_EUNSUPPORTED)
	   || (nErr == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED))) {
		// OK, the DSP does pay attention to the version. It
		// should also support wait counts and optionally
		// driver signaling. Create the queue.
		FARF(HIGH,
		     "Initial queue create failed with %0x%x as "
		     "expected",
		     nErr);
		...
		VERIFY((nErr = dspqueue_rpc_create_queue(
			    dq->dsp_handle, q->id, q->user_queue_fd,
			    queue_count, &q->dsp_id))
		       == 0);
		q->have_wait_counts = 1;
		// Note that we expect the DSP will support both wait
		// counts and driver signaling or neither. However we
		// can operate with wait counts only in case we have an
		// updated DSP image but an older CPU kernel driver
		// without driver signaling support.
	} else if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "dspqueue_rpc_create_queue failed: 0x%x", nErr);
		goto bail;
	} else {
		FARF(HIGH,
		     "First-cut queue create succeeded unexpectedly? "
		     "0x%x",
		     nErr);
		// No new features available, including driver
		// signaling
		if(q->have_driver_signaling) {
			unsigned i;
			FARF(HIGH, "Driver signaling not supported "
			           "on DSP, fall back to FastRPC "
			           "signaling");
			for(i = 0; i < signals; i++) {
				VERIFY((nErr = dspsignal_destroy(
					    domain, QUEUE_SIGNAL(id, i)))
				       == 0);
			}
			signals = 0;
		}
	...
	if(q->packet_callback) {
		...
		pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_JOINABLE);
		VERIFY(
		    (nErr = pthread_create(&q->packet_callback_thread, &tattr,
		                           dspqueue_packet_callback_thread, q))
		    == 0);
		callbackthread = 1;
	}
	...
	*queue = q;
	...
	dq->queues[id] = q;
	pthread_mutex_unlock(&dq->queue_list_mutex);
	FARF(ALWAYS, "%s: created Queue %u, %p, DSP 0x%08x for domain %d",
	     __func__, q->id, q, (unsigned)q->dsp_id, q->domain);
...
bail:
	...
	if(dq != NULL) {
		...
		dq->num_queues--;
		if(dq->num_queues == 0) {
			// This would have been the first queue
			// for this domain
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed (domain %d, flags 0x%x, "
		     "sizes %u, %u errno %s)",
		     nErr, __func__, domain, (unsigned)flags,
		     (unsigned)req_queue_size, (unsigned)resp_queue_size,
		     strerror(errno));
	}
	return nErr;
}
 ...
 */
static int
dspqueue_multidomain_close(struct dspqueue *q, bool queue_mut)
{
	...
	for(unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
		...
		if(err) {
			// Return the first failure's error
			// code to client
			nErr = nErr ? nErr : err;
		}
	}
...
bail:
	if(nErr) {
		FARF(ALWAYS, "Error 0x%x: %s: failed for queue %p", nErr,
		     __func__, q);
	}
	return nErr;
}
...
AEEResult
dspqueue_close(dspqueue_t queue)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_close' on each individual
		// queue
	...
	if(!dq->dsp_error) {
		VERIFY((nErr = dspqueue_rpc_is_imported(dq->dsp_handle,
		                                        q->dsp_id, &imported))
		       == 0);
		if(imported) {
			FARF(ERROR,
			     "Attempting to close queue 0x%p "
			     "still open on the DSP",
			     queue);
			nErr = AEE_EBADPARM;
			goto bail;
		}
	...
	if(q->error_callback_thread) {
		...
		if(nErr == EDEADLK || nErr == EINVAL) {
			FARF(ERROR,
			     "Error %d: %s: Error callback "
			     "thread join failed for thread "
			     ": %d",
	...
	// Cancel any outstanding blocking read/write calls (from callback
	// threads)
	if(q->have_driver_signaling) {
		/*
		...
		if(dq->dsp_error != AEE_ECONNRESET) {
			for(i = 0; i < DSPQUEUE_NUM_SIGNALS; i++) {
				nErr = dspsignal_cancel_wait(
				    q->domain, QUEUE_SIGNAL(q->id, i));
	...
	if(q->packet_callback) {
		...
		if(nErr && nErr != ESRCH) {
			FARF(ERROR,
			     "Error: %s: packet callback "
			     "thread for queue %p joined "
			     "with error %d",
			     __func__, q, nErr);
			goto bail;
		}
	...
	if(dq->dsp_error) {
		...
		VERIFY((nErr = dspqueue_rpc_destroy_queue(dq->dsp_handle,
		                                          q->dsp_id))
		       == 0);
		VERIFY(
		    (nErr = fastrpc_munmap(dq->domain, q->user_queue_fd,
		                           q->user_queue, q->user_queue_size))
		    == 0);
	}
	rpcmem_free(q->user_queue);
	/*
	...
	dq->num_queues--;
	if(dq->num_queues == 0) {
		FARF(ALWAYS, "%s: destroying queues and signals for domain %d",
		     __func__, q->domain);
	...
	pthread_mutex_unlock(&queues->mutex);
	FARF(ALWAYS, "%s: closed Queue %u, %p, DSP 0x%08x for domain %d",
	     __func__, q->id, q, (unsigned)q->dsp_id, q->domain);
	free(q);
...
AEEResult
dspqueue_export(dspqueue_t queue, uint64_t *queue_id)
{
	...
	if(q->mdq.is_mdq) {
		FARF(ALWAYS,
		     "Warning: %s not supported for multi-domain "
		     "queue, already exported during create",
		     __func__);
...
static int
dspqueue_multidomain_create(dspqueue_create_req *create)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_context_get_domains(
		       create->ctx, &effec_domain_ids, &num_domain_ids)));
	...
	VERIFYC(AEE_SUCCESS
	            == (nErr = pthread_once(&queues_once,
	                                    init_process_queues_once)),
	        AEE_ENOTINITIALIZED);
	...
	for(unsigned int ii = 0; ii < num_domain_ids; ii++) {
		VERIFY(AEE_SUCCESS
		       == (nErr = dspqueue_create(
			       effec_domain_ids[ii], create->flags,
			       create->req_queue_size, create->resp_queue_size,
		...
		VERIFY(AEE_SUCCESS
		       == (nErr = dspqueue_export(mdq->queues[ii],
		                                  &mdq->dsp_ids[ii])));
	}
	...
	FARF(ALWAYS,
	     "%s: created queue %p for ctx 0x%" PRIx64
	     ", sizes: req %u, rsp %u, num domains %u",
	     __func__, q, create->ctx, create->req_queue_size,
	     create->resp_queue_size, num_domain_ids);
bail:
	if(nErr) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for ctx 0x%" PRIx64
		     ", queue sizes: req %u, rsp %u, num ids %u",
...
int
dspqueue_request(dspqueue_request_payload *req)
{
	...
	case DSPQUEUE_CREATE: {
		VERIFY(AEE_SUCCESS
		       == (nErr = dspqueue_multidomain_create(&req->create)));
		break;
	}
	default:
		nErr = AEE_EUNSUPPORTED;
...
static void
get_queue_state_write(void *memory, struct dspqueue_packet_queue_header *pq,
...
static void
get_queue_state_read(void *memory, struct dspqueue_packet_queue_header *pq,
...
write_64(volatile uint8_t *packet_queue, uint32_t write_pos,
         uint32_t queue_len, uint64_t data)
{
...
static inline uint32_t
write_data(volatile uint8_t *packet_queue, uint32_t write_pos,
           uint32_t queue_len, const void *data, uint32_t data_len)
{
...
// Send a signal
static AEEResult
send_signal(struct dspqueue *q, uint32_t signal_no)
{
	...
	if(q->have_driver_signaling) {
		...
		VERIFY((nErr = dspsignal_signal(
			    q->domain, QUEUE_SIGNAL(q->id, signal_no)))
		       == 0);
	} else {
		pthread_mutex_lock(&dq->send_signal_mutex);
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for queue %p signal %u",
		     nErr, __func__, q, (unsigned)signal_no);
	}
	return nErr;
}
...
// infinite wait. Only DSPQUEUE_SIGNAL_REQ_SPACE and
// DSPQUEUE_SIGNAL_RESP_PACKET supported. The appropriate mutex must be locked.
static AEEResult
wait_signal_locked(struct dspqueue *q, uint32_t signal_no,
                   struct timespec *timeout)
{
	...
	if(q->have_driver_signaling) {
		...
		if(timeout) {
			struct timespec now;
			VERIFYC(clock_gettime(CLOCK_REALTIME, &now) == 0,
			        AEE_EFAILED);
			to_now = timespec_diff_us(
			    timeout,
			    &now); // microseconds until expiry
			if(to_now == 0) {
				return AEE_EEXPIRED;
			}
		} else {
		...
		VERIFY((nErr = dspsignal_wait(
			    q->domain, QUEUE_SIGNAL(q->id, signal_no), to_now))
		       == 0);
	} else {
		// Not using driver signaling, wait for the appropriate
		// condition variable and its associated state
		uint32_t *count;
		uint32_t *mask;
		pthread_mutex_t *mutex;
		pthread_cond_t *cond;
...
static int
dspqueue_multidomain_write(struct dspqueue *q, uint32_t flags,
                           ...
                           uint32_t timeout_us, bool block)
{
	...
	for(unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
		if(block) {
			/*
			 * Multi-domain blocking write
			 * operation will serially block on
			 * each individual queue.
			 */
			VERIFY(AEE_SUCCESS
			       == (nErr = dspqueue_write(
				       mdq->queues[ii], flags, num_buffers,
				       buffers, message_length, message,
				       timeout_us)));
		} else {
			VERIFY(AEE_SUCCESS
			       == (nErr = dspqueue_write_noblock(
				       mdq->queues[ii], flags, num_buffers,
				       buffers, message_length, message)));
		}
	}
bail:
	...
	if(nErr) {
		/*
		 * If multi-domain write operation failed on some but
		 * not all queues, then the MDQ is now in an
		 * irrecoverable bad-state as the packet was partially
		 * written to some domains but not all and it cannot be
		 * "erased". Client is expected to close queue.
		 */
		nErr = AEE_EBADSTATE;
		FARF(ERROR,
		     "Error 0x%x: %s (block %d): failed for queue %p, "
		     "flags 0x%x, num bufs %u, msg len %u, timeout %u",
...
AEEResult
dspqueue_write_noblock(dspqueue_t queue, uint32_t flags, uint32_t num_buffers,
                       struct dspqueue_buffer *buffers,
                       uint32_t message_length, const uint8_t *message)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_write_noblock' on
		// individual queues
		return dspqueue_multidomain_write(q, flags, num_buffers,
		                                  buffers, message_length,
		                                  message, 0, false);
	}
	...
	len = 8 + num_buffers * sizeof(struct dspqueue_buffer)
	      + message_length;
	alen = (len + 7) & (~7);

	if(alen > (qsize - 8)) {
		FARF(ERROR, "Packet size %u too large for queue size %u",
		     (unsigned)len, (unsigned)qsize);
		nErr = AEE_EBADPARM;
		goto bail;
	}
	...
	if((qsize - w) < alen) {
		// Don't wrap the packet around queue end, but rather
		// move it to the beginning, replicating the header
		wrap = 1;
	...
	for(i = 0; i < num_buffers; i++) {
		...
		// Find buffer in internal FastRPC structures and
		// handle refcounts
		if(b->flags & DSPQUEUE_BUFFER_FLAG_REF) {
			VERIFYC((b->flags & DSPQUEUE_BUFFER_FLAG_DEREF) == 0,
			        AEE_EBADPARM);
			nErr = fastrpc_buffer_ref(q->domain, b->fd, 1, &va,
			                          &size);
		} else if(b->flags & DSPQUEUE_BUFFER_FLAG_DEREF) {
			nErr = fastrpc_buffer_ref(q->domain, b->fd, -1, &va,
			                          &size);
		} else {
			nErr = fastrpc_buffer_ref(q->domain, b->fd, 0, &va,
			                          &size);
		}
		if(nErr == AEE_ENOSUCHMAP) {
			FARF(ERROR,
			     "Buffer FD %d in queue message "
			     "not mapped to domain %d",
			     b->fd, q->domain);
			goto bail;
		}
		VERIFY(nErr == 0);
		...
		// Ensure buffer offset and size are within the buffer
		// as mapped. Use mapped size if not specified by the
		// client
		if(b->size != 0) {
			uint64_t bend
			    = ((uint64_t)b->offset) + ((uint64_t)b->size);
	...
	phdr = (((uint64_t)(len & 0xffffffff))
	        | (((uint64_t)(flags & 0xffff)) << 32)
	...
	// Update write pointer. This marks the message available in the user
	// queue
	...
	if(q->have_wait_counts) {
		// Only send a signal if the other end is potentially
		// waiting
		barrier_full();
		...
		if(read_state->wait_count) {
			...
			VERIFY(
			    (nErr = send_signal(q, DSPQUEUE_SIGNAL_REQ_PACKET))
			    == 0);
		} else {
		...
		VERIFY((nErr = send_signal(q, DSPQUEUE_SIGNAL_REQ_PACKET))
		       == 0);
	}
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for queue %p (flags "
		     "0x%x, num_buffers %u, "
		     "message_length %u)",
		     nErr, __func__, queue, (unsigned)flags,
		     (unsigned)num_buffers, (unsigned)message_length);
	}
	return nErr;
}

static void
timespec_add_us(struct timespec *ts, uint32_t us)
{
	uint64_t ns
	    = (uint64_t)ts->tv_nsec + (uint64_t)(1000 * (us % 1000000));
	if(ns > 1000000000ULL) {
...
AEEResult
dspqueue_write(dspqueue_t queue, uint32_t flags, uint32_t num_buffers,
               ...
               const uint8_t *message, uint32_t timeout_us)
{
	...
	errno = 0;
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_write' on individual
		// queues.
		return dspqueue_multidomain_write(q, flags, num_buffers,
		                                  buffers, message_length,
		                                  message, timeout_us, true);
	}
	...
	if(q->have_wait_counts) {
		...
		waiting = 1;
		nErr
		    = dspqueue_write_noblock(queue, flags, num_buffers,
		                             buffers, message_length, message);
		if(nErr != AEE_EWOULDBLOCK) {
			goto bail;
		}
...
AEEResult
dspqueue_write_early_wakeup_noblock(dspqueue_t queue, uint32_t wakeup_delay,
                                    uint32_t packet_flags)
{
	...
	struct dspqueue *q = queue;
	const uint8_t *msg
	    = wakeup_delay ? (const uint8_t *)&wakeup_delay : NULL;
	uint32_t msg_len = wakeup_delay ? sizeof(wakeup_delay) : 0;

	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_write_noblock' on
		// individual queues
		return dspqueue_multidomain_write(q, flags, 0, NULL, msg_len,
		                                  msg, 0, false);
	}
...
static AEEResult
peek_locked(volatile const uint8_t *qp, uint32_t r, uint32_t *flags,
            ...
            uint64_t *raw_header)
{
	...
	VERIFYC(len >= (8 + nb * sizeof(struct dspqueue_buffer)),
	        AEE_EBADITEM);
	...
	if(message_length != NULL) {
		if(f & DSPQUEUE_PACKET_FLAG_MESSAGE) {
			*message_length
			    = len - 8 - nb * sizeof(struct dspqueue_buffer);
		} else {
			*message_length = 0;
		}
	}
 ...
 */
static int
dspqueue_multidomain_read(struct dspqueue *q, uint32_t *flags,
                          uint32_t max_buffers, uint32_t *num_buffers,
                          struct dspqueue_buffer *buffers,
                          uint32_t max_message_length,
                          uint32_t *message_length, uint8_t *message,
                          uint32_t timeout_us, bool read, bool block)
{
	int nErr = AEE_SUCCESS;
	struct dspqueue_multidomain *mdq = NULL;
	bool locked = false;
	...
	if(block && mdq->num_domain_ids > 1) {
		/*
		 * Blocked read cannot be supported on multi-domain
		 * queues because the read operation is done in a
		 * serial manner on the individual queues of each
		 * domain and the first queue with a valid packet is
		 * returned to client. Say if the first domain's queue
		 * does not have any response packets to consume but
		 * the second domain's queue does, then the read call
		 * will just block indefinitely on the first queue
		 * without ever checking the subsequence queues.
		 */
		nErr = AEE_EUNSUPPORTED;
		FARF(ALWAYS,
		     "Error 0x%x: %s: not supported for multi-domain "
		     "queue %p",
		     nErr, __func__, q);
		return nErr;
	}
		...
		if(read) {
			if(block) {
				nErr = dspqueue_read(
				    ...
				    buffers, max_message_length,
				    message_length, message, timeout_us);
			} else {
				nErr = dspqueue_read_noblock(
				    cq, flags, max_buffers, num_buffers,
				    buffers, max_message_length,
				    message_length, message);
			}
			...
			if(block) {
				nErr = dspqueue_peek(cq, flags, num_buffers,
				                     message_length,
				                     timeout_us);
			} else {
				nErr = dspqueue_peek_noblock(
				    cq, flags, num_buffers, message_length);
			}
		}
		if(!nErr) {
			...
			// If queue is empty, proceed to next
			// queue
			continue;
		} else {
			// If any queue is in bad state, bail
			// out with error
			goto bail;
		}
	}
bail:
	...
	if(nErr && nErr != AEE_EWOULDBLOCK) {
		FARF(ALWAYS,
		     "Error 0x%x: %s (read %d, block %d): failed for "
		     "queue %p, max bufs %u, max msg len %u",
		     nErr, __func__, read, block, q, max_buffers,
		     max_message_length);
	}
	return nErr;
}
...
AEEResult
dspqueue_peek_noblock(dspqueue_t queue, uint32_t *flags, uint32_t *num_buffers,
                      uint32_t *message_length)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_peek_noblock' on
		// individual queues
		return dspqueue_multidomain_read(q, flags, 0, num_buffers,
		                                 NULL, 0, message_length, NULL,
		                                 0, false, false);
	}
	...
	VERIFY((nErr
	        = peek_locked(qp, r, flags, num_buffers, message_length, NULL))
	       == 0);
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for queue %p", nErr,
		     __func__, queue);
	}
	return nErr;
}
...
AEEResult
dspqueue_peek(dspqueue_t queue, uint32_t *flags, uint32_t *num_buffers,
              uint32_t *message_length, uint32_t timeout_us)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_read_noblock' on
		// individual queues
		return dspqueue_multidomain_read(q, flags, 0, num_buffers,
		                                 NULL, 0, message_length, NULL,
		                                 timeout_us, false, true);
	}
	...
	nErr
	    = dspqueue_peek_noblock(queue, flags, num_buffers, message_length);
	if(nErr != AEE_EWOULDBLOCK) {
	...
	if(q->have_wait_counts) {
		...
		cache_flush_word(wait_count);
		waiting = 1;
		nErr = dspqueue_peek_noblock(queue, flags, num_buffers,
		                             message_length);
		if(nErr != AEE_EWOULDBLOCK) {
			goto bail;
		}
	...
	while(1) {
		...
		VERIFY((nErr = wait_signal_locked(
			    q, DSPQUEUE_SIGNAL_RESP_PACKET, timeout_ts))
		       == 0);
...
static inline uint32_t
read_data(volatile const uint8_t *packet_queue, uint32_t read_pos,
          uint32_t queue_len, void *data, uint32_t data_len)
{
...
AEEResult
dspqueue_read_noblock(dspqueue_t queue, uint32_t *flags, uint32_t max_buffers,
                      ...
                      uint8_t *message)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_read_noblock' on
		// individual queues
		return dspqueue_multidomain_read(
		    q, flags, max_buffers, num_buffers, buffers,
		    max_message_length, message_length, message, 0, true,
		    false);
	}
	...
	// Check if we have a packet available
	FARF(LOW, "Queue %u wp %u", (unsigned)q->id,
	     (unsigned)write_state->position);
	get_queue_state_read(q->header, pq, &qleft, &r, NULL);
	if(qleft < 8) {
		pthread_mutex_unlock(&q->mutex);
		return AEE_EWOULDBLOCK;
	...
	if(f & DSPQUEUE_PACKET_FLAG_WAKEUP) {
		...
		if(q->have_wait_counts) {
			// Only signal if the other end is
			// potentially waiting
			cache_invalidate_word(&write_state->wait_count);
			if(write_state->wait_count) {
				VERIFY((nErr = send_signal(
					    q, DSPQUEUE_SIGNAL_RESP_SPACE))
				       == AEE_SUCCESS);
			}
		} else {
			VERIFY(
			    (nErr = send_signal(q, DSPQUEUE_SIGNAL_RESP_SPACE))
			    == AEE_SUCCESS);
		}
				...
				if(((t1 + waittime) > t2)
				   && (((t1 + waittime) - t2)
					...
					FARF(LOW,
					     ...
					     "%"
					     "u",
					     (unsigned)EARLY_WAKEUP_SLEEP);
				}
				get_queue_state_read(q->header, pq, &qleft, &r,
				                     NULL);
				if(qleft >= 8) {
					if(t2 != 0) {
						q->early_wakeup_wait
		...
		VERIFY((nErr = peek_locked(qp, r, &f, &num_b, &msg_l, &header))
		       == 0);
	}
	...
	if((f & DSPQUEUE_PACKET_FLAG_BUFFERS) && (buffers != NULL)) {
		if(max_buffers < num_b) {
			FARF(ERROR, "Too many buffer references in "
			            "packet to fit in output buffer");
			nErr = AEE_EBADPARM;
			goto bail;
		}
	}
	...
	if((f & DSPQUEUE_PACKET_FLAG_MESSAGE) && (message != NULL)) {
		if(max_message_length < msg_l) {
			FARF(ERROR, "Message in packet too large to "
	...
	if((buffers != NULL) && (num_b > 0)) {
		for(i = 0; i < num_b; i++) {
			...
			// Find buffer in internal FastRPC
			// structures and handle refcounts
			if(b->flags & DSPQUEUE_BUFFER_FLAG_REF) {
				VERIFYC((b->flags & DSPQUEUE_BUFFER_FLAG_DEREF)
				            == 0,
				        AEE_EBADPARM);
				nErr = fastrpc_buffer_ref(q->domain, b->fd, 1,
				                          &va, &size);
			} else if(b->flags & DSPQUEUE_BUFFER_FLAG_DEREF) {
				nErr = fastrpc_buffer_ref(q->domain, b->fd, -1,
				                          &va, &size);
			} else {
				nErr = fastrpc_buffer_ref(q->domain, b->fd, 0,
				                          &va, &size);
			}
			if(nErr == AEE_ENOSUCHMAP) {
				FARF(ERROR,
			...
			// Check and use offset and size from
			// the packet if specified
			if(b->size != 0) {
				uint64_t bend = ((uint64_t)b->offset)
				                + ((uint64_t)b->size);
...
bail:
	for(i = 0; i < buf_refs; i++) {
		...
		struct dspqueue_buffer *b = &buffers[i];
		if(b->flags & DSPQUEUE_BUFFER_FLAG_REF) {
			nErr = fastrpc_buffer_ref(q->domain, b->fd, -1, NULL,
			                          NULL);
		} else if(b->flags & DSPQUEUE_BUFFER_FLAG_DEREF) {
			nErr = fastrpc_buffer_ref(q->domain, b->fd, 1, NULL,
			                          NULL);
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for queue %p errno %s",
		     nErr, __func__, queue, strerror(errno));
	}
	return nErr;
}

AEEResult
dspqueue_read(dspqueue_t queue, uint32_t *flags, uint32_t max_buffers,
              uint32_t *num_buffers, struct dspqueue_buffer *buffers,
              uint32_t max_message_length, uint32_t *message_length,
              uint8_t *message, uint32_t timeout_us)
{
	...
	if(q->mdq.is_mdq) {
		...
		return dspqueue_multidomain_read(
		    q, flags, max_buffers, num_buffers, buffers,
		    max_message_length, message_length, message, timeout_us,
		    true, true);
	}
	...
	// Try a read first before dealing with timeouts
	nErr = dspqueue_read_noblock(queue, flags, max_buffers, num_buffers,
	                             buffers, max_message_length,
	                             message_length, message);
	if(nErr != AEE_EWOULDBLOCK) {
		// Have a packet or got an error
	...
	if(q->have_wait_counts) {
		...
		waiting = 1;
		nErr = dspqueue_read_noblock(
		    queue, flags, max_buffers, num_buffers, buffers,
...
static int
dspqueue_multidomain_get_stat(struct dspqueue *q, enum dspqueue_stat stat,
                              uint64_t *value)
{
	int nErr = AEE_SUCCESS;
	struct dspqueue_multidomain *mdq = NULL;
	...
	if(mdq->num_domain_ids > 1) {
		/* Stats requests are not supported for multi-domain
		 * queues */
		nErr = AEE_EUNSUPPORTED;
		goto bail;
	}
	 ...
	 */
	VERIFY(AEE_SUCCESS
	       == (nErr = dspqueue_get_stat(mdq->queues[0], stat, value)));
bail:
	if(locked)
		pthread_mutex_unlock(&q->mutex);
...
AEEResult
dspqueue_get_stat(dspqueue_t queue, enum dspqueue_stat stat, uint64_t *value)
{
	...
	case DSPQUEUE_STAT_READ_QUEUE_PACKETS: {
		...
		struct dspqueue_packet_queue_state *write_state
		    = (struct dspqueue_packet_queue_state
		           *)(((uintptr_t)q->header) + pq->write_state_offset);
		uint32_t c;
	...
	case DSPQUEUE_STAT_WRITE_QUEUE_PACKETS: {
		...
		struct dspqueue_packet_queue_state *read_state
		    = (struct dspqueue_packet_queue_state
		           *)(((uintptr_t)q->header) + pq->read_state_offset);
		uint32_t c;
...
static void *
error_callback_thread(void *arg)
{
	...
	FARF(ALWAYS, "%s starting for queue %p with id %u", __func__, a->queue,
	     a->queue->id);
	a->queue->error_callback(a->queue, a->error,
	                         a->queue->callback_context);
	free(a);
	return NULL;
}
...
static void
error_callback(struct dspqueue_domain_queues *dq, AEEResult error)
{
			...
			if(q->error_callback != NULL) {
				...
				if(a != NULL) {
					...
					pthread_attr_setdetachstate(
					    &tattr, PTHREAD_CREATE_JOINABLE);
					a->queue = q;
					a->error = error;
					err = pthread_create(
					    &q->error_callback_thread, &tattr,
...
static void *
dspqueue_send_signal_thread(void *arg)
{

	struct dspqueue_domain_queues *dq
	    = (struct dspqueue_domain_queues *)arg;
	AEEResult nErr = 0;
	...
	while(1) {
		...
		while(dq->send_signal_mask == 0) {
			pthread_cond_wait(&dq->send_signal_cond,
			                  &dq->send_signal_mutex);
		}
		if(dq->send_signal_mask & SIGNAL_BIT_CANCEL) {
			...
			VERIFY((nErr = dspqueue_rpc_signal(dq->dsp_handle))
			       == 0);
		}
	}

bail:
	FARF(ERROR, "dspqueue_send_signal_thread failed with %d errno %s",
	     nErr, strerror(errno));
...
static void *
dspqueue_receive_signal_thread(void *arg)
{
	...
	while(1) {
		int32_t signal;
		VERIFY(
		    (nErr = dspqueue_rpc_wait_signal(dq->dsp_handle, &signal))
		    == 0);
		...
		// Got a signal - at least one queue has more packets
		// or space. Find out which one and signal it.
		FARF(LOW, "Got signal");
		...
		for(i = 0; i <= dq->max_queue; i++) {
			if((dq->queues[i] != UNUSED_QUEUE)
			   && (dq->queues[i] != INVALID_QUEUE)) {
				...
				pthread_mutex_lock(&q->packet_mutex);
				if(q->resp_packet_count
				   != dq->state->resp_packet_count[i]) {
					q->resp_packet_count
					    = dq->state->resp_packet_count[i];
					FARF(LOW,
					     "Q"
				...
				pthread_mutex_unlock(&q->packet_mutex);
				pthread_mutex_lock(&q->space_mutex);
				if(q->req_space_count
				   != dq->state->req_space_count[i]) {
					q->req_space_count
					    = dq->state->req_space_count[i];
					FARF(LOW,
					     "Q"
...
bail:
	FARF(ERROR, "dspqueue_receive_signal_thread failed with %d errno %s",
	     nErr, strerror(errno));
	if(nErr == -1) {
		// Process died (probably)
		nErr = AEE_ECONNRESET;
...
static void *
dspqueue_packet_callback_thread(void *arg)
{
	...
	struct dspqueue_packet_queue_state *write_state
	    = (struct dspqueue_packet_queue_state *)(((uintptr_t)q->header)
	                                             + pq->write_state_offset);
	_Atomic uint32_t *wait_count
	    = (_Atomic uint32_t *)&read_state->wait_count;
	...
	while(1) {
		...
		// Call the callback if we have any packets we haven't
		// seen yet.
		cache_invalidate_word(&write_state->packet_count);
		...
		// Mark we're waiting and call again if we just got
		// more packets
		if(q->have_wait_counts) {
			atomic_fetch_add(wait_count, 1);
			cache_flush_word(wait_count);
		...
		// Wait for a signal
		nErr
		    = wait_signal_locked(q, DSPQUEUE_SIGNAL_RESP_PACKET, NULL);
		if(nErr == AEE_EINTERRUPTED || nErr == AEE_EBADSTATE) {
			FARF(HIGH, "Queue %u exit callback thread",
			     (unsigned)q->id);
			if(q->have_wait_counts) {
				atomic_fetch_sub(wait_count, 1);
				cache_flush_word(wait_count);
...
static int
dspqueue_notif_callback(void *context, int domain, int session,
                        remote_rpc_status_flags_t status)
{
	...
	if(session && domain < NUM_DOMAINS) {
		// Did not receive effective domain ID for extended
		// session. Compute it.
		effec_domain_id = GET_EFFECTIVE_DOMAIN_ID(domain, session);
	}
	FARF(ALWAYS, "%s for domain %d, session %d, status %u", __func__,
