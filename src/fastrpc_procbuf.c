...
int proc_sharedbuf_init(int dev, int domain) {
  ...
  errno = 0;
  VERIFYC(NULL != (proc_sharedbuf = rpcmem_alloc_internal(
                       0, RPCMEM_HEAP_DEFAULT, (size_t)proc_sharedbuf_size)),
          AEE_ENORPCMEMORY);
  hlist[domain].proc_sharedbuf = proc_sharedbuf;
  VERIFYC(-1 != (sharedbuf_info.buf_fd = rpcmem_to_fd_internal(proc_sharedbuf)),
          AEE_ERPC);
  ...
  if (ioErr) {
    if (errno == ENOTTY) {
      ...
      FARF(ERROR,
           "Error 0x%x: %s: sharedbuff capability not supported by kernel "
           "(errno %d, %s).",
...
static int get_non_preload_lib_names(char **lib_names, size_t *buffer_size,
                                     int domain) {
  ...
  VERIFYC(NULL != (data_paths = calloc(1, sizeof(char) * dsp_search_path_len)),
          AEE_ENOMEMORY);
  VERIFYC(AEE_SUCCESS == apps_std_getenv(DSP_LIBRARY_PATH, data_paths,
      ...
      if (entry->d_type == DT_REG) {
        ...
        if (std_strstr(file, FILE_EXT) != NULL) {
          if (concat_len + std_strlen(file) > MAX_NON_PRELOAD_LIBS_LEN) {
            FARF(ALWAYS,
                 "ERROR: Failed to pack library names in custom "
                 "DSP_LIBRARY_PATH as required buffer size exceeds Max limit "
                 "(%d).",
...
static int pack_proc_shared_buf_params(int domain, uint32_t param_id,
  ...
  if (param_addr == NULL || param_size <= 0 || param_id < 0 ||
    ...
    FARF(ERROR, "Error: %s: invalid param %u or size %u or addr 0x%x", __func__,
         param_id, param_size, param_addr);
    return AEE_EBADPARM;
  }
  if (buf_write_addr == NULL) {
    /*
     * Total no of param ids (4 bytes) are packed at shared buffer initial
     * address, so add 4 bytes to start pack process params
     */
    buf_write_addr = (uint32_t *)((char *)buf_start_addr + WORD_SIZE);
  }
  /* Align the params size in multiple of 4 bytes (word) for easy unpacking at
   * DSP */
  align_param_size = ALIGN_B(param_size, WORD_SIZE);

  if (buf_last_addr < (uint32_t *)((char *)buf_write_addr + align_param_size +
                                   sizeof(param_id))) {
    FARF(ERROR,
         "Error: %s: proc shared buffer exhausted to pack param_id:%u params",
         __func__, param_id);
    return AEE_ERPC;
  }
  /* Write param_id */
  ...
  *buf_write_addr =
      (*buf_write_addr) + (PROC_ATTR_BUF_ID_SIZE_MASK & align_param_size);
  buf_write_addr++;

  std_memscpy(buf_write_addr, (buf_last_addr - buf_write_addr), param_addr,
              param_size);
...
void fastrpc_process_pack_params(int dev, int domain) {
  ...
  nErr = pack_proc_shared_buf_params(domain, HLOS_PID_ID, &pid, sizeof(pid));
  if (nErr) {
  ...
  if (nErr) {
    FARF(ERROR,
         "Error 0x%x: %s: Failed to pack thread parameters in shared buffer",
         nErr, __func__);
  }
  ...
  if (nErr) {
    FARF(ERROR,
         "Error 0x%x: %s: Failed to pack process config parameters in shared "
         "buffer",
         nErr, __func__);
  }
  ...
  if (err_codes_to_send) {
    ...
    if (nErr) {
      FARF(ERROR,
           "Error 0x%x: %s: Failed to pack panic error codes in shared buffer",
           nErr, __func__);
    }
  }
  nErr = pack_proc_shared_buf_params(domain, HLOS_PROC_EFFEC_DOM_ID, &domain,
                                     sizeof(domain));
  if (nErr) {
    FARF(ERROR,
         "Error 0x%x: %s: Failed to pack effective domain id %d in shared "
         "buffer",
  ...
  if (lib_names) {
    if (AEE_SUCCESS ==
        get_non_preload_lib_names(&lib_names, &buffer_size, domain)) {
      nErr = pack_proc_shared_buf_params(domain, CUSTOM_DSP_SEARCH_PATH_LIBS_ID,
                                         lib_names, buffer_size);
      if (nErr) {
        FARF(ERROR,
             "Error 0x%x: %s: Failed to pack the directory list in shared "
             "buffer",
             nErr, __func__);
      }
    }
