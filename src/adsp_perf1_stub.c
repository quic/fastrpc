...
static const uint16_t methodStrings[13]
    = { 10, 14, 27, 19, 47, 56, 63, 0, 52, 34, 60, 41, 63 };
...
static __inline int
_stub_method_1(remote_handle64 _handle, uint32_t _mid, char *_rout0[1],
               uint32_t _rout0Len[1])
{
	...
	_TRY_FARF(_nErr, __QAIC_REMOTE(remote_handle64_invoke)(
			     _handle,
			     REMOTE_SCALARS_MAKEX(0, _mid, 1, 1, 0, 0), _pra));
	_CATCH_FARF(_nErr)
	{
		_QAIC_FARF(RUNTIME_ERROR,
		           "ERROR 0x%x: handle=0x%" PRIx64
		           ", scalar=0x%x, method ID=%d: %s failed\n",
		           _nErr, _handle,
		           REMOTE_SCALARS_MAKEX(0, _mid, 1, 1, 0, 0), _mid,
		           __func__);
	}
	return _nErr;
}
__QAIC_STUB_EXPORT int
__QAIC_STUB(adsp_perf1_get_usecs)(remote_handle64 _handle, int64_t *dst,
...
static __inline int
_stub_method_2(remote_handle64 _handle, uint32_t _mid, char *_rout0[1],
