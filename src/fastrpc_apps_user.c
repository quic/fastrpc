...
inline static void
deinit_fastrpc_dsp_lib_refcnt(void)
{
	int ii = 0;
...
enum fastrpc_proc_attr {
	...
	FASTRPC_MODE_PRIVILEGED
	= 0x40, // this attribute will be populated in kernel
	// Attribute to enable pd dump feature for both signed/unsigned pd
	FASTRPC_MODE_ENABLE_PDDUMP = 0x80,
	// System attribute to enable pd dump debug data collection on rooted
	// devices
	...
	// Attribute to set Leak detect for heap. Bits 19-20 are reserved for
	// leak detect.
	FASTRPC_MODE_ENABLE_LEAK_DETECT = 0x180000,
	// Attribute to change caller stack for heap. Bits 21-23 are reserved
	// the call stack num
...
const char *ENV_DEBUG_VAR_NAME[] = { "FASTRPC_PROCESS_ATTRS",
	                             ...
	                             "ro.debuggable" };
const char *ANDROIDP_DEBUG_VAR_NAME[]
...
int
fastrpc_session_open(int domain, int *dev)
{
	int device = -1;
...
void
fastrpc_session_close(int domain, int dev)
{
	if(!hlist)
		return;
...
int
fastrpc_session_get(int domain)
{
	int ref = -1;
	do {
		if(hlist) {
			pthread_mutex_lock(&hlist[domain].mut);
			if(hlist[domain].state
			   == FASTRPC_DOMAIN_STATE_DEINIT) {
...
int
fastrpc_session_put(int domain)
{
	int ref = -1;
	do {
		if(hlist) {
...
int
fastrpc_session_dev(int domain, int *dev)
{
	*dev = INVALID_DEVICE;
...
static void
GenCrc32Tab(uint32_t GenPoly, uint32_t *crctab)
{
	uint32_t crc;
	int i, j;
...
static uint32_t
crc32_lut(unsigned char *data, int nbyte, uint32_t *crctab)
{
	uint32_t crc = 0;
	if(!data || !crctab)
		return 0;
...
int
property_get_int32(const char *name, int value)
{
	return 0;
}

int
property_get(const char *name, int *def, int *value)
{
	return 0;
}

int
fastrpc_get_property_int(fastrpc_properties UserPropKey, int defValue)
{
	if(((int)UserPropKey > NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR,
		     "%s: Index %d out-of-bound for "
		     "ENV_DEBUG_VAR_NAME array of len %d",
		     __func__, UserPropKey,
		     NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS);
		return defValue;
	}
	...
	if(((int)UserPropKey > NO_ANDROIDP_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR,
		     "%s: Index %d out-of-bound for "
		     "ANDROIDP_DEBUG_VAR_NAME array of len %d",
		     __func__, UserPropKey,
		     NO_ANDROIDP_DEBUG_VAR_NAME_ARRAY_ELEMENTS);
		return defValue;
	}
	...
	if(((int)UserPropKey > NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR,
		     "%s: Index %d out-of-bound for "
		     "ANDROID_DEBUG_VAR_NAME array of len %d",
		     __func__, UserPropKey,
		     NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS);
		return defValue;
	}
	return (int)property_get_int32(ANDROID_DEBUG_VAR_NAME[UserPropKey],
	                               defValue);
#endif
...
int
fastrpc_get_property_string(fastrpc_properties UserPropKey, char *value,
                            char *defValue)
{
	int len = 0;
	if(((int)UserPropKey > NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
	...
	return property_get(ANDROID_DEBUG_VAR_NAME[UserPropKey], value,
	                    defValue);
#endif
#else // non-Android platforms
	if(defValue != NULL) {
...
static inline int
is_first_reverse_rpc_call(int domain, remote_handle handle, uint32_t sc)
{
	int ret = 0;

	if(IS_REVERSE_RPC_CALL(handle, sc)
	   && IS_SESSION_OPEN_ALREADY(domain)) {
...
static inline void
trace_marker_deinit(int domain)
{
	if(hlist[domain].trace_marker_fd > 0) {
...
int
fastrpc_set_remote_uthread_params(int domain)
{
	...
	if((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
		nErr = remotectl1_set_param(handle, th_params->reqID,
		                            (uint32_t *)th_params, paramsLen);
		if(nErr) {
			FARF(ALWAYS,
			     "Warning 0x%x: %s: remotectl1 "
			     "domains not supported for "
			     "domain %d\n",
			...
			// Set remotectlhandle to
			// INVALID_HANDLE, so that all
			// subsequent calls are non-domain
			// calls
			hlist[domain].remotectlhandle = INVALID_HANDLE;
			VERIFY(AEE_SUCCESS
			       == (nErr = remotectl_set_param(
				       th_params->reqID, (uint32_t *)th_params,
				       paramsLen)));
		}
	} else {
		VERIFY(AEE_SUCCESS
...
bail:
	if(nErr != AEE_SUCCESS) {
		if(th_params) {
			FARF(ERROR,
			     "Error 0x%x: %s failed domain %d "
			     "thread priority %d stack size %d "
			     "(errno %s)",
			     nErr, __func__, domain,
			     th_params->thread_priority, th_params->stack_size,
			     strerror(errno));
		} else {
		...
		FARF(ALWAYS,
		     "Successfully set remote user thread priority "
		     "to %d and stack size to "
		     "%d for domain %d",
		     th_params->thread_priority, th_params->stack_size,
		     domain);
	}
...
static inline bool
is_valid_local_handle(int domain, struct handle_info *hinfo)
{
	QNode *pn;
	int ii = 0;
	if(domain == -1) {
		FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
		{
			...
			{
				struct handle_info *hi = STD_RECOVER_REC(
				    struct handle_info, qn, pn);
				if(hi == hinfo) {
		...
		QLIST_FOR_ALL(&hlist[domain].ql, pn)
		{
			struct handle_info *hi
			    = STD_RECOVER_REC(struct handle_info, qn, pn);
			if(hi == hinfo) {
...
static int
verify_local_handle(int domain, remote_handle64 local)
{
...
int
get_domain_from_handle(remote_handle64 local, int *domain)
{
	struct handle_info *hinfo = (struct handle_info *)(uintptr_t)local;
	...
	VERIFYM(
	    IS_VALID_EFFECTIVE_DOMAIN_ID(dom), AEE_EINVHANDLE,
 ...
 */
static int
get_domain_from_domain_name(const char *domain_name, int domain_name_len)
{
	...
	if(domain_name) {
		...
		} else if(!strncmp(domain_name,
		                   SUBSYSTEM_NAME[CDSP1_DOMAIN_ID],
...
static const char *
get_domain_from_id(int domain_id)
{
...
static int
get_handle_remote(remote_handle64 local, remote_handle64 *remote)
{
	struct handle_info *hinfo = (struct handle_info *)(uintptr_t)local;
	int nErr = AEE_SUCCESS;
...
print_open_handles(int domain)
{
	...
	QLIST_FOR_ALL(&hlist[domain].ql, pn)
	{
		...
		if(hi->name)
			FARF(ALWAYS, "%s, handle 0x%" PRIx64 "", hi->name,
			     hi->remote);
	}
	pthread_mutex_unlock(&hlist[domain].mut);
}
...
static int
fastrpc_alloc_handle(int domain, QList *me, remote_handle64 remote,
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for local handle 0x%x, "
		     "remote handle 0x%x, "
		     "domain %d\n",
...
static int
fastrpc_free_handle(int domain, QList *me, remote_handle64 remote)
{
	pthread_mutex_lock(&hlist[domain].lmut);
	if(!QList_IsEmpty(me)) {
...
int
fastrpc_update_module_list(uint32_t req, int domain, remote_handle64 h,
                           remote_handle64 *local, const char *name)
{
	int nErr = AEE_SUCCESS;
	...
	case DOMAIN_LIST_DEQUEUE: {
		VERIFY(AEE_SUCCESS
		       == (nErr = fastrpc_free_handle(domain,
		                                      &hlist[domain].ql, h)));
		if(IS_CONST_HANDLE(h)) {
			pthread_mutex_lock(&hlist[domain].lmut);
			hlist[domain].constCount--;
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for request ID %u, "
		     "handle 0x%x, domain %d\n",
		     nErr, __func__, req, h, domain);
	} else {
		FARF(RUNTIME_RPC_HIGH,
		     "Library D count %d, C count %d, N count %d, R "
		     "count %d\n",
...
static void
fastrpc_clear_handle_list(uint32_t req, int domain)
{
	int nErr = AEE_SUCCESS;
	QNode *pn = NULL;
	...
	case NON_DOMAIN_HANDLE_LIST_ID: {
		...
		if(!QList_IsNull(&hlist[domain].nql)) {
			while((pn = QList_Pop(&hlist[domain].nql))) {
				struct handle_info *h = STD_RECOVER_REC(
				    struct handle_info, qn, pn);
				free(h);
				h = NULL;
			}
	...
	case REVERSE_HANDLE_LIST_ID: {
		if(!QList_IsNull(&hlist[domain].rql)) {
			while((pn = QList_Pop(&hlist[domain].rql))) {
				struct handle_info *hi = STD_RECOVER_REC(
				    struct handle_info, qn, pn);
				close_reverse_handle(hi->local, dlerrstr,
...
static int
wakelock_control_kernel_pm(int domain, int dev, uint32_t timeout)
{
	int nErr = AEE_SUCCESS;
	...
	if(nErr) {
		if(errno == EBADRQC || errno == ENOTTY) {
			VERIFY_WPRINTF("Warning: %s: kernel does not "
			               "support PM management (%s)",
			               __func__, strerror(errno));
		} else if(errno == EACCES || errno == EPERM) {
			VERIFY_WPRINTF("Warning: %s: application does "
			               "not have permission for PM "
			               "management (%s)",
			               __func__, strerror(errno));
		}
		FARF(ERROR,
		     "Error 0x%x: %s PM control failed for domain "
		     "%d, dev %d with timeout "
		     "%d (errno %s)",
...
static void
fastrpc_timer_callback(void *ptr)
{
	...
	FARF(ALWAYS,
	     "%s fastrpc time out of %d ms on thread %d on domain %d sc 0x%x "
	     "handle 0x%x\n",
	...
	if(nErr) {
		FARF(ERROR,
		     "%s: Failed to create exception in the remote "
		     "process on domain %d "
		     "(errno %s)",
...
static void
fastrpc_add_timer(fastrpc_timer *frpc_timer)
{
...
static void
fastrpc_delete_timer(timer_t *timer)
{
	int nErr = AEE_SUCCESS;
...
int
remote_handle_invoke_domain(int domain, remote_handle handle,
                            fastrpc_async_descriptor_t *desc, uint32_t sc,
                            remote_arg *pra)
{
	int dev, total, bufs, handles, i, nErr = 0, wake_lock = 0,
					  rpc_timeout = 0;
	unsigned req;
	uint32_t len;
	...
	if(IS_QTF_TRACING_ENABLED(hlist[domain].procattrs)
	   && !IS_STATIC_HANDLE(handle) && trace_marker_fd > 0) {
		write(trace_marker_fd, INVOKE_BEGIN_TRACE_STR,
		      invoke_begin_trace_strlen);
		trace_enabled = true;
	}
	...
	if(fastrpc_wake_lock_enable[domain]) {
		if(!IS_REVERSE_RPC_CALL(handle, sc)
		   || is_first_reverse_rpc_call(domain, handle, sc)) {
			...
			/* Since wake-lock is not released at the end
			 * of previous "remote_handle_invoke" for
			 * subsequent reverse RPC calls, it doesn't
			 * have to be taken again here.
			 * It will be released before "ioctl invoke"
			 * call to kernel */
			wake_lock = 1;
	}
	...
	if(desc) {
		...
		asyncjob.jobid
		    = ((((time_spec.tv_sec) / SECONDS_PER_HOUR)
	...
	for(i = 0; i < bufs; i++) {
		...
		if(pra[i].buf.nLen) {
			...
			VERIFY(AEE_SUCCESS
			       == (nErr = fdlist_fd_from_buf(
				       pra[i].buf.pv, (int)pra[i].buf.nLen,
				       &nova, &base, &attr, &fd)));
			if(fd != -1) {
				set_args_fd(i, fd);
				req = INVOKE_FD;
			}
			// AsyncRPC doesn't support Non-ion
			// output buffers
			if(asyncjob.isasyncjob
			   && i >= (int)REMOTE_SCALARS_INBUFS(sc)) {
			...
			if(nova) {
				...
				// pra[i].buf.pv =
				// (void*)((uintptr_t)pra[i].buf.pv
				// - (uintptr_t)base);
				VERIFY_IPRINTF("nova buffer idx: "
				               "%d addr: %p "
				               "size: %d",
				               i, pra[i].buf.pv,
				               pra[i].buf.nLen);
			...
			if(attr & FASTRPC_ATTR_COHERENT) {
				...
				VERIFY_IPRINTF("coherent buffer "
				               "idx: %d addr: %p "
				               "size: %d",
				               i, pra[i].buf.pv,
				               pra[i].buf.nLen);
			...
			if(attr & FASTRPC_ATTR_FORCE_NOINVALIDATE) {
				req = INVOKE_ATTRS;
				append_args_attr(
				    i, FASTRPC_ATTR_FORCE_NOINVALIDATE);
				VERIFY_IPRINTF("force no "
				               "invalidate "
				               "buffer idx: %d "
				               "addr: %p size: "
				               "%d",
	...
	for(i = bufs; i < total; i++) {
		...
		if(hlist[domain].dma_handle_reverse_rpc_map_capability
		   && (attr & FASTRPC_ATTR_NOMAP)) {
			// Register fd again, for reverse RPC
			// call to retrive FASTRPC_ATTR_NOMAP
			// flag for fd
			remote_register_dma_handle_attr(pra[i].dma.fd, len,
			                                FASTRPC_ATTR_NOMAP);
		}
		dma_fd = pra[i].dma.fd;
		set_args(i, (void *)(uintptr_t)pra[i].dma.offset, len, dma_fd,
		         attr);
		append_args_attr(i, FASTRPC_ATTR_NOVA);
	}
	...
	if(IS_CRC_CHECK_ENABLED(hlist[domain].procattrs)
	   && (!IS_STATIC_HANDLE(handle)) && !asyncjob.isasyncjob) {
		...
		VERIFYC(crc_local != NULL && crc_remote != NULL,
		        AEE_ENOMEMORY);
		VERIFYC(!(NULL == pra && nInBufs > 0), AEE_EBADPARM);
		for(i = 0; (i < nInBufs) && (i < M_CRCLIST); i++)
			crc_local[i]
			    = crc32_lut((unsigned char *)pra[i].buf.pv,
		...
		for(i = 0; (i < bufs) && (i < M_CRCLIST); i++) {
			if(crc_local[i] != crc_remote[i]) {
				FARF(ERROR,
				     ...
				     "crc local %x "
				     "remote %x",
				     i, bufs, crc_local[i], crc_remote[i]);
				break;
			}
		}
	}
...
bail:
	...
	if(wake_lock) {
		// Keep holding wake-lock for reverse RPC calls to keep
		// CPU awake for any further processing
		if(!IS_REVERSE_RPC_CALL(handle, sc)) {
			fastrpc_wake_unlock();
			wake_lock = 0;
		}
	...
	if(trace_enabled) {
		write(trace_marker_fd, INVOKE_END_TRACE_STR,
		      invoke_end_trace_strlen);
	}
	if(nErr != AEE_SUCCESS) {
		...
		// on domain %d (sc 0x%x)\n", nErr, __func__,
		// (int)handle, domain, sc);
	}
	return nErr;
}

int
remote_handle_invoke(remote_handle handle, uint32_t sc, remote_arg *pra)
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s called with handle 0x%x , scalar 0x%x",
	                       __func__, (int)handle, sc);
	VERIFYC(handle != (remote_handle)-1, AEE_EINVHANDLE);

	domain = get_current_domain();
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_invoke_domain(domain, handle, NULL, sc,
	                                              pra)));
bail:
	...
	if(nErr != AEE_SUCCESS) {
		...
		/*
		 * handle_info or so name cannot be obtained from
		 * remote handles which are used for non-domain calls.
		 */
		if(0 == check_rpc_error(nErr)) {
			if(get_logger_state(domain)) {
				FARF(ERROR,
				     ...
				     "(sc 0x%x) (errno "
				     "%s)\n",
				     nErr, __func__, (int)handle,
				     REMOTE_SCALARS_METHOD(sc), domain, sc,
				     strerror(errno));
			}
		}
	}
...
int
remote_handle64_invoke(remote_handle64 local, uint32_t sc, remote_arg *pra)
{
	...
	if(IS_STATICPD_HANDLE(local)) {
		nErr = AEE_EINVHANDLE;
		FARF(ERROR,
		     "Error 0x%x: %s cannot be called for staticPD "
		     "handle 0x%" PRIx64 "\n",
		     nErr, __func__, local);
		goto bail;
	}
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_invoke_domain(domain, remote, NULL, sc,
	                                              pra)));
bail:
	FASTRPC_PUT_REF(domain);
	if(nErr != AEE_SUCCESS) {
		...
		if(0 == check_rpc_error(nErr)) {
			if(get_logger_state(domain)) {
				FARF(ERROR,
				     ...
				     "failed for "
				     "module %s, "
				     "handle 0x%" PRIx64 ", method %d on "
				     "domain %d (sc "
				     "0x%x) (errno "
				     "%s)\n",
				     nErr, __func__, h->name, local,
				     REMOTE_SCALARS_METHOD(sc), domain, sc,
				     strerror(errno));
			}
		}
...
int
remote_handle_invoke_async(remote_handle handle,
                           fastrpc_async_descriptor_t *desc, uint32_t sc,
                           remote_arg *pra)
{
	int domain = -1, nErr = AEE_SUCCESS, ref = 0;
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_invoke_domain(domain, handle, desc, sc,
	                                              pra)));
bail:
	FASTRPC_PUT_REF(domain);
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for handle "
			     "0x%x, method %d async type %d on "
			     "domain %d (sc 0x%x) (errno %s)\n",
			     nErr, __func__, (int)handle,
			     REMOTE_SCALARS_METHOD(sc), desc->type, domain, sc,
			     strerror(errno));
		}
	}
	FASTRPC_ATRACE_END();
...
int
remote_handle64_invoke_async(remote_handle64 local,
                             fastrpc_async_descriptor_t *desc, uint32_t sc,
                             remote_arg *pra)
{
	remote_handle64 remote = 0;
	...
	VERIFY(AEE_SUCCESS == (nErr = get_handle_remote(local, &remote)));
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_invoke_domain(domain, remote, desc, sc,
	                                              pra)));
bail:
	FASTRPC_PUT_REF(domain);
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for "
			     "handle 0x%" PRIx64
			     ", method %d on domain %d (sc "
			     "0x%x) (errno %s)\n",
			     nErr, __func__, local, REMOTE_SCALARS_METHOD(sc),
...
int
remote_handle_open_domain(int domain, const char *name, remote_handle *ph,
                          uint64_t *t_spawn, uint64_t *t_load)
{
	...
	FASTRPC_ATRACE_BEGIN_L(
	    "%s called with domain %d, name %s, handle 0x%x", __func__, domain,
	    name, ph);
	/* If the total reference count exceeds one then exit the application.
	 */
	if(total_dsp_lib_refcnt > MAX_LIB_INSTANCE_ALLOWED) {
		FARF(ERROR,
		     "Error: aborting due to %d instances of "
		     "libxdsprpc. Only %d allowed\n",
	...
	if(!strncmp(name, ITRANSPORT_PREFIX "attachguestos",
		...
		FARF(RUNTIME_RPC_HIGH, "setting attach mode to guestos : %d",
		     domain);
	...
	if(!strncmp(name, ITRANSPORT_PREFIX "createstaticpd",
		...
		FARF(RUNTIME_RPC_HIGH, "creating static pd on domain: %d",
		     domain);
		name_len = strlen(name);
		VERIFYC(NULL
		            != (pdname_uri = (char *)malloc((name_len + 1)
		                                            * sizeof(char))),
		        AEE_ENOMEMORY);
		strlcpy(pdname_uri, name, name_len + 1);
		char *pdName
		    = pdname_uri + strlen(ITRANSPORT_PREFIX "createstaticpd:");

		/*
		 * Support sessions feature for static PDs.
		 * For eg, the same app can call 'remote_handle64_open'
		 * with "createstaticpd:sensorspd&_dom=adsp&_session=0"
		 * and "createstaticpd:oispd&_dom=adsp&_session=1" to
		 * create a session on both static PDs.
		 */
		if(strstr(pdName, get_domain_from_id(
				      GET_DOMAIN_FROM_EFFEC_DOMAIN_ID(domain)))
		   && strstr(pdName, FASTRPC_SESSION_URI)) {
			strlcpy(pdName, pdName,
			        (strlen(pdName)
			         - strlen(get_domain_from_id(
				     GET_DOMAIN_FROM_EFFEC_DOMAIN_ID(domain)))
			         - strlen(FASTRPC_SESSION1_URI) + 1));
		} else if(strstr(pdName, get_domain_from_id(domain))) {
			strlcpy(pdName, pdName,
			        (strlen(pdName)
			         - strlen(get_domain_from_id(domain)) + 1));
		}
	...
	if(!strncmp(name, ITRANSPORT_PREFIX "attachuserpd",
		...
		FARF(RUNTIME_RPC_HIGH, "setting attach mode to userpd : %d",
		     domain);
		hlist[domain].dsppd = USERPD;
		return AEE_SUCCESS;
	}
	PROFILE_ALWAYS(
	    t_spawn, VERIFY(AEE_SUCCESS == (nErr = domain_init(domain, &dev)));
	...
	PROFILE_ALWAYS(
	    ...
	    if((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
		    ...
		    if(nErr) {
			    FARF(ALWAYS,
			         "Warning 0x%x: %s: remotectl1 "
			         "domains not supported for domain "
			         "%d\n",
			         nErr, __func__, domain);
			    fastrpc_update_module_list(
				DOMAIN_LIST_DEQUEUE, domain,
				_const_remotectl1_handle, NULL, NULL);

			    // Set remotectlhandle to INVALID_HANDLE,
			    // so that all subsequent calls are
			    // non-domain calls
			    hlist[domain].remotectlhandle = INVALID_HANDLE;
			    VERIFY(AEE_SUCCESS
			           == (nErr = remotectl_open(
					   name, (int *)ph, dlerrstr,
					   sizeof(dlerrstr), &dlerr)));
		    }
	    } else {
		    VERIFY(
...
bail:
	if(dlerr != 0) {
		FARF(ERROR,
		     "Error 0x%x: %s: dynamic loading failed for %s "
		     "on domain %d (dlerror "
		     "%s) (errno %s)\n",
	...
	if(nErr == AEE_ECONNRESET) {
		if(!hlist[domain].domainsCount
		   && !hlist[domain].nondomainsCount) {
			/* Close session if there are no open
			 * remote handles */
...
int
remote_handle_open(const char *name, remote_handle *ph)
{
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_open_domain(domain, name, ph, &t_spawn,
	                                            &t_load)));
	fastrpc_update_module_list(NON_DOMAIN_LIST_PREPEND, domain, *ph,
	                           &local, name);
bail:
	if(nErr) {
		if(*ph) {
		...
		FARF(ALWAYS,
		     "%s: Successfully opened handle 0x%x for %s on "
		     "domain %d (spawn time "
...
int
remote_handle64_open(const char *name, remote_handle64 *ph)
{
	remote_handle h = 0;
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_open_domain(domain, name, &h, &t_spawn,
	                                            &t_load)));
	/* Returning local handle to "geteventd" call causes bad fd error when
	   daemon polls on it, hence return remote handle (which is the actual
	   fd) for "geteventd" call*/
	if(!strncmp(name, ITRANSPORT_PREFIX "geteventfd",
	            strlen(ITRANSPORT_PREFIX "geteventfd"))
	   || IS_STATICPD_HANDLE(h)) {
		*ph = h;
	} else {
		fastrpc_update_module_list(DOMAIN_LIST_PREPEND, domain, h,
		                           &local, name);
...
bail:
	if(nErr) {
		...
		FARF(ALWAYS,
		     "%s: Successfully opened handle 0x%" PRIx64
		     " (remote 0x%" PRIx64
...
int
remote_handle_close_domain(int domain, remote_handle h)
{
	char *dlerrstr = NULL;
	int dlerr = 0, nErr = AEE_SUCCESS;
	...
	VERIFYC(NULL != (dlerrstr = (char *)calloc(1, err_str_len)),
	        AEE_ENOMEMORY);
	PROFILE_ALWAYS(
	    &t_close,
	    if((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
		    nErr = remotectl1_close1(handle, h, dlerrstr, err_str_len,
		                             &dlerr);
		    if(nErr) {
			    FARF(ALWAYS,
			         "Warning 0x%x: %s: remotectl1 "
			    ...
			    // non-domain calls
			    hlist[domain].remotectlhandle = INVALID_HANDLE;
			    nErr = remotectl_close(h, dlerrstr, err_str_len,
			                           &dlerr);
		    } else if(nErr)
			    goto bail;
	    } else {
...
bail:
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for "
			     "handle 0x%x, domain %d (dlerr "
			     "%s) (errno "
			     "%s)\n",
			     nErr, __func__, h, domain, dlerrstr,
			     strerror(errno));
		}
	} else {
		FARF(ALWAYS,
		     "%s: closed module with handle 0x%x (skel "
		     "unload time %" PRIu64 " us)",
		     __func__, h, t_close);
	}
	if(dlerrstr) {
		free(dlerrstr);
...
int
remote_handle_close(remote_handle h)
{
...
int
remote_handle64_close(remote_handle64 handle)
{
	remote_handle64 remote = 0;
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = get_domain_from_handle(handle, &domain)));
	...
	if(hlist[domain].domainsCount <= 1 && !hlist[domain].nondomainsCount)
		start_deinit = true;
	/*
	 * If session termination is not initiated and the remote handle is
	 * valid, then close the remote handle on DSP.
	 */
	if(!start_deinit && remote) {
		VERIFY(AEE_SUCCESS
		       == (nErr = remote_handle_close_domain(
			       domain, (remote_handle)remote)));
	}
	FARF(ALWAYS,
	     "%s: closed module %s with handle 0x%" PRIx64
	     " remote handle 0x%" PRIx64 ", num of open handles: %u",
	     __func__, hi->name, handle, remote,
	     hlist[domain].domainsCount - 1);
		...
		if(nErr != AEE_SUCCESS) {
			...
			if(0 == check_rpc_error(nErr))
				FARF(ERROR,
				     ...
				     " (errno %s), num of open "
				     "handles: %u\n",
				     nErr, __func__, hi->name, handle, remote,
				     strerror(errno),
				     hlist[domain].domainsCount);
		}
	}
...
static int
manage_pm_qos(int domain, remote_handle64 h, uint32_t enable, uint32_t latency)
{
	return fastrpc_set_pm_qos(&hlist[domain].qos, enable, latency);
}

static int
manage_adaptive_qos(int domain, uint32_t enable)
{
	int nErr = AEE_SUCCESS;
	remote_handle64 handle = INVALID_HANDLE;
	...
	if(hlist[domain].dev != -1) {
		/* If session is already open on DSP, then make rpc
		 * call directly to user PD
		 */
		if((handle = get_remotectl1_handle(domain))
		   != INVALID_HANDLE) {
			nErr = remotectl1_set_param(handle, RPC_ADAPTIVE_QOS,
			                            &enable, 1);
			if(nErr) {
				FARF(ALWAYS,
				     "Warning 0x%x: "
				...
				// non-domain calls
				hlist[domain].remotectlhandle = INVALID_HANDLE;
				nErr = remotectl_set_param(RPC_ADAPTIVE_QOS,
				                           &enable, 1);
			}
		} else {
			nErr = remotectl_set_param(RPC_ADAPTIVE_QOS, &enable,
			                           1);
		}
		if(nErr) {
			FARF(ERROR,
			     "Error: %s: remotectl_set_param "
			     "failed to reset adaptive QoS "
			     "on DSP "
			     "to %d on domain %d",
			     __func__, enable, domain);
			goto bail;
		} else {
			hlist[domain].qos.adaptive_qos
			    = ((enable == RPC_ADAPTIVE_QOS) ? 1 : 0);
		}
	} else {
		/* If session is not created already, then just set
		 * process attribute */
		hlist[domain].qos.adaptive_qos
		    = ((enable == RPC_ADAPTIVE_QOS) ? 1 : 0);
	}
	...
	if(enable)
		FARF(ALWAYS,
		     "%s: Successfully enabled adaptive QoS on domain %d",
		     __func__, domain);
	else
		FARF(ALWAYS, "%s: Disabled adaptive QoS on domain %d",
		     __func__, domain);
bail:
	return nErr;
}
...
static int
manage_poll_qos(int domain, remote_handle64 h, uint32_t enable,
                uint32_t latency)
{
	...
	if(h == INVALID_HANDLE) {
		if(!hlist || (hlist && hlist[domain].dev == -1)) {
			VERIFY(AEE_SUCCESS
			       == (nErr = domain_init(domain, &dev)));
			VERIFYM(-1 != dev, AEE_ERPC, "open dev failed\n");
		}
	}
	/* If the multi-domain handle is valid, then verify that session is
	 * created already */
	...
	if(h == INVALID_HANDLE) {
		VERIFY(AEE_SUCCESS
		       == (nErr
		           = adsp_current_process_poll_mode(enable, latency)));
	} else {
		remote_handle64 handle
		    = get_adsp_current_process1_handle(domain);
		VERIFY(AEE_SUCCESS
		       == (nErr = adsp_current_process1_poll_mode(
			       handle, enable, latency)));
	}
	FARF(ALWAYS,
...
bail:
	if(nErr) {
		FARF(ERROR,
		     "Error 0x%x (errno %d): %s failed for domain "
		     "%d, handle 0x%" PRIx64 ", enable %u, timeout %u (%s)\n",
		     nErr, errno, __func__, domain, h, enable, latency,
		     strerror(errno));
	}
	return nErr;
}
...
void
fastrpc_qos_activity(int domain)
{
...
static inline int
enable_process_state_notif_on_dsp(int domain)
{
	int nErr = AEE_SUCCESS;
	...
	if((notif_handle = get_adsp_current_process1_handle(domain))
	   != INVALID_HANDLE) {
		VERIFY(AEE_SUCCESS
		       == (nErr = adsp_current_process1_enable_notifications(
			       notif_handle)));
	} else {
		VERIFY(
		    AEE_SUCCESS
		    == (nErr = adsp_current_process_enable_notifications()));
	}
bail:
	return nErr;
}
...
*/
static int
fastrpc_set_qos_latency(int domain, remote_handle64 h, uint32_t latency)
{
	int nErr = 0;
...
static int
update_kernel_wakelock_status(int domain, int dev, uint32_t wl_enable)
{
	int nErr = AEE_SUCCESS;
	...
	if(nErr) {
		...
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d, dev %d, "
		     "wakelock control %d "
		     "errno %s",
		     nErr, __func__, domain, dev, wl_enable, strerror(errno));
	} else
		FARF(ALWAYS,
		     "%s: updated kernel wakelock control status to %d for "
		     "domain %d",
...
static int
wakelock_control(int domain, remote_handle64 h, uint32_t wl_enable)
{
	int nErr = AEE_SUCCESS;
...
bail:
	if(nErr)
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d, handle 0x%x, "
		     "enable %d",
		     nErr, __func__, domain, h, wl_enable);
	else {
		if(wl_enable)
			FARF(ALWAYS,
			     "%s: enabled wakelock control for "
			     "domain %d",
			     __func__, domain);
		else
			FARF(ALWAYS,
			     "%s: disable wakelock control for "
			     "domain %d",
			     __func__, domain);
	}
	return nErr;
...
static int
fastrpc_dsp_process_clean(int domain)
{
...
int
remote_handle_control_domain(int domain, remote_handle64 h, uint32_t req,
                             void *data, uint32_t len)
{
	...
	case DSPRPC_CONTROL_LATENCY: {
		...
		VERIFYC(len == sizeof(struct remote_rpc_control_latency),
		        AEE_EBADPARM);

		switch(lp->enable) {
		/* Only one of PM QoS or adaptive QoS
		 * can be enabled */
		case RPC_DISABLE_QOS: {
			VERIFY(AEE_SUCCESS
			       == (nErr = manage_adaptive_qos(
				       domain, RPC_DISABLE_QOS)));
			VERIFY(AEE_SUCCESS
			       == (nErr
			           = manage_pm_qos(domain, h, RPC_DISABLE_QOS,
			                           lp->latency)));
			VERIFY(AEE_SUCCESS
			       == (nErr = manage_poll_qos(domain, h,
			                                  RPC_DISABLE_QOS,
			                                  lp->latency)));
			/* Error ignored,
			 * currently meeting
			 ...
			 * to error out in
			 * later targets */
			fastrpc_set_qos_latency(domain, h,
			                        FASTRPC_QOS_MAX_LATENCY_USEC);
			break;
		}
		case RPC_PM_QOS: {
			VERIFY(AEE_SUCCESS
			       == (nErr = manage_adaptive_qos(
				       domain, RPC_DISABLE_QOS)));
			VERIFY(AEE_SUCCESS
			       == (nErr = manage_pm_qos(domain, h, RPC_PM_QOS,
			                                lp->latency)));
			/* Error ignored,
			 * currently meeting
			 * qos requirement is
		...
		case RPC_POLL_QOS: {
			VERIFY(AEE_SUCCESS
			       == (nErr = manage_poll_qos(
				       domain, h, RPC_POLL_QOS, lp->latency)));

			/*
			 * Poll QoS option also
			 * enables PM QoS to
			 * enable early
			 * response from DSP
			 * and stop the CPU
			...
			VERIFY(AEE_SUCCESS
			       == (nErr
			           = manage_pm_qos(domain, h, RPC_PM_QOS,
		...
		FARF(ALWAYS,
		     "%s: requested QOS %d, latency %u for "
		     "domain %d handle 0x%" PRIx64 "\n",
	...
	case DSPRPC_GET_DSP_INFO: {
		...
		struct remote_dsp_capability *cap
		    = (struct remote_dsp_capability *)data;

		(void)dev;
		VERIFYC(cap, AEE_EBADPARM);
		VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(cap->domain),
		        AEE_EBADPARM);
	...
	case DSPRPC_CONTROL_WAKELOCK: {
		...
		VERIFYC(len == sizeof(struct remote_rpc_control_wakelock),
		        AEE_EBADPARM);
		VERIFY(AEE_SUCCESS
		       == (nErr = wakelock_control(domain, h, wp->enable)));
		break;
	}
	case DSPRPC_GET_DOMAIN: {
		remote_rpc_get_domain_t *pGet
		    = (remote_rpc_get_domain_t *)data;
	...
	default:
		...
		FARF(RUNTIME_RPC_LOW,
		     "Error: %s: remote handle control called with "
...
bail:
	if(nErr != AEE_SUCCESS)
		FARF(ERROR,
		     "Error 0x%x: %s failed for request ID %d on domain %d "
		     "(errno %s)",
...
int
remote_handle_control(uint32_t req, void *data, uint32_t len)
{
	int domain = -1, nErr = AEE_SUCCESS, ref = 0;
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_control_domain(domain, INVALID_HANDLE,
	                                               req, data, len)));
bail:
	FASTRPC_PUT_REF(domain);
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for request ID %d (errno "
		     "%s)",
		     nErr, __func__, req, strerror(errno));
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for "
			     "request ID %d (errno %s)",
			     nErr, __func__, req, strerror(errno));
		}
	}
	return nErr;
...
int
remote_handle64_control(remote_handle64 handle, uint32_t req, void *data,
                        uint32_t len)
{
	int nErr = AEE_SUCCESS, domain = -1, ref = 0;
	...
	FARF(RUNTIME_HIGH,
	     "Entering %s, handle %llu, req %d, data %p, size %d\n", __func__,
	     handle, req, data, len);
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_control_domain(domain, handle, req,
	                                               data, len)));
bail:
	FASTRPC_PUT_REF(domain);
	if(nErr != AEE_SUCCESS) {
...
static int
close_domain_session(int domain)
{
	QNode *pn = NULL, *pnn = NULL;
	char dlerrstr[255];
	int dlerr = 0, nErr = AEE_SUCCESS;
	...
	FARF(ALWAYS,
	     "%s: user requested to close fastrpc session on domain %d, dev "
	     "%d\n",
...
int
get_unsigned_pd_attribute(uint32_t domain, int *unsigned_module)
{
	int nErr = AEE_SUCCESS;
...
static int
set_unsigned_pd_attribute(int domain, int enable)
{
	int nErr = AEE_SUCCESS;
	...
	if(hlist[domain].dev != -1) {
		if(hlist[domain].unsigned_module == enable) {
			FARF(HIGH,
			     "%s: %s session already open on "
			     "domain %d , enable %d ",
			     __func__,
			     hlist[domain].unsigned_module ? "Unsigned"
			                                   : "Signed",
			     domain, enable);
		} else {
			nErr = AEE_EALREADYLOADED;
			FARF(ERROR,
			     "Error 0x%x: %s: %s session "
			     "already open on domain %d , "
			     "enable %d ",
			     nErr, __func__,
			     hlist[domain].unsigned_module ? "Unsigned"
			                                   : "Signed",
...
static int
store_domain_thread_params(int domain, int thread_priority, int stack_size)
{
	int nErr = AEE_SUCCESS;
	...
	if(thread_priority != -1) {
		if((thread_priority < MIN_THREAD_PRIORITY)
		   || (thread_priority > MAX_THREAD_PRIORITY)) {
			nErr = AEE_EBADPARM;
			FARF(ERROR,
			     "%s: Thread priority %d is "
			     "invalid! Should be between %d "
			     "and %d",
			     __func__, thread_priority, MIN_THREAD_PRIORITY,
			     MAX_THREAD_PRIORITY);
			goto bail;
		} else {
			hlist[domain].th_params.thread_priority
	...
	if(hlist[domain].dev != -1) {
		VERIFY(AEE_SUCCESS
		       == (nErr = fastrpc_set_remote_uthread_params(domain)));
		FARF(ALWAYS,
		     "Dynamically set remote user thread priority to "
		     "%d and stack size to "
		     "%d for domain %d",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d for thread "
		     "priority %d, stack "
		     "size %d",
...
static int
set_pd_dump_attribute(int domain, int enable)
{
	int nErr = AEE_SUCCESS;
	...
	if(hlist[domain].dev != -1) {
		nErr = AEE_ERPC;
		FARF(ERROR,
		     "%s: Session already open on domain %d ! "
		     "Request unsigned offload "
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed to enable %d for domain %d",
		     nErr, __func__, enable, domain);
	}
	return nErr;
}
...
static int
store_domain_pd_initmem_size(int domain, uint32_t pd_initmem_size)
{
	int nErr = AEE_SUCCESS;
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d for pd "
		     "init mem size :0x%x",
		     nErr, __func__, domain, pd_initmem_size);
	}
	return nErr;
}

/* Set remote session parameters like thread stack size, running on unsigned
 * PD, killing remote process PD etc */
int
remote_session_control(uint32_t req, void *data, uint32_t datalen)
{
	...
	case FASTRPC_THREAD_PARAMS: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_thread_params),
		        AEE_EBADPARM);
		if(params->domain != -1) {
			if(!IS_VALID_EFFECTIVE_DOMAIN_ID(params->domain)) {
				...
				FARF(RUNTIME_RPC_LOW,
				     ...
				     "ID %d "
				     "passed",
				     __func__, params->domain);
				goto bail;
			}
			VERIFY(AEE_SUCCESS
			       == (nErr = store_domain_thread_params(
				       params->domain, params->prio,
				       params->stack_size)));
		} else {
			/* If domain is -1, then set
			 * parameters for all domains
			 */
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				VERIFY(AEE_SUCCESS
		...
		FARF(ALWAYS,
		     "%s DSP info request for domain %d, "
		     "thread priority %d, stack size %d",
		     __func__, params->domain, params->prio,
		     params->stack_size);
		break;
	}
	case DSPRPC_CONTROL_UNSIGNED_MODULE: {
		...
		VERIFYC(
		    datalen
			== sizeof(struct remote_rpc_control_unsigned_module),
		    AEE_EBADPARM);
		...
		if(um->domain != -1) {
			VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(um->domain),
			        AEE_EBADPARM);
			VERIFY(AEE_SUCCESS
			       == (nErr = set_unsigned_pd_attribute(
				       um->domain, um->enable)));
		} else {
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
		...
		FARF(ALWAYS,
		     "%s Unsigned PD enable %d request for "
		     "domain %d",
		     __func__, um->enable, um->domain);
		break;
	}
	case FASTRPC_RELATIVE_THREAD_PRIORITY: {
		int thread_priority = DEFAULT_UTHREAD_PRIORITY,
		    rel_thread_prio = 0;
		...
		VERIFYC(
		    datalen
			== sizeof(struct remote_rpc_relative_thread_priority),
		    AEE_EBADPARM);
		VERIFYC(params, AEE_EBADPARM);
		rel_thread_prio = params->relative_thread_priority;

		// handle thread priority overflow and
		// out-of-range conditions
		if(rel_thread_prio < 0) {
			thread_priority
			    = ((~rel_thread_prio)
			       >= DEFAULT_UTHREAD_PRIORITY - 1)
			          ? MIN_THREAD_PRIORITY
			          : (thread_priority + rel_thread_prio);
		} else {
			thread_priority
			    = (rel_thread_prio > (MAX_THREAD_PRIORITY
			                          - DEFAULT_UTHREAD_PRIORITY))
			          ? MAX_THREAD_PRIORITY
			          : (thread_priority + rel_thread_prio);
		}
		if(params->domain != -1) {
			...
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				VERIFY(AEE_SUCCESS
				       == (nErr = store_domain_thread_params(
					       ii, thread_priority, -1)));
			}
		}
		FARF(ALWAYS,
		     "%s DSP thread priority request for "
		     "domain %d, priority %d",
	...
	case FASTRPC_REMOTE_PROCESS_KILL: {
		...
		VERIFYC(datalen
		            == sizeof(struct remote_rpc_process_clean_params),
		        AEE_EBADPARM);
		VERIFYC(dp, AEE_EBADPARM);
		domain = dp->domain;
		VERIFY(AEE_SUCCESS
		       == (nErr = fastrpc_dsp_process_clean(domain)));
		FARF(ALWAYS,
		     "%s Remote process kill request for "
		     "domain %d",
		     __func__, domain);
		break;
	}
	case FASTRPC_SESSION_CLOSE: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_session_close),
		        AEE_EBADPARM);
		if(sclose->domain != -1) {
			...
			VERIFY(
			    AEE_SUCCESS
			    == (nErr = close_domain_session(sclose->domain)));
		} else {
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				VERIFY(AEE_SUCCESS
				       == (nErr = close_domain_session(ii)));
			}
		}
		FARF(ALWAYS,
		     "%s Fastrpc session close request for "
		     "domain %d\n",
		     __func__, sclose->domain);
		break;
	}
	case FASTRPC_CONTROL_PD_DUMP: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_control_pd_dump),
		        AEE_EBADPARM);
		VERIFYC(pddump != NULL, AEE_EBADPARM);
		if(pddump->domain != -1) {
			VERIFY(AEE_SUCCESS
			       == (nErr = set_pd_dump_attribute(
				       pddump->domain, pddump->enable)));
		} else {
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				VERIFY(AEE_SUCCESS
		...
		FARF(ALWAYS,
		     "%s PD dump request to enable(%d) for "
		     "domain %d",
		     __func__, pddump->enable, pddump->domain);
		break;
	}
	case FASTRPC_REMOTE_PROCESS_EXCEPTION: {
		// Trigger non fatal exception in the User PD
		// of DSP.
		remote_rpc_process_exception *dp
		    = (remote_rpc_process_exception *)data;
		remote_handle64 handle = 0;
		int ret = 0;

		VERIFYC(datalen == sizeof(remote_rpc_process_exception),
		        AEE_EBADPARM);
		VERIFYC(dp, AEE_EBADPARM);
		domain = dp->domain;
		VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(domain), AEE_EBADPARM);
		/*
		 * If any request causes exception on DSP, DSP
		 * returns AEE_EBADSTATE for the request
		 * thread.
		 */
		...
		FARF(ALWAYS,
		     "%s Remote process exception request "
		     "for domain %d, handle 0x%" PRIx64 "\n",
		     __func__, domain, handle);
		break;
	}
	case FASTRPC_REMOTE_PROCESS_TYPE: {
		struct remote_process_type *typ
		    = (struct remote_process_type *)data;
		int ret_val = -1;
		VERIFYC(datalen == sizeof(struct remote_process_type),
		        AEE_EBADPARM);
		VERIFYC(typ, AEE_EBADPARM);
		domain = typ->domain;
		VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(domain), AEE_EBADPARM);
		ret_val = hlist[domain].unsigned_module;
		if(ret_val != PROCESS_TYPE_UNSIGNED
	...
	case FASTRPC_REGISTER_STATUS_NOTIFICATIONS: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_notif_register),
		        AEE_EBADPARM);
		VERIFYC(notif != NULL, AEE_EBADPARM);
		domain = notif->domain;
		VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(domain), AEE_EBADPARM);
		if(domain != -1) {
			VERIFYC(is_status_notif_version2_supported(domain),
			        AEE_EUNSUPPORTED);
			VERIFY(
			    AEE_SUCCESS
			    == (nErr = fastrpc_notif_register(domain, notif)));
		} else {
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				VERIFYC(is_status_notif_version2_supported(ii),
				        AEE_EUNSUPPORTED);
				VERIFY(AEE_SUCCESS
				       == (nErr = fastrpc_notif_register(
					       ii, notif)));
			}
		}
		FARF(ALWAYS,
		     "%s Register PD status notification "
		     "request for domain %d\n",
		     __func__, domain);
		break;
	}
	case FASTRPC_PD_INITMEM_SIZE: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_pd_initmem_size),
		        AEE_EBADPARM);
		...
		if(params->domain != -1) {
			...
			VERIFY(
			    AEE_SUCCESS
			    == (nErr = store_domain_pd_initmem_size(
			...
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				if(hlist[ii].unsigned_module) {
					...
					VERIFY(
					    AEE_SUCCESS
					    == (nErr
					        = store_domain_pd_initmem_size(
						    ii,
						    params->pd_initmem_size)));
				}
			}
		}
		FARF(ALWAYS,
		     "%s DSP userpd memlen request for "
		     "domain %d, userpd memlen 0x%x",
	...
	case FASTRPC_RESERVE_NEW_SESSION: {
		...
		VERIFYC(datalen == sizeof(remote_rpc_reserve_new_session_t),
		        AEE_EBADPARM);
		VERIFYC(sess && sess->domain_name != NULL
		            && sess->domain_name_len > 0 && sess->session_name
		            && sess->session_name_len > 0,
		        AEE_EBADPARM);
		domain = get_domain_from_name(sess->domain_name,
		                              DOMAIN_NAME_STAND_ALONE);
		VERIFYC(IS_VALID_DOMAIN_ID(domain), AEE_EBADPARM);
		// Initialize effective domain ID to 2nd
		// session of domain, first session is default
		// usage and cannot be reserved
		ii = domain + NUM_DOMAINS;
		// Initialize session to 1, session 0 is
		// default session
		jj = 1;
		// Set effective_domain_id and session_id to
		// invalid IDs
		...
		do {
			...
			if(!hlist[ii].is_session_reserved) {
				...
				sess->session_id = jj;
				strlcpy(hlist[ii].sessionname,
				        sess->session_name,
				        STD_MIN(sess->session_name_len,
				                (MAX_DSPPD_NAMELEN - 1)));
				pthread_mutex_unlock(&hlist[ii].init);
				break;
			}
			pthread_mutex_unlock(&hlist[ii].init);
			// Increment to next session of
			// domain
			ii = ii + NUM_DOMAINS;
		...
		VERIFYC(
		    IS_VALID_EFFECTIVE_DOMAIN_ID(sess->effective_domain_id),
		    AEE_ENOSESSION);
		break;
	}
	case FASTRPC_GET_EFFECTIVE_DOMAIN_ID: {
		...
		VERIFYC(datalen == sizeof(remote_rpc_effective_domain_id_t),
		        AEE_EBADPARM);
		...
		domain = get_domain_from_name(effec_domain_id->domain_name,
		                              DOMAIN_NAME_STAND_ALONE);
		VERIFYC(IS_VALID_DOMAIN_ID(domain), AEE_EBADPARM);
		effec_domain_id->effective_domain_id = GET_EFFECTIVE_DOMAIN_ID(
		    domain, effec_domain_id->session_id);
		VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(
	...
	case FASTRPC_GET_URI: {
		...
		VERIFYC(rpc_uri && rpc_uri->domain_name
		            && rpc_uri->domain_name_len > 0
		            && rpc_uri->session_id < NUM_SESSIONS,
		        AEE_EBADPARM);
		domain = get_domain_from_name(rpc_uri->domain_name,
		                              DOMAIN_NAME_STAND_ALONE);
		VERIFYC(IS_VALID_DOMAIN_ID(domain), AEE_EBADPARM);
		VERIFYC(rpc_uri->module_uri != NULL
		            && rpc_uri->module_uri_len > 0,
		        AEE_EBADPARM);
		VERIFYC(rpc_uri->uri != NULL
		            && rpc_uri->uri_len > rpc_uri->module_uri_len,
		        AEE_EBADPARM);
		...
		if(rpc_uri->uri_len <= ret_val) {
			nErr = AEE_EBADSIZE;
			FARF(ERROR,
			     "ERROR 0x%x: %s Session "
			     "URI length %u is not "
			     "enough, need %u "
			     "characters",
			     nErr, __func__, rpc_uri->uri_len, ret_val);
		}
		ret_val = snprintf(rpc_uri->uri, rpc_uri->uri_len,
		                   "%s%s%s%s%d", rpc_uri->module_uri,
		                   FASTRPC_DOMAIN_URI, SUBSYSTEM_NAME[domain],
		                   FASTRPC_SESSION_URI, rpc_uri->session_id);
		if(ret_val < 0) {
			nErr = AEE_EBADSIZE;
			FARF(ERROR,
			     "ERROR 0x%x: %s Invalid "
			     "Session URI length %u",
			     nErr, __func__, rpc_uri->uri_len);
		}
		break;
	}
	case FASTRPC_CONTEXT_CREATE:
		VERIFYC(datalen == sizeof(fastrpc_context_create) && data,
		        AEE_EBADPARM);
		VERIFY(AEE_SUCCESS == (nErr = fastrpc_create_context(data)));
		break;
	case FASTRPC_CONTEXT_DESTROY: {
		fastrpc_context_destroy *dest
		    = (fastrpc_context_destroy *)data;

		VERIFYC(datalen == sizeof(fastrpc_context_destroy) && dest
		            && !dest->flags,
		        AEE_EBADPARM);
		VERIFY(AEE_SUCCESS
		       == (nErr = fastrpc_destroy_context(dest->ctx)));
		break;
	}
	default:
		nErr = AEE_EUNSUPPORTED;
		FARF(ERROR, "ERROR 0x%x: %s Unsupported request ID %d", nErr,
		     __func__, req);
		break;
	}
bail:
 ...
 */
int
get_domain_from_name(const char *uri, uint32_t type)
{
...
int
fastrpc_get_pd_type(int domain)
{
	if(hlist && (hlist[domain].dev != -1)) {
...
domain_deinit(int domain)
{
	...
	if(olddev != -1) {
		...
		FASTRPC_ATRACE_BEGIN_L(
		    "%s called for handle 0x%x, domain %d, dev %d", __func__,
		    handle, domain, olddev);
		...
		if(domain == DEFAULT_DOMAIN_ID) {
			fastrpc_clear_handle_list(NON_DOMAIN_HANDLE_LIST_ID,
			                          domain);
		}
		fastrpc_perf_deinit();
		...
		FARF(ALWAYS,
		     "%s: closed device %d on domain %d (kill time "
		     "%" PRIu64 " us)",
...
int
open_device_node(int domain_id)
{
	int dev = -1, nErr = 0;
	int domain = GET_DOMAIN_FROM_EFFEC_DOMAIN_ID(domain_id);
	...
	case MDSP_DOMAIN_ID:
		...
		if((dev < 0) && (errno == ENOENT)) {
			FARF(RUNTIME_RPC_HIGH,
			     "Device node %s open failed for "
			     "domain %d (errno %s),"
			     "falling back to node %s \n",
			     get_secure_domain_name(domain), domain,
			     strerror(errno), get_domain_name(domain));
			...
			if((dev < 0) && (errno == ENOENT)) {
				FARF(RUNTIME_RPC_HIGH,
				     ...
				     "falling back "
				     "to node %s \n",
				     get_domain_name(domain), domain,
				     strerror(errno), DEFAULT_DEVICE);
				dev = open(DEFAULT_DEVICE, O_NONBLOCK);
			}
		} else if((dev < 0) && (errno == EACCES)) {
			// Open the default device node if
			// unable to open the secure device
			// node due to permissions
			FARF(RUNTIME_RPC_HIGH,
			     "Device node %s open failed for "
			     "domain %d (errno %s),"
			     "falling back to node %s \n",
	...
	if(dev < 0)
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain ID %d, sess ID %d "
		     "secure dev : %s, "
		     "dev : %s. (errno %d, %s) (Either the remote processor "
		     "is down, or "
		     "application does not have permission to access the "
		     "remote "
		     "processor\n",
		     nErr, __func__, domain_id, sess_id,
		     get_secure_domain_name(domain), get_domain_name(domain),
		     errno, strerror(errno));
	return dev;
}

static int
close_device_node(int domain_id, int dev)
{
	int nErr = 0;
   ...
   dsp_client_instance[sess_id])
		{
			nErr = close_hal_session(dsp_client_instance[sess_id],
			                         domain_id, dev);
			FARF(ALWAYS,
			     "%s: close device %d thru HAL on session %d\n",
			     __func__, dev, sess_id);
		}
	else {
#endif
		nErr = close(dev);
		FARF(ALWAYS, "%s: closed dev %d on domain %d", __func__, dev,
		     domain_id);
#ifndef NO_HAL
	}
#endif
...
get_process_attrs(int domain)
{
	...
	attrs
	    |= hlist[domain].qos.adaptive_qos ? FASTRPC_MODE_ADAPTIVE_QOS : 0;
	attrs |= hlist[domain].unsigned_module ? FASTRPC_MODE_UNSIGNED_MODULE
	                                       : 0;
	...
	attrs |= fastrpc_config_is_sysmon_reserved_bit_enabled()
	             ...
	             : 0;
	attrs |= fastrpc_config_is_logpacket_enabled()
	             ? FASTRPC_MODE_LOG_PACKET
	             : 0;
...
static void
get_process_testsig(apps_std_FILE *fp, uint64_t *ptrlen)
{
	int nErr = 0;
	uint64_t len = 0;
	...
	if(fastrpc_get_property_string(FASTRPC_DEBUG_TESTSIG, testsig, NULL)) {
		...
		nErr = apps_std_fopen_with_env(ADSP_LIBRARY_PATH, ";", testsig,
		                               "r", fp);
...
static int
open_shell(int domain_id, apps_std_FILE *fh, int unsigned_shell)
{
	char *absName = NULL;
	...
	VERIFYC(NULL != (domain_str = (char *)malloc(sizeof(domain))),
	        AEE_ENOMEMORY);
	...
	VERIFYC(NULL
	            != (shell_absName
	                = (char *)malloc(sizeof(char) * shell_absNameLen)),
	        AEE_ENOMEMORY);
	...
	if(nErr) {
		...
		VERIFYC(NULL
		            != (absName = (char *)realloc(
				    absName, sizeof(char) * absNameLen)),
		        AEE_ENOMEMORY);
		strlcpy(absName, DSP_MOUNT_LOCATION, absNameLen);
	...
	if(nErr) {
		absNameLen
		    = strlen(VENDOR_DSP_LOCATION) + shell_absNameLen + 1;
		VERIFYC(NULL
		            != (absName = (char *)realloc(
				    absName, sizeof(char) * absNameLen)),
		        AEE_ENOMEMORY);
		...
		nErr = apps_std_fopen(absName, "r", fh);
		if(nErr) {
			absNameLen = strlen(VENDOR_DOM_LOCATION)
			             + shell_absNameLen + 1;
			VERIFYC(
			    NULL
				!= (absName = (char *)realloc(
	...
	if(!nErr)
		FARF(ALWAYS, "Successfully opened %s, domain %d", absName,
		     domain);
bail:
	if(domain_str) {
		free(domain_str);
	...
	if(nErr != AEE_SUCCESS) {
		if(domain == SDSP_DOMAIN_ID && fh != NULL) {
			...
			*fh = -1;
			...
			FARF(ERROR,
			     "Error 0x%x: %s failed for domain "
			     "%d search paths used are %s, %s, "
			     "%s (errno %s)\n",
			     nErr, __func__, domain, DSP_MOUNT_LOCATION,
			     VENDOR_DSP_LOCATION, VENDOR_DOM_LOCATION,
			     strerror(errno));
		}
	}
	return nErr;
 ...
 */
static int
fastrpc_enable_kernel_optimizations(int domain)
{
	...
	if((nErr == -1 || nErr == (DSP_AEE_EOFFSET + AEE_ERPC))
	   && (errno == ENOTTY || errno == EINVAL || errno == EBADRQC)) {
		/*
		 * Kernel optimizations not supported. Ignore IOCTL
		 * failure
		 * TODO: kernel cleanup to return ENOTTY for all
		 * unsupported IOCTLs
		 */
		nErr = 0;
	}
...
void
print_process_attrs(int domain)
{
	bool dbgMode = false, crc = false, signedMd = false,
	     unsignedMd = false, qos = false, configPDdump = false;
	...
	FARF(
	    ...
	    "timeout:%d, Dbg Mode:%s, CRC:%s, Unsigned:%s, Signed:%s, Adapt "
	    "QOS:%s, "
	    "PD dump: (Config:%s, Dbg:%s), Perf: (Kernel:%s, DSP:%s), "
	    "Iregion:%s, "
	    ...
	    (signedMd ? "Y" : "N"), (qos ? "Y" : "N"),
	    (configPDdump ? "Y" : "N"), (debugPDdump ? "Y" : "N"),
	    (KernelPerf ? "Y" : "N"), (DSPperf ? "Y" : "N"),
	    (iregion ? "Y" : "N"), (qtf ? "Y" : "N"), (uaf ? "Y" : "N"),
	    pd_initmem_size, (logpkt ? "Y" : "N"));
	return;
}

static int
remote_init(int domain)
{
	...
	VERIFYC(pd_type > DEFAULT_UNUSED && pd_type < MAX_PD_TYPE,
	        AEE_EBADITEM);
	if(hlist[domain].dev == -1) {
		dev = open_device_node(domain);
		VERIFYC(dev >= 0, AEE_ECONNREFUSED);
		// Set session relation info using
		...
		if(nErr == AEE_SUCCESS) {
			...
			// Skip setting session related info in
			// multiple ioctl calls, if
			// FASTRPC_CONTROL_SESS_INFO is
			// supported
			goto set_sess_info_supported;
		} else {
			// Fallback to previous approach, if
			// FASTRPC_CONTROL_SESS_INFO is not
			// supported
			FARF(RUNTIME_RPC_HIGH,
			     "%s: FASTRPC_CONTROL_SESS_INFO "
			     "not supported with error %d",
			     __func__, nErr);
		}
	...
	set_sess_info_supported:
		...
		if(nErr == AEE_SUCCESS) {
			...
			FARF(ERROR,
			     "Error %d: %s: app does not "
			     "have access to fastrpc device "
			     "of domain "
			     "%d (%s)",
			     nErr, __func__, domain, strerror(errno));
			goto bail;
			...
			FARF(ERROR,
			     "Error %d: %s: fastRPC device "
			     "driver is disabled (%s)",
			     nErr, __func__, strerror(errno));
			goto bail;
		} else if(nErr) {
			FARF(ERROR,
			     "Error 0x%x: %s: failed to setup "
			     "fastrpc session in kernel",
			     nErr, __func__);
			goto bail;
		}
		...
		if(IS_EXTENDED_DOMAIN_ID(domain))
			VERIFY(AEE_SUCCESS
			       == (nErr
			           = ioctl_setmode(dev, FASTRPC_SESSION_ID1)));

		FARF(RUNTIME_RPC_HIGH,
		     "%s: device %d opened with info 0x%x (attach %d)",
		...
		if(pd_type == ROOT_PD || pd_type == GUEST_OS_SHARED
		   || pd_type == SECURE_STATICPD) {
			FARF(RUNTIME_RPC_HIGH,
			     "%s: attaching to guest OS/Secure "
			     "PD (attach %d) for domain %d",
			     __func__, pd_type, domain);
			if(pd_type == SECURE_STATICPD) {
				file = calloc(
				    1, (int)(strlen(hlist[domain].dsppdname)
				             + 1));
			...
			ioErr
			    = ioctl_init(dev, flags, 0, (unsigned char *)file,
			                 filelen, -1, 0, 0, 0, 0);
			if(file) {
				free(file);
				file = NULL;
			}
			VERIFYC((!ioErr || errno == ENOTTY || errno == ENXIO
			         || errno == EINVAL),
			        AEE_ERPC);
		} else if(pd_type == AUDIO_STATICPD
		          || pd_type == OIS_STATICPD) {
			FARF(RUNTIME_RPC_HIGH,
			     "%s: creating static user PD "
			     "for domain %d",
			     __func__, domain);
			file = rpcmem_alloc_internal(
			    0, RPCMEM_HEAP_DEFAULT,
			...
			// 3MB of remote heap for dynamic
			// loading is available only for Audio
			// PD.
			...
			ioErr
			    = ioctl_init(dev, flags, 0, (unsigned char *)file,
			                 filelen, -1, 0, memlen, 0, 0);
			if(ioErr) {
				nErr = convert_kernel_to_user_error(ioErr,
				                                    errno);
				goto bail;
			}
		} else if(pd_type == SENSORS_STATICPD) {
			FARF(RUNTIME_RPC_HIGH,
			     "%s: attaching to sensors PD "
			     "for domain %d",
			     __func__, domain);
			flags = FASTRPC_INIT_ATTACH_SENSORS;
			ioErr = ioctl_init(dev, flags, 0, (unsigned char *)0,
			                   0, -1, 0, 0, 0, 0);
			VERIFYC((!ioErr || errno == ENOTTY || errno == ENXIO
			         || errno == EINVAL),
			        AEE_ERPC);
			...
			if(fh != -1) {
				VERIFY(AEE_SUCCESS
				       == (nErr = apps_std_flen(fh, &len)));
				filelen = len + siglen;
				VERIFYC(filelen && filelen < INT_MAX,
				        AEE_EFILE);
				file = rpcmem_alloc_internal(
				    0, RPCMEM_HEAP_DEFAULT, (size_t)filelen);
				VERIFYC(file, AEE_ENORPCMEMORY);
				VERIFY(AEE_SUCCESS
				       == (nErr = apps_std_fread(
					       fh, (unsigned char *)file, len,
					       &readlen, &eof)));
			...
			if(!(FASTRPC_MODE_UNSIGNED_MODULE
			     & hlist[domain].procattrs)) {
				memlen = hlist[domain].pd_initmem_size;
			} else {
				if(hlist[domain].pd_initmem_size
				   != DEFAULT_PD_INITMEM_SIZE)
					FARF(ERROR, "Setting "
					            "user PD "
					            "initial "
					            "memory "
					...
					VERIFY(
					    ...
					    == (nErr = apps_std_fread(
						    ...
						    (unsigned char *)(file
					                              + len),
						    siglen, &readlen, &eof)));
					VERIFYC(siglen == (uint64_t)readlen,
					        AEE_EFILE);
					filelen = len + siglen;
				}
			}
			ioErr = ioctl_init(dev, flags, hlist[domain].procattrs,
			                   (unsigned char *)file, filelen,
...
bail:
	// errno is being set to 0 in apps_std_fclose and we need original
	// errno to return proper error to user call
	errno_save = errno;
	if(file) {
	...
	FARF(RUNTIME_RPC_HIGH, "Done with %s, err: 0x%x, dev: %d", __func__,
	     nErr, hlist[domain].dev);
	return nErr;
}
...
remote_handle64
get_adsp_current_process1_handle(int domain)
{
	remote_handle64 local;
	int nErr = AEE_SUCCESS;
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: adsp current process handle "
		     "failed. domain %d (errno "
		     "%s)\n",
...
remote_handle64
get_adspmsgd_adsp1_handle(int domain)
{
	remote_handle64 local;
	int nErr = AEE_SUCCESS;
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: get adsp msgd handle failed. "
		     "domain %d (errno %s)\n",
		     nErr, domain, strerror(errno));
	}
	return INVALID_HANDLE;
...
remote_handle64
get_adsp_listener1_handle(int domain)
{
	remote_handle64 local;
	int nErr = AEE_SUCCESS;
...
remote_handle64
get_remotectl1_handle(int domain)
{
	remote_handle64 local;
	int nErr = AEE_SUCCESS;

	// If remotectlhandle is 0 allocate handle, else return handle even
	// though INVALID_HANDLE handle
	if(hlist[domain].remotectlhandle) {
		return hlist[domain].remotectlhandle;
	}
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_update_module_list(
		       DOMAIN_LIST_PREPEND, domain, _const_remotectl1_handle,
		       &local, NULL)));
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: remotectl1 handle failed. domain "
		     "%d (errno %s)\n",
		     nErr, domain, strerror(errno));
	}
	return INVALID_HANDLE;
...
remote_handle64
get_adsp_perf1_handle(int domain)
{
	remote_handle64 local;
	int nErr = AEE_SUCCESS;
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_update_module_list(
		       DOMAIN_LIST_PREPEND, domain, _const_adsp_perf1_handle,
		       &local, NULL)));
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: adsp_perf1 handle failed. domain "
		     "%d (errno %s)\n",
		     nErr, domain, strerror(errno));
	}
	return INVALID_HANDLE;
...
static int
domain_init(int domain, int *dev)
{
	...
	if(fastrpc_wake_lock_enable[domain]) {
		VERIFY(AEE_SUCCESS
		       == (nErr = update_kernel_wakelock_status(
			       ...
			       fastrpc_wake_lock_enable[domain])));
	}
	...
	if(dom == CDSP_DOMAIN_ID || dom == CDSP1_DOMAIN_ID) {
		...
		if(panic_handle != INVALID_HANDLE) {
			int ret = -1;
			/* If error codes are available in
			 * debug config, send panic error codes
			 * to dsp to crash. */
			...
			FARF(ALWAYS,
			     "%s : current process handle is "
			     "not valid\n",
			     __func__);
		}
	}
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_enable_kernel_optimizations(domain)));
	initFileWatcher(domain); // Ignore errors
	trace_marker_init(domain);

	// If client notifications are registered, initialize notification
	// thread and enable notifications on domains
	if(fastrpc_notif_flag) {
		int ret = 0;
		ret = enable_process_state_notif_on_dsp(domain);
		if(ret == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED)) {
			VERIFY_WPRINTF("Warning: %s: DSP does not "
	...
	if((dom != SDSP_DOMAIN_ID) && hlist[domain].dsppd == ROOT_PD) {
		...
		if(handle != INVALID_HANDLE) {
			adspmsgd_init(handle, 0x10); // enable PD
			                             // exception logging
		}
	}
#endif
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_latency_init(hlist[domain].dev,
	                                       &hlist[domain].qos)));
	...
	pthread_mutex_unlock(&hlist[domain].mut);
	VERIFY(AEE_SUCCESS
	       == (nErr = listener_android_domain_init(
...
bail:
	if(nErr != AEE_SUCCESS) {
		...
		if(hlist) {
			FARF(ERROR,
			     "Error 0x%x: %s (%d) failed for "
			     "domain %d (errno %s)\n",
			     nErr, __func__, hlist[domain].dev, domain,
			     strerror(errno));
		}
		*dev = -1;
		return nErr;
	...
	if(hlist) {
		...
		*dev = -1;
		FARF(ERROR,
		     "Error 0x%x: Unable to get dev as hlist is NULL "
		     "for domain %d\n",
		     nErr, __func__, domain);
		return nErr;
	}
...
fastrpc_apps_user_deinit(void)
{
	...
	if(hlist) {
		FOR_EACH_EFFECTIVE_DOMAIN_ID(i)
		{
			fastrpc_clear_handle_list(MULTI_DOMAIN_HANDLE_LIST_ID,
			                          i);
...
exit_thread(void *value)
{
	...
	FOR_EACH_EFFECTIVE_DOMAIN_ID(domain)
	{
		if(hlist[domain].dev != -1) {
			if((handle = get_adsp_current_process1_handle(domain))
			   != INVALID_HANDLE) {
				nErr = adsp_current_process1_thread_exit(
				    handle);
				if(nErr) {
					FARF(RUNTIME_RPC_HIGH,
					     "%s: "
...
fastrpc_apps_user_init(void)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = pthread_key_create(&tlsKey, exit_thread)));
...
bail:
	...
	*/
	if(nErr) {
		FARF(ERROR,
		     "Error 0x%x: %s failed. default domain:%x and "
		     "&fastrpc_trace:%p \n",
		     nErr, __func__, DEFAULT_DOMAIN_ID, &fastrpc_trace);
		fastrpc_apps_user_deinit();
	} else {
		FARF(ALWAYS,
		     "%s done. default domain:%x and "
		     "&fastrpc_trace:%p\n",
		     __func__, DEFAULT_DOMAIN_ID, &fastrpc_trace);
	}
	return nErr;
}
...
check_multilib_util(void)
{
	...
	if(ret != 0 && errno == ENOMEM) {
		nErr = ERRNO;
		FARF(ERROR,
		     "Error 0x%x: setenv failed for %s (domain %u), "
		     "errno is %s\n",
		     nErr, env_name, DEFAULT_DOMAIN_ID, strerror(ERRNO));
		/*
		 * If failed to set env variable then free the memory
		 * allocated to env variables and exit the application.
		 */
		deinit_fastrpc_dsp_lib_refcnt();
		exit(EXIT_FAILURE);
	} else {
	...
	/* Get the values of all env variables and increment the refcount
	 * accordingly.
	 */
	FOR_EACH_DOMAIN_ID(ii)
	{
		...
		/* Skip the check to get default domain env variable value as
		 * we already set its value. */
		...
		if(NULL != env_val) {
			/*
			 * Add env value to total reference count to
			 * check the total number of dsp library
			 * instances loaded.
			 */
			total_dsp_lib_refcnt += atoi(env_val);

			/* If the total reference count exceeds one
			 * then show warning, application will abort on
			 * first handle open. */
			if(total_dsp_lib_refcnt > MAX_LIB_INSTANCE_ALLOWED) {
				FARF(ERROR,
				     "Warning: %s %d instances "
				     "of libxdsprpc (already "
				     "loaded %s). Only "
				     "%d allowed\n",
				     fastrpc_library[DEFAULT_DOMAIN_ID],
				     total_dsp_lib_refcnt, fastrpc_library[ii],
				     MAX_LIB_INSTANCE_ALLOWED);
			}
		}
	}
}
...
static void
multidsplib_env_init(void)
{
	const char *local_fastrpc_lib_refcnt[NUM_DOMAINS]
	    = { "FASTRPC_ADSP_REFCNT", "FASTRPC_MDSP_REFCNT",
		"FASTRPC_SDSP_REFCNT", "FASTRPC_CDSP_REFCNT",
		"FASTRPC_CDSP1_REFCNT" };
	char buf[64] = { 0 };
	...
	/* Initialize all global array with env variable names along with
	 * process id.
	 */
	FOR_EACH_DOMAIN_ID(ii)
	{
		snprintf(buf, sizeof(buf), "%s_%d",
		         local_fastrpc_lib_refcnt[ii], pid);
		...
		if(env_name) {
			...
			FARF(ERROR,
			     "Error %d: %s: env variable allocation "
			     "for %zu bytes failed, domain "
			     "%d\n",
	...
	check_multilib_util();
	FARF(ALWAYS, "%s: %s loaded", __func__,
	     fastrpc_library[DEFAULT_DOMAIN_ID]);
}
