...
static const char strings[166]
    = "invoke_get_in_bufs\0routBufLenReq\0get_in_bufs2\0inBufLenReq\0next_"
      ...
      "offset\0handle\0next2\0init2\0init\0ctx\0sc\0";
...
static void
_adsp_listener_pls_dtor(void *data)
{
...
static int
_adsp_listener_pls_ctor(void *ctx, void *data)
{
	remote_handle *ph = (remote_handle *)data;
	*ph = _const_adsp_listener_handle;
...
_adsp_listener_handle(void)
{
	remote_handle *ph = 0;
	if(adsp_pls_add_lookup) {
		if(0
		   == adsp_pls_add_lookup(
		       (uint32_t)_adsp_listener_handle, 0, sizeof(*ph),
...
#endif //_USRDLL
uint32_t
_adsp_listener_atomic_CompareAndExchange(uint32_t *volatile puDest,
...
_adsp_listener_handle(void)
{
	...
	if((remote_handle)-1 != handle) {
		...
		remote_handle tmp;
		int nErr
		    = _adsp_listener_pls_ctor("adsp_listener", (void *)&tmp);
		if(nErr) {
			return (remote_handle)-1;
		}
		if(((remote_handle)-1 != handle)
...
__QAIC_STUB(adsp_listener_next_invoke)(
    ...
    uint32_t *sc, adsp_listener_buffer *inBuffers, int inBuffersLen,
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(adsp_listener_invoke_get_in_bufs)(
...
static __inline int
_stub_method_2(remote_handle _handle, uint32_t _mid)
{
	remote_arg *_pra = 0;
	int _nErr = 0;
	_TRY(_nErr,
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(adsp_listener_next2)(adsp_listener_invoke_ctx prevCtx,
                                 int prevResult, const uint8_t *prevbufs,
                                 int prevbufsLen,
                                 adsp_listener_invoke_ctx *ctx,
                                 adsp_listener_remote_handle *handle,
                                 uint32_t *sc, uint8_t *bufs, int bufsLen,
...
__QAIC_STUB(adsp_listener_get_in_bufs2)(adsp_listener_invoke_ctx ctx,
                                        int offset, uint8_t *bufs, int bufsLen,
                                        int *bufsLenReq) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 5;
	remote_handle _handle = _adsp_listener_handle();
