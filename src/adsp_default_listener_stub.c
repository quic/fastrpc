...
__QAIC_STUB_EXPORT remote_handle _adsp_default_listener_handle(void) {
  ...
  if ((remote_handle)-1 != handle) {
    ...
    remote_handle tmp;
    int nErr =
        _adsp_default_listener_pls_ctor("adsp_default_listener", (void *)&tmp);
    if (nErr) {
      return (remote_handle)-1;
    }
    if (((remote_handle)-1 != handle) ||
        ((remote_handle)-1 !=
         (remote_handle)_adsp_default_listener_atomic_CompareAndExchange(
