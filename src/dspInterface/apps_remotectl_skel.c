...
static __inline int _skel_method(int (*_pfn)(int, char*, int, int*), uint32_t _sc, remote_arg* _pra) {
   ...
   _TRY(_nErr, _pfn((int)*_in0, (char*)*_rout1, (int)*_rout1Len, (int*)_rout2));
   _COPY(_primROut, 0, _rout2, 0, 4);
   _CATCH(_nErr) {}
   return _nErr;
}
static __inline int _skel_method_1(int (*_pfn)(const char*, int*, char*, int, int*), uint32_t _sc, remote_arg* _pra) {
   remote_arg* _praEnd = 0;
   char* _in0[1] = {0};
   ...
   _TRY(_nErr, _pfn((const char*)*_in0, (int*)_rout1, (char*)*_rout2, (int)*_rout2Len, (int*)_rout3));
