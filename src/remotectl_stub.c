...
static const Parameter parameters[6]
...
static const Method *const methodArrays[4]
    = { &(methods[0]), &(methods[1]), &(methods[2]), &(methods[3]) };
static const char strings[84] = "set_param\0grow_"
				"heap\0phyAddr\0dlerror\0params\0handle\0reqID"
				"\0nSize\0close\0nErr\0name\0open\0";
static const uint16_t methodStrings[15]
    = { 78, 73, 43, 28, 68, 62, 43, 28, 68, 0, 50, 36, 10, 20, 56 };
static const uint16_t methodStringsArrays[4] = { 0, 5, 12, 9 };
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(remotectl_slim)
    = { 4,      &(methodArrays[0]),        0,
...
static int
_remotectl_pls_ctor(void *ctx, void *data)
{
	remote_handle *ph = (remote_handle *)data;
	*ph = _const_remotectl_handle;
...
_remotectl_handle(void)
{
	remote_handle *ph = 0;
	if(adsp_pls_add_lookup) {
		if(0
		   == adsp_pls_add_lookup((uint32_t)_remotectl_handle, 0,
		                          sizeof(*ph), _remotectl_pls_ctor,
		                          "remotectl", _remotectl_pls_dtor,
		                          (void **)&ph)) {
			return *ph;
		}
		...
		if(0
		   == HAP_pls_add_lookup((uint32_t)_remotectl_handle, 0,
		                         sizeof(*ph), _remotectl_pls_ctor,
		                         "remotectl", _remotectl_pls_dtor,
		                         (void **)&ph)) {
			return *ph;
...
uint32_t _remotectl_atomic_CompareAndExchange(uint32_t *volatile puDest,
                                              uint32_t uExchange,
                                              uint32_t uCompare);
...
#endif //_USRDLL
uint32_t
_remotectl_atomic_CompareAndExchange(uint32_t *volatile puDest,
                                     uint32_t uExchange, uint32_t uCompare)
{
...
_remotectl_handle(void)
{
	...
	if((remote_handle)-1 != handle) {
		...
		if(((remote_handle)-1 != handle)
		   || ((remote_handle)-1
		       != (remote_handle)_remotectl_atomic_CompareAndExchange(
...
#endif
static __inline int
_stub_method(remote_handle _handle, uint32_t _mid, char *_in0[1],
...
__QAIC_STUB(remotectl_open)(const char *name, int *handle, char *dlerror,
                            int dlerrorLen, int *nErr) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 0;
	remote_handle _handle = _remotectl_handle();
	if(_handle != (remote_handle)-1) {
		return _stub_method(_handle, _mid, (char **)&name,
		                    (uint32_t *)handle, (char **)&dlerror,
		                    (uint32_t *)&dlerrorLen, (uint32_t *)nErr);
	} else {
		return AEE_EINVHANDLE;
	}
}
static __inline int
_stub_method_1(remote_handle _handle, uint32_t _mid, uint32_t _in0[1],
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl_close)(int handle, char *dlerror, int dlerrorLen,
                             int *nErr) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 1;
	remote_handle _handle = _remotectl_handle();
	if(_handle != (remote_handle)-1) {
		return _stub_method_1(
		    _handle, _mid, (uint32_t *)&handle, (char **)&dlerror,
		    (uint32_t *)&dlerrorLen, (uint32_t *)nErr);
	} else {
		return AEE_EINVHANDLE;
	}
...
static __inline int
_stub_method_2(remote_handle _handle, uint32_t _mid, uint32_t _in0[1],
               uint32_t _in1[1])
{
	remote_arg _pra[1] = { 0 };
	uint32_t _primIn[2] = { 0 };
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl_grow_heap)(uint32_t phyAddr,
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl_set_param)(int reqID, const uint32_t *params,
