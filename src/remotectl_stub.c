...
static const Method *const methodArrays[4] = {&(methods[0]), &(methods[1]),
                                              &(methods[2]), &(methods[3])};
...
static const uint16_t methodStrings[15] = {78, 73, 43, 28, 68, 62, 43, 28,
                                           68, 0,  50, 36, 10, 20, 56};
static const uint16_t methodStringsArrays[4] = {0, 5, 12, 9};
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(remotectl_slim) = {
    4,      &(methodArrays[0]), 0, 0, &(methodStringsArrays[0]), methodStrings,
    strings};
...
__QAIC_STUB_EXPORT remote_handle _remotectl_handle(void) {
  ...
  if (adsp_pls_add_lookup) {
    if (0 == adsp_pls_add_lookup((uint32_t)_remotectl_handle, 0, sizeof(*ph),
                                 _remotectl_pls_ctor, "remotectl",
                                 _remotectl_pls_dtor, (void **)&ph)) {
      return *ph;
    }
    return (remote_handle)-1;
  } else if (HAP_pls_add_lookup) {
    if (0 == HAP_pls_add_lookup((uint32_t)_remotectl_handle, 0, sizeof(*ph),
                                _remotectl_pls_ctor, "remotectl",
                                _remotectl_pls_dtor, (void **)&ph)) {
      return *ph;
    }
...
static __inline int _stub_method_1(remote_handle _handle, uint32_t _mid,
                                   uint32_t _in0[1], char *_rout1[1],
                                   uint32_t _rout1Len[1], uint32_t _rout2[1]) {
  int _numIn[1] = {0};
  remote_arg _pra[3] = {0};
  uint32_t _primIn[2] = {0};
...
__QAIC_STUB(remotectl_close)(int handle, char *dlerror, int dlerrorLen,
  ...
  remote_handle _handle = _remotectl_handle();
  if (_handle != (remote_handle)-1) {
    return _stub_method_1(_handle, _mid, (uint32_t *)&handle, (char **)&dlerror,
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl_grow_heap)(uint32 phyAddr,
                                 uint32 nSize) __QAIC_STUB_ATTRIBUTE {
  uint32_t _mid = 2;
  remote_handle _handle = _remotectl_handle();
  if (_handle != (remote_handle)-1) {
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl_set_param)(int reqID, const uint32 *params,
                                 int paramsLen) __QAIC_STUB_ATTRIBUTE {
  uint32_t _mid = 3;
  remote_handle _handle = _remotectl_handle();
  if (_handle != (remote_handle)-1) {
    return _stub_method_3(_handle, _mid, (uint32_t *)&reqID, (char **)&params,
                          (uint32_t *)&paramsLen);
  } else {
    return AEE_EINVHANDLE;
  }
