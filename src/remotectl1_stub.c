...
static const Method methods[6]
	...
	{ REMOTE_SCALARS_MAKEX(0, 0, 0x1, 0x0, 0x0, 0x0), 0x8, 0x0, 2, 2,
	  (&(parameterArrays[9])), 0x4, 0x0 },
	{ REMOTE_SCALARS_MAKEX(0, 0, 0x2, 0x0, 0x0, 0x0), 0x8, 0x0, 3, 2,
	  (&(parameterArrays[7])), 0x4, 0x0 } };
static const Method *const methodArrays[6]
    = { &(methods[0]), &(methods[1]), &(methods[2]),
	&(methods[3]), &(methods[4]), &(methods[5]) };
static const char strings[103]
    = "set_param\0grow_"
      "heap\0phyAddr\0dlerror\0params\0close1\0handle\0reqID\0nSize\0open1\0cl"
      "ose\0nErr\0name\0open\0uri\0h\0";
static const uint16_t methodStrings[20]
    = { 69, 86, 50, 28, 81, 43, 50, 28,  81, 0,
	57, 36, 10, 20, 63, 91, 96, 100, 75, 100 };
static const uint16_t methodStringsArrays[6] = { 15, 18, 0, 5, 12, 9 };
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(remotectl1_slim)
    = { 6,      &(methodArrays[0]),        0,
...
__QAIC_STUB(remotectl1_open)(const char *uri,
                             remote_handle64 *h) __QAIC_STUB_ATTRIBUTE
{
	return __QAIC_REMOTE(remote_handle64_open)(uri, h);
}
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl1_close)(remote_handle64 h) __QAIC_STUB_ATTRIBUTE
{
	return __QAIC_REMOTE(remote_handle64_close)(h);
}
static __inline int
_stub_method(remote_handle64 _handle, uint32_t _mid, char *_in0[1],
             uint32_t _rout1[1], char *_rout2[1], uint32_t _rout2Len[1],
             uint32_t _rout3[1])
{
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl1_open1)(remote_handle64 _handle, const char *name,
                              int *handle, char *dlerror, int dlerrorLen,
                              int *nErr) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 2;
	return _stub_method(_handle, _mid, (char **)&name, (uint32_t *)handle,
	                    (char **)&dlerror, (uint32_t *)&dlerrorLen,
	                    (uint32_t *)nErr);
}
static __inline int
_stub_method_1(remote_handle64 _handle, uint32_t _mid, uint32_t _in0[1],
               char *_rout1[1], uint32_t _rout1Len[1], uint32_t _rout2[1])
{
	int _numIn[1] = { 0 };
	remote_arg _pra[3] = { 0 };
	uint32_t _primIn[2] = { 0 };
...
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl1_close1)(remote_handle64 _handle, int handle,
                               char *dlerror, int dlerrorLen,
                               int *nErr) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 3;
	return _stub_method_1(_handle, _mid, (uint32_t *)&handle,
	                      (char **)&dlerror, (uint32_t *)&dlerrorLen,
	                      (uint32_t *)nErr);
}
static __inline int
_stub_method_2(remote_handle64 _handle, uint32_t _mid, uint32_t _in0[1],
               uint32_t _in1[1])
{
	remote_arg _pra[1] = { 0 };
	uint32_t _primIn[2] = { 0 };
	...
	_TRY_FARF(_nErr, __QAIC_REMOTE(remote_handle64_invoke)(
			     _handle,
			     REMOTE_SCALARS_MAKEX(0, _mid, 1, 0, 0, 0), _pra));
	_CATCH_FARF(_nErr)
	{
		_QAIC_FARF(RUNTIME_ERROR,
		           "ERROR 0x%x: handle=0x%" PRIx64
		           ", scalar=0x%x, method ID=%d: %s failed\n",
		           _nErr, _handle,
		           REMOTE_SCALARS_MAKEX(0, _mid, 1, 0, 0, 0), _mid,
		           __func__);
	}
	return _nErr;
}
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl1_grow_heap)(remote_handle64 _handle, uint32_t phyAddr,
                                  uint32_t nSize) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 4;
	return _stub_method_2(_handle, _mid, (uint32_t *)&phyAddr,
	                      (uint32_t *)&nSize);
}
static __inline int
_stub_method_3(remote_handle64 _handle, uint32_t _mid, uint32_t _in0[1],
               char *_in1[1], uint32_t _in1Len[1])
{
	remote_arg _pra[2] = { 0 };
	uint32_t _primIn[2] = { 0 };
	remote_arg *_praIn = 0;
	...
	_TRY_FARF(_nErr, __QAIC_REMOTE(remote_handle64_invoke)(
			     _handle,
			     REMOTE_SCALARS_MAKEX(0, _mid, 2, 0, 0, 0), _pra));
	_CATCH_FARF(_nErr)
	{
		_QAIC_FARF(RUNTIME_ERROR,
		           "ERROR 0x%x: handle=0x%" PRIx64
		           ", scalar=0x%x, method ID=%d: %s failed\n",
		           _nErr, _handle,
		           REMOTE_SCALARS_MAKEX(0, _mid, 2, 0, 0, 0), _mid,
		           __func__);
	}
	return _nErr;
}
__QAIC_STUB_EXPORT int
__QAIC_STUB(remotectl1_set_param)(remote_handle64 _handle, int reqID,
                                  const uint32_t *params,
                                  int paramsLen) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 5;
	return _stub_method_3(_handle, _mid, (uint32_t *)&reqID,
	                      (char **)&params, (uint32_t *)&paramsLen);
}
#ifdef __cplusplus
}
#endif
