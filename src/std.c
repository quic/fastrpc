...
int std_wstrlcpy(AECHAR *pwcDst, const AECHAR *cpwszSrc, int nDestSize) {
  ...
  if (0 < nDestSize) {
    ...
    (void)std_memsmove(pwcDst, nDestSize * sizeof(AECHAR), cpwszSrc,
                       ((size_t)n) * sizeof(AECHAR));
...
//
//
static char *x_scanbytes(const char *pcBuf, const char *cpszChars, int nLen,
                         unsigned char cStop, boolean bTestEqual) {
  int n;
  unsigned a[8];
...
char *std_striends(const char *cpsz, const char *cpszSuffix) {
  ...
  if ((0 <= nOffset) && (0 == std_stricmp(cpsz + nOffset, cpszSuffix))) {
...
char *std_strends(const char *cpsz, const char *cpszSuffix) {
  ...
  if ((0 <= nOffset) && (0 == std_strcmp(cpsz + nOffset, cpszSuffix))) {
