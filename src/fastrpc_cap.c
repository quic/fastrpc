...
const char *RPROC_SUBSYSTEM_NAME[] = {
    "adsp", "mss", "spss", "cdsp", "cdsp1", "reserved", "reserved", "reserved"};
...
static inline uint32_t fastrpc_check_if_dsp_present_pil(uint32_t domain) {
  ...
  const char *SUBSYSTEM_DEV_NAME[] = {
      "/dev/subsys_adsp",  "",         "/dev/subsys_slpi", "/dev/subsys_cdsp",
      "/dev/subsys_cdsp1", "reserved", "reserved",         "reserved",
...
bail:
  ...
  if (nErr) {
    FARF(ERROR,
         "Warning 0x%x: %s failed to get attribute %u for domain %u (errno %s)",
...
static int check_status_notif_version2_capability(int domain) {
  ...
  if (nErr == 0) {
    ...
    FARF(RUNTIME_RPC_HIGH, "%s: Capability not found. nErr: 0x%x", __func__,
         nErr);
  }
  return 0;
}
...
int is_proc_sharedbuf_supported_dsp(int domain) {
  ...
  if (proc_sharedbuf_capability == -1) {
    ...
    if (nErr == AEE_SUCCESS) {
      ...
      FARF(RUNTIME_RPC_HIGH, "Error 0x%x: %s: capability not found", nErr,
           __func__);
