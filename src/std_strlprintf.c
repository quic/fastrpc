...
#define FORMATNUMBER_SIZE                                                      \
  24 // octal: 22 + '0' + null ;  decimal: 20 + sign + null
...
FormatNumber(FieldFormat *me, char pcBuf[FORMATNUMBER_SIZE], uint64 uNum64) {
  ...
  nBase = ('o' == cType ? 8 : 'x' == TOLOWER(cType) ? 16 : 10);
  cpszDigits = ((cType == 'X') ? "0123456789ABCDEF" : "0123456789abcdef");
...
static int ConvertFloat(FieldFormat *me, double dNumber, char *pcBuffer,
  ...
  if (cType != 'a') {
    ...
    nError =
        std_dtoa_decimal(dNumber, nPrecision, szIntegerPart, szFractionPart);
...
//
// The size of the output buffer pcBuffer should be at least
// STD_DTOA_FORMAT_FLOAT_SIZE.
//
static int FormatFloat(FieldFormat *me, double dNumber,
  ...
  case FP_TYPE_GENERAL:
    ...
    nError = ConvertFloat(me, dNumber, pcBuffer, STD_DTOA_FORMAT_FLOAT_SIZE);
    CLEANUP_ON_ERROR(nError, bail);
...
static int std_strlprintf_inner(char *pszDest, int nDestSize,
  ...
  for (;;) {
    ...
    if ('s' == cType) {
    ...
    } else if ('u' == cType || 'o' == cType || 'd' == cType || 'i' == cType ||
               'p' == cType || 'x' == TOLOWER(cType)) {
      ...
      FormatNumber(&ff, achBuf, uArg64);
               ...
               ('e' == TOLOWER(cType) || 'f' == TOLOWER(cType) ||
                'g' == TOLOWER(cType) || 'a' == TOLOWER(cType))) {
...
int std_vstrlprintf(char *pszDest, int nDestSize, const char *cpszFmt,
                    AEEVaList args) {
  return std_strlprintf_inner(pszDest, nDestSize, cpszFmt, args, NULL);
}

int std_vsnprintf(char *pszDest, int nDestSize, const char *cpszFmt,
                  AEEVaList args)
/*
