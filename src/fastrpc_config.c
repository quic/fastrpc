...
int
fastrpc_read_config_file_from_path(const char *base, const char *file)
{
	int nErr = 0;
	apps_std_FILE fp = -1;
	uint64_t len;
	...
	do {
		...
		VERIFY(AEE_SUCCESS
		       == (nErr = apps_std_fgets(fp, buf, len, &eof)));
		if(eof) {
			break;
		}
		...
		if(strncmp(param, CONFIG_ERR_CODES, strlen(CONFIG_ERR_CODES))
		   == 0) {
			...
			FARF(ALWAYS,
			     "%s: panic error codes applicable "
			     "only to cdsp domain\n",
			     __func__);
			do {
				...
				if(param != NULL) {
					...
					if(err_code >= MIN_DSP_ERR_RNG
					   && err_code <= MAX_DSP_ERR_RNG) {
						frpc_config.err_codes_to_crash
						    .err_code[ii]
						    = err_code;
						FARF(ALWAYS,
						     "%s : panic error codes "
			...
			num_err_codes = ii;
			frpc_config.err_codes_to_crash.num_err_codes
			    = num_err_codes;
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS, "fastrpc config "
				             "enabling PD "
				             "dump\n");
			}
		} else if(strncmp(param, CONFIG_RPCTIMEOUT,
		                  strlen(CONFIG_RPCTIMEOUT))
		          == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if(param != NULL) {
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS, "fastrpc config "
		                  ...
		                  strlen(CONFIG_COLLECT_RUNTIME_FARF))
		          == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if(param != NULL) {
				...
				VERIFYC(NULL
						    ...
						    * logFileNameLen)),
				        AEE_ENOMEMORY);
				strlcpy(frpc_config.farf_log_filename, param,
				        logFileNameLen);
				FARF(ALWAYS,
				     "fastrpc config "
				     "enabling farf "
			      ...
			      param, CONFIG_COLLECT_RUNTIME_FARF_USERSPACE,
			...
			if(param != NULL) {
				...
				FARF(ALWAYS, "fastrpc config "
		                  ...
		                  strlen(CONFIG_DEBUG_SYSMON_LOGGING))
		          == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if(param != NULL) {
...
struct err_codes *
fastrpc_config_get_errcodes(void)
{
...
bool
fastrpc_config_is_sysmon_reserved_bit_enabled(void)
{
...
bool
fastrpc_config_is_qtf_tracing_enabled(void)
{
...
int
fastrpc_config_get_caller_level(void)
{
	return frpc_config.caller_level;
}

// Function to get if uaf should be enabled in heap
bool
fastrpc_config_is_uaf_enabled(void)
{
	return frpc_config.uaf_enabled;
}

// Function to get if Log packet is enabled.
bool
fastrpc_config_is_logpacket_enabled(void)
{
...
int
fastrpc_config_get_leak_detect(void)
{
	return frpc_config.leak_detect;
}

// Function to return the call stack num
int
fastrpc_config_get_caller_stack_num(void)
{
	return frpc_config.num_call_stack;
}

bool
fastrpc_config_is_setdmabufname_enabled(void)
{
...
fastrpc_config_init()
{
	...
	VERIFYC(NULL != (name = std_basename(__progname)),
	        AEE_EINVALIDPROCNAME);
	len = strlen(name) + strlen(file_extension) + 1;
	VERIFYC(NULL != (config_file = calloc(1, sizeof(char) * len)),
	        AEE_ENOMEMORY);
	...
	for(i = 0; i < (int)numPaths; ++i) {
		VERIFYC(NULL
		            != (paths[i].data
		                = calloc(1, sizeof(char) * maxPathLen)),
		        AEE_ENOMEMORY);
	...
	VERIFYC(NULL
	            != (data_paths
	                = calloc(1, sizeof(char) * maxPathLen * numPaths)),
	        AEE_ENOMEMORY);

	// Get the paths
	VERIFY(AEE_SUCCESS
	       == (nErr = apps_std_get_search_paths_with_env(
		       ADSP_LIBRARY_PATH, ";", paths, numPaths, &len,
		       &maxPathLen)));
	maxPathLen += 1;
	for(i = 0; i < (int)numPaths; ++i) {
		...
		strlcat(data_paths, ", ",
		        sizeof(char) * maxPathLen * numPaths);
		if(0
		   == fastrpc_read_config_file_from_path(paths[i].data,
		                                         config_file)) {
			file_found = 1;
			FARF(ALWAYS,
			     "Read fastrpc config file %s "
			     "found at %s\n",
			     config_file, paths[i].data);
			break;
		}
	}
	if(!file_found) {
		FARF(ALWAYS, "%s: Couldn't find file %s, errno (%s) at %s\n",
		     __func__, config_file, strerror(errno), data_paths);
	}
bail:
	if(nErr) {
