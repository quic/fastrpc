...
static const char *
get_secure_device_name(int domain_id)
{
	const char *name;
...
static const char *
get_default_device_name(int domain_id)
{
	const char *name;
	int domain = GET_DOMAIN_FROM_EFFEC_DOMAIN_ID(domain_id);
 ...
 */
static bool
fastrpc_dev_exists(const char *dev_name)
{
	struct stat buffer;
	char *path = NULL;
	uint64_t len;
 ...
 */
static int
fastrpc_wait_for_device(int domain)
{
	...
	if(fastrpc_dev_exists(sec_dev_name)
	   || fastrpc_dev_exists(def_dev_name))
		return 0;

	inotify_fd = inotify_init();
	if(inotify_fd < 0) {
		VERIFY_EPRINTF("Error: inotify_init failed, invalid "
		               "fd errno = %s\n",
		               strerror(errno));
	...
	if(watch_fd < 0) {
		close(inotify_fd);
		VERIFY_EPRINTF("Error: inotify_add_watch failed, "
		               "invalid fd errno = %s\n",
		               strerror(errno));
		return AEE_EINVALIDFD;
	}
	...
	while(1) {
		...
		if(ret < 0) {
			VERIFY_EPRINTF("Error: %s: polling for event "
			               "failed errno(%s)\n",
		...
		for(char *ptr = buffer; ptr < buffer + len;
			...
			/* Check if the event corresponds to
			 * the creation of the device node. */
...
int
adsp_default_listener_start(int argc, char *argv[])
{
	struct pollfd pfd;
	eventfd_t event = 0;
	...
	if(argc > 2) {
		...
		VERIFYC(AEE_SUCCESS
		            == (nErr = fastrpc_wait_for_device(domain_id)),
		        AEE_ECONNREFUSED);
		...
		namelen = strlen(ITRANSPORT_PREFIX CREATE_STATICPD)
		          + strlen(argv[1]) + strlen(ADSP_DOMAIN);
		VERIFYC(NULL
		...
		if(!strncmp(argv[1], ROOTPD_NAME, strlen(argv[1]))) {
			...
			strlcat(name, argv[1],
			        strlen(ITRANSPORT_PREFIX CREATE_STATICPD)
			            + strlen(argv[1]) + 1);
		}
		...
		VERIFY(AEE_SUCCESS
		       == (nErr = remote_handle64_open(name, &fd)));
		goto start_listener;
	} else if(argc > 1) {
		/* One arguments is passed in below format
		 * Example: ./adsprpcd
		 * "createstaticpd:audiopd&dom=adsp" (or) ./adsprpcd
		 * audiopd Get domain name from arguments and use
		 * domains API.
		 */
		...
		if(domain_id != INVALID_DOMAIN_ID) {
			VERIFYC(
			    ...
			    AEE_ECONNREFUSED);
			VERIFY(AEE_SUCCESS
			       == (nErr = remote_handle64_open(argv[1], &fd)));
			goto start_listener;
		}
		...
		// If no arguments passed, default/rootpd daemon of
		// remote subsystem
	...
	VERIFYC(AEE_SUCCESS
	            == (nErr = fastrpc_wait_for_device(DEFAULT_DOMAIN_ID)),
	        AEE_ECONNREFUSED);
	// Default case: Open non-domain static process handle
	VERIFY_IPRINTF("%s started with arguments %s\n", __func__, name);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_open(name, (remote_handle *)&fd)));
start_listener:
	VERIFYC(!setenv("ADSP_LISTENER_MEM_CACHE_SIZE", "1048576", 0),
	        AEE_ESETENV);

	// If domain name part of arguments, use domains API
	if(domain_id != INVALID_DOMAIN_ID) {
		// Allocate memory and copy adsp_default_listener1 URI
		// Example: "adsp_default_listener1_URI&dom=adsp"
		VERIFYC(
		    NULL
		...
		if(nErr < 0) {
			VERIFY_EPRINTF("ERROR: %s: %d returned from "
			               "snprintf\n",
			               __func__, nErr);
			nErr = AEE_EFAILED;
			goto bail;
		...
		nErr = adsp_default_listener1_open(
		    adsp_default_listener1_URI_domain, &listener_fd);

		// Register daemon as default listener to static
		// process
		if(nErr == AEE_SUCCESS) {
			VERIFY(0
			       == (nErr = adsp_default_listener1_register(
				       listener_fd)));
			goto start_poll;
		}
	}
	...
	VERIFY_IPRINTF("%s domains support is not available for "
	               ...
	               __func__);
	VERIFY(0
	       == (nErr = remote_handle_open("adsp_default_listener",
	                                     (remote_handle *)&listener_fd)));
	VERIFY(0 == (nErr = adsp_default_listener_register()));
start_poll:
	// If domain name part of arguments, use domains API
	if(domain_id != INVALID_DOMAIN_ID) {
		// Allocate memory and copy geteventfd URI Example:
		// "ITRANSPORT_PREFIX geteventfd&dom=adsp"
		eventfdlen = strlen(ITRANSPORT_PREFIX "geteventfd")
		             + MAX_DOMAIN_URI_SIZE;
		VERIFYC(NULL
		            != (eventfd_domain
		                = (char *)malloc((eventfdlen) * sizeof(char))),
		        AEE_ENOMEMORY);
		nErr = snprintf(eventfd_domain, eventfdlen, "%s%s",
		                ITRANSPORT_PREFIX "geteventfd",
		                dsp_domain->uri);
		if(nErr < 0) {
			VERIFY_EPRINTF("ERROR: %s: %d returned from "
			               "snprintf\n",
		...
		VERIFY(0
		       == (nErr
		           = remote_handle64_open(eventfd_domain, &event_fd)));
		pfd.fd = (remote_handle)event_fd;
	} else {
		VERIFY(0
		       == (nErr
		           = remote_handle_open(ITRANSPORT_PREFIX "geteventfd",
