...
main(int argc, char *argv[])
{
	...
	while(1) {
		if(NULL
		   != (sdsphandler
		       = dlopen(SDSP_DEFAULT_LISTENER_NAME, RTLD_NOW))) {
			if(NULL
			   != (listener_start = (adsp_default_listener_start_t)
			           dlsym(sdsphandler, "adsp_default_"
			                              "listener_start"))) {
				VERIFY_IPRINTF("sdsp_default_"
				               "listener_start "
				               "called");
		...
		if(nErr == AEE_ECONNREFUSED) {
			VERIFY_EPRINTF("fastRPC device driver is "
			               "disabled, retrying...");
		}
