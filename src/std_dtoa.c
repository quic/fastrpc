...
double fp_pow_10(int nPow) {
  ...
  double aTablePos[] = {0, 1e1, 1e2, 1e4, 1e8, 1e16, 1e32, 1e64, 1e128, 1e256};
  ...
  for (ulI = 0; (nPrecision > 0) && (ulI < STD_DTOA_FORMAT_FRACTION_SIZE - 1);
    ...
    if (0.0 == dFractionPart) {
      ...
      double dRoundingValue =
          FP_POW_10(-(nPrecision + STD_DTOA_PRECISION_ROUNDING_VALUE));
      acFractionPart[ulI] =
          (int)((dFractionPart + dRoundingValue) * 10.0) + '0';
