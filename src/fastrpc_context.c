...
static int fastrpc_context_init_domains(fastrpc_context_create *create,
  ...
  for (i = 0; i < num_domain_ids; i++) {
    ...
    VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(effec_domain_id), AEE_EBADDOMAIN);
  ...
  nErr = AEE_EBADPARM;
  FARF(ALWAYS,
       "Error 0x%x: %s: context cannot be both multi-session & multi-domain",
       nErr, __func__);
  return nErr;
bail:
  if (nErr)
...
static fastrpc_context *fastrpc_context_init(unsigned int num_domain_ids) {
  ...
  VERIFYC(NULL != (ctx = (fastrpc_context *)calloc(1, sizeof(fastrpc_context))),
          AEE_ENOMEMORY);
  ...
  VERIFYC(NULL != (ctx->effec_domain_ids = (unsigned int *)calloc(
          ...
          AEE_ENOMEMORY);
  VERIFYC(NULL != (ctx->domains = (unsigned int *)calloc(
                       num_domain_ids, sizeof(*ctx->domains))),
          AEE_ENOMEMORY);
  VERIFYC(NULL !=
              (ctx->devs = (int *)calloc(num_domain_ids, sizeof(*ctx->devs))),
          AEE_ENOMEMORY);
...
int fastrpc_destroy_context(uint64_t ctxid) {
  ...
  VERIFYC(-1 != (dev = get_device_fd(ctx->effec_domain_ids[0])),
          AEE_EINVALIDDEVICE);
  VERIFY(AEE_SUCCESS ==
         (nErr = ioctl_mdctx_manage(dev, FASTRPC_MDCTX_REMOVE, NULL, NULL, 0,
                                    &ctx->ctxid)));
...
bail:
  if (nErr) {
    FARF(ALWAYS, "Error 0x%x: %s failed for ctx 0x%" PRIx64 "", nErr, __func__,
         ctxid);
  }
  return nErr;
}
...
int fastrpc_create_context(fastrpc_context_create *create) {
  ...
  VERIFYC(num_domain_ids && num_domain_ids < NUM_DOMAINS_EXTEND, AEE_EBADPARM);

  VERIFYC(NULL != (ctx = fastrpc_context_init(num_domain_ids)),
          AEE_EBADCONTEXT);
  VERIFYC(AEE_SUCCESS == (nErr = fastrpc_context_init_domains(create, ctx)),
          AEE_EBADPARM);
  ...
  for (i = 0; i < num_domain_ids; i++) {
    ...
    VERIFY(AEE_SUCCESS ==
           (nErr = fastrpc_session_open(effec_domain_id, &ctx->devs[i])));
  }
  ...
  FARF(ALWAYS, "%s done with context 0x%" PRIx64 " on %u domain ids", __func__,
       ctx->ctxid, num_domain_ids);
bail:
  if (nErr) {
...
int fastrpc_context_get_domains(uint64_t ctxid, unsigned int **effec_domain_ids,
