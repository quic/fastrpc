...
static inline void
fastrpc_context_add_to_table(fastrpc_context *ctx)
{
...
static inline void
fastrpc_context_remove_from_table(fastrpc_context *ctx)
{
	pthread_mutex_lock(&gctx.mut);
...
static inline fastrpc_context *
fastrpc_context_validate(uint64_t ctxid)
{
 ...
 * basis:
 *		Context can either be created on multiple sessions on one
 *domain or multiple domains, but not both. For example, context cannot be
 *created on 2 sessions on CDSP + 1 session on ADSP.
 *
 * returns 0 on success
 */
static int
fastrpc_context_init_domains(fastrpc_context_create *create,
                             fastrpc_context *ctx)
{
	int nErr = AEE_SUCCESS, i = 0, j = 0;
	unsigned int effec_domain_id = 0,
	...
	for(i = 0; i < num_domain_ids; i++) {
		...
		domain_ids[i]
		    = GET_DOMAIN_FROM_EFFEC_DOMAIN_ID(effec_domain_id);

		/*
		 * If client is trying to create context on multiple
		 * sessions on same domain, validate that all domain
		 * ids are the same.
		 */
	...
	nErr = AEE_EBADPARM;
	FARF(ALWAYS,
	     "Error 0x%x: %s: context cannot be both multi-session & "
	     "multi-domain",
	     nErr, __func__);
	return nErr;
bail:
 ...
 */
static int
fastrpc_context_deinit(fastrpc_context *ctx)
{
 ...
 */
static fastrpc_context *
fastrpc_context_init(unsigned int num_domain_ids)
{
	fastrpc_context *ctx = NULL;
	...
	VERIFYC(NULL
	            != (ctx = (fastrpc_context *)calloc(
			    1, sizeof(fastrpc_context))),
	        AEE_ENOMEMORY);

	pthread_mutex_init(&ctx->mut, 0);

	VERIFYC(NULL
	            != (ctx->effec_domain_ids = (unsigned int *)calloc(
			    num_domain_ids, sizeof(*ctx->effec_domain_ids))),
	        AEE_ENOMEMORY);
	VERIFYC(NULL
	            != (ctx->domains = (unsigned int *)calloc(
			    num_domain_ids, sizeof(*ctx->domains))),
	        AEE_ENOMEMORY);
	VERIFYC(NULL
	            != (ctx->devs
	                = (int *)calloc(num_domain_ids, sizeof(*ctx->devs))),
	        AEE_ENOMEMORY);
...
int
fastrpc_destroy_context(uint64_t ctxid)
{
	...
	VERIFYC(-1 != (dev = get_device_fd(ctx->effec_domain_ids[0])),
	        AEE_EINVALIDDEVICE);
	VERIFY(AEE_SUCCESS
	       == (nErr = ioctl_mdctx_manage(dev, FASTRPC_MDCTX_REMOVE, NULL,
	                                     NULL, 0, &ctx->ctxid)));
...
int
fastrpc_create_context(fastrpc_context_create *create)
{
	int nErr = AEE_SUCCESS, dev = -1;
	...
	VERIFYC(NULL != (ctx = fastrpc_context_init(num_domain_ids)),
	        AEE_EBADCONTEXT);
	VERIFYC(AEE_SUCCESS
	            == (nErr = fastrpc_context_init_domains(create, ctx)),
	        AEE_EBADPARM);
	...
	for(i = 0; i < num_domain_ids; i++) {
		...
		VERIFY(AEE_SUCCESS
		       == (nErr = fastrpc_session_open(effec_domain_id,
		                                       &ctx->devs[i])));
	}
	...
	VERIFYC(-1 != (dev = get_device_fd(ctx->effec_domain_ids[0])),
	        AEE_EINVALIDDEVICE);
	VERIFY(AEE_SUCCESS
	       == (nErr = ioctl_mdctx_manage(dev, FASTRPC_MDCTX_SETUP, ctx,
	                                     ctx->domains, num_domain_ids,
	                                     &ctx->ctxid)));
...
int
fastrpc_context_get_domains(uint64_t ctxid, unsigned int **effec_domain_ids,
...
int
fastrpc_context_table_init(void)
{
	int nErr = AEE_SUCCESS;
...
int
fastrpc_context_table_deinit(void)
{
	int nErr = AEE_SUCCESS;
