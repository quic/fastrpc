...
static void
_adspmsgd_adsp_pls_dtor(void *data)
{
...
static int
_adspmsgd_adsp_pls_ctor(void *ctx, void *data)
{
	remote_handle *ph = (remote_handle *)data;
	*ph = _const_adspmsgd_adsp_handle;
...
_adspmsgd_adsp_handle(void)
{
	remote_handle *ph = 0;
	if(adsp_pls_add_lookup) {
		if(0
		   == adsp_pls_add_lookup(
		       (uint32_t)_adspmsgd_adsp_handle, 0, sizeof(*ph),
...
#endif //_USRDLL
uint32_t
_adspmsgd_adsp_atomic_CompareAndExchange(uint32_t *volatile puDest,
...
_adspmsgd_adsp_handle(void)
{
	...
	if((remote_handle)-1 != handle) {
		...
		remote_handle tmp;
		int nErr
		    = _adspmsgd_adsp_pls_ctor("adspmsgd_adsp", (void *)&tmp);
		if(nErr) {
			return (remote_handle)-1;
		}
		if(((remote_handle)-1 != handle)
...
__QAIC_STUB(adspmsgd_adsp_init)(int heapid, uint32_t ion_flags,
                                uint32_t filter, uint32_t buf_size,
...
static __inline int
_stub_method_1(remote_handle _handle, uint32_t _mid)
{
	remote_arg *_pra = 0;
	int _nErr = 0;
	_TRY(_nErr,
