...
/* check async support */
int
is_async_fastrpc_supported(void)
{
 ...
 */
const char *
get_secure_domain_name(int domain_id)
{
	const char *name;
...
int
ioctl_init(int dev, uint32_t flags, int attr, unsigned char *shell,
           int shelllen, int shellfd, char *mem, int memlen, int memfd,
           int tessiglen)
{
	int ioErr = 0;
	...
	case FASTRPC_INIT_CREATE:
		...
		ioErr = ioctl(dev, FASTRPC_IOCTL_INIT_CREATE,
		              (unsigned long)&init);
		break;
	default:
		FARF(ERROR, "ERROR: %s Invalid init flags passed %d", __func__,
		     flags);
		ioErr = AEE_EBADPARM;
		break;
	}
...
int
ioctl_invoke(int dev, int req, remote_handle handle, uint32_t sc, void *pra,
             ...
             uint64_t *perf_kernel, uint64_t *perf_dsp)
{
	...
	if(req >= INVOKE && req <= INVOKE_FD)
		ioErr
		    = ioctl(dev, FASTRPC_IOCTL_INVOKE, (unsigned long)&invoke);
	else
...
int
ioctl_invoke2_response(int dev, fastrpc_async_jobid *jobid,
...
int
ioctl_invoke2_notif(int dev, int *domain, int *session, int *status)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_mmap(int dev, int req, uint32_t flags, int attr, int fd, int offset,
           size_t len, uintptr_t vaddrin, uint64_t *vaddrout)
{
	...
	default:
		FARF(ERROR, "ERROR: %s Invalid request passed %d", __func__,
		     req);
		ioErr = AEE_EBADPARM;
		break;
	}
...
int
ioctl_munmap(int dev, int req, int attr, void *buf, int fd, int len,
             uint64_t vaddr)
{
	int ioErr = AEE_SUCCESS;
	...
	case MUNMAP_FD: {
		...
		unmap.vaddr = vaddr;
		unmap.length = len;
		ioErr = ioctl(dev, FASTRPC_IOCTL_MEM_UNMAP,
		              (unsigned long)&unmap);
	} break;
	case MUNMAP:
	case MUNMAP_64: {
		...
		ioErr
		    = ioctl(dev, FASTRPC_IOCTL_MUNMAP, (unsigned long)&unmap);
	} break;
	default:
		FARF(ERROR, "ERROR: %s Invalid request passed %d", __func__,
		     req);
...
int
ioctl_getinfo(int dev, uint32_t *info)
{
	*info = 1;
	return AEE_SUCCESS;
}
...
int
ioctl_getdspinfo(int dev, int domain, uint32_t attr, uint32_t *capability)
{
	int ioErr = AEE_SUCCESS;
...
int
ioctl_setmode(int dev, int mode)
{
...
int
ioctl_control(int dev, int req, void *c)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_getperf(int dev, int key, void *data, int *datalen)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_signal_create(int dev, uint32_t signal, uint32_t flags)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_signal_destroy(int dev, uint32_t signal)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_signal_signal(int dev, uint32_t signal)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_signal_wait(int dev, uint32_t signal, uint32_t timeout_usec)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_signal_cancel_wait(int dev, uint32_t signal)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_sharedbuf(int dev, struct fastrpc_proc_sharedbuf_info *sharedbuf_info)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_session_info(int dev, struct fastrpc_proc_sess_info *sess_info)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_optimization(int dev, uint32_t max_concurrency)
{
	return AEE_EUNSUPPORTED;
}

int
ioctl_mdctx_manage(int dev, int req, void *user_ctx, unsigned int *domain_ids,
