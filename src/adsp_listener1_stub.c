...
static const char strings[183] =
    ...
    "set\0handle\0next2\0init2\0close\0init\0open\0ctx\0uri\0sc\0h\0";
...
static __inline void _count(int _numIn[1], int _numROut[1], int _numInH[1],
                            int _numROutH[1], _ATTRIBUTE_UNUSED char *_rout0[1],
                            _ATTRIBUTE_UNUSED uint32_t _rout0Len[1]) {
  _numIn[0] += 0;
  _numROut[0] += 1;
  _numInH[0] += 0;
...
static __inline void _count_1(int _numIn[1], int _numROut[1], int _numInH[1],
                              int _numROutH[1], _ATTRIBUTE_UNUSED char *_in0[1],
                              _ATTRIBUTE_UNUSED uint32_t _in0Len[1]) {
  _numIn[0] += 1;
  _numROut[0] += 0;
  _numInH[0] += 0;
...
_stub_method(remote_handle64 _handle, uint32_t _mid, uint32_t _in0[1],
  ...
  _TRY_FARF(_nErr, __QAIC_REMOTE(remote_handle64_invoke)(
                       _handle,
                       REMOTE_SCALARS_MAKEX(0, _mid, (_numIn[0] + 1),
                                            (_numROut[0] + 1), (_numInH[0] + 0),
                                            (_numROutH[0] + 0)),
                       _pra));
  for (_ii = 0, _seq_nat2 = (char *)_in2[0]; _ii < (int)_in2Len[0];
       ++_ii, _seq_nat2 = (_seq_nat2 + SLIM_IFPTR32(8, 16))) {
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener1_next_invoke)(
    ...
    uint32 *sc, adsp_listener1_buffer *inBuffers, int inBuffersLen,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener1_invoke_get_in_bufs)(
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener1_init)(remote_handle64 _handle)
    __QAIC_STUB_ATTRIBUTE {
  uint32_t _mid = 4;
  return _stub_method_2(_handle, _mid);
}
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener1_init2)(
...
static __inline int _stub_method_3(remote_handle64 _handle, uint32_t _mid,
  ...
  _CATCH_FARF(_nErr) {
    _QAIC_FARF(RUNTIME_ERROR,
               "ERROR 0x%x: handle=0x%" PRIx64
               ", scalar=0x%x, method ID=%d: %s failed\n",
               _nErr, _handle, REMOTE_SCALARS_MAKEX(0, _mid, 2, 2, 0, 0), _mid,
               __func__);
  }
  return _nErr;
}
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener1_next2)(
    remote_handle64 _handle, adsp_listener1_invoke_ctx prevCtx, int prevResult,
    const uint8 *prevbufs, int prevbufsLen, adsp_listener1_invoke_ctx *ctx,
    adsp_listener1_remote_handle *handle, uint32 *sc, uint8 *bufs, int bufsLen,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener1_get_in_bufs2)(
    remote_handle64 _handle, adsp_listener1_invoke_ctx ctx, int offset,
    uint8 *bufs, int bufsLen, int *bufsLenReq) __QAIC_STUB_ATTRIBUTE {
  uint32_t _mid = 7;
