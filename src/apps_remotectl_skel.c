...
static __inline int
_skel_method(int (*_pfn)(int, char *, int, int *), uint32_t _sc,
             remote_arg *_pra)
{
	...
	_TRY(_nErr, _pfn((int)*_in0, (char *)*_rout1, (int)*_rout1Len,
	                 (int *)_rout2));
...
static __inline int
_skel_method_1(int (*_pfn)(const char *, int *, char *, int, int *),
...
__QAIC_SKEL(apps_remotectl_skel_invoke)(uint32_t _sc,
                                        remote_arg *_pra) __QAIC_SKEL_ATTRIBUTE
{
	switch(REMOTE_SCALARS_METHOD(_sc)) {
	case 0:
		return _skel_method_1(__QAIC_IMPL(apps_remotectl_open), _sc,
		                      _pra);
	case 1:
		return _skel_method(__QAIC_IMPL(apps_remotectl_close), _sc,
		                    _pra);
	}
