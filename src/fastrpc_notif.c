...
bail:
	dlerror();
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: %s FastRPC notification "
		               "worker thread exited "
		               "for domain %d (errno %s), "
		               "notif_domain_deinit started %d",
...
void
notif_thread_exit_handler(int sig)
{
	FARF(ALWAYS,
	     "Notification FastRPC worker thread exiting with signal %d\n",
	     sig);
	pthread_exit(0);
}
...
void
fastrpc_notif_domain_deinit(int domain)
{
	...
	if(me->thread) {
		FARF(ALWAYS,
		     "%s: Waiting for FastRPC notification worker "
		     "thread to join",
		     __func__);
		me->deinit_started = 1;
		...
		me->thread = 0;
		FARF(ALWAYS, "%s: Fastrpc notification worker thread joined",
		     __func__);
	}
	me->init_done = 0;
	return;
...
int
fastrpc_notif_domain_init(int domain)
{
	notif_config *me = NULL;
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = pthread_create(&me->thread, 0, notif_fastrpc_thread,
	                                 (void *)me)));
	// Register signal handler to interrupt thread, while thread is waiting
	// in kernel
...
int
fastrpc_notif_register(int domain, struct remote_rpc_notif_register *notif)
{
	int nErr = AEE_SUCCESS;
	struct fastrpc_notif *lnotif = NULL;

	// Initialize fastrpc structures, if in case this is the first call to
	// library
...
static int
fastrpc_notify_status(int domain, int session, int status)
{
	QNode *pn, *pnn;
	struct fastrpc_notif *lnotif = NULL;
	int nErr = AEE_SUCCESS;
	...
	if(!QList_IsEmpty(&notif_list.ql)) {
		QLIST_NEXTSAFE_FOR_ALL(&notif_list.ql, pn, pnn)
		{
			...
			if(lnotif && (lnotif->notif.domain == domain)) {
				lnotif->notif.notifier_fn(
				    lnotif->notif.context, domain, session,
				    status);
			}
		}
	}
...
int
get_remote_notif_response(int domain)
{
	...
	FARF(ALWAYS,
	     "%s: received status notification %u for domain %d, session %d",
	     __func__, status, dom, session);
	fastrpc_notify_status(dom, session, status);
bail:
	if(nErr && (errno != EBADF) && (nErr != AEE_EEXPIRED)) {
		FARF(ERROR,
		     "Error 0x%x: %s failed to get notification "
		     "response data errno %s",
...
int
fastrpc_exit_notif_thread(int domain)
{
	int nErr = AEE_SUCCESS, dev;
