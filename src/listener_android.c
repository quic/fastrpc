...
__QAIC_IMPL(apps_remotectl_open)(const char *name, uint32_t *handle,
                                 ...
                                 int *dlErr) __QAIC_IMPL_ATTRIBUTE
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr
	           = mod_table_open(name, handle, dlStr, dlerrorLen, dlErr)));
	VERIFY(AEE_SUCCESS
...
rpcmem_realloc(int heapid, uint32_t flags, void *buf, int oldsize, size_t size)
{
...
listener(listener_config *me)
{
	...
	if(eheap || eflags || emin) {
		FARF(RUNTIME_RPC_HIGH,
		     "listener using ion heap: %d flags: %x cache: "
		     "%lld\n",
	...
	invoke:
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s responding 0x%x for ctx 0x%x, handle 0x%x, "
		     "sc 0x%x",
		     __func__, result, ctx, handle, sc);
		FASTRPC_PUT_REF(domain);
		if(me->adsp_listener1_handle != INVALID_HANDLE) {
			nErr = __QAIC_HEADER(adsp_listener1_next2)(
			    me->adsp_listener1_handle, ctx, result, outBufs,
			    outBufsLen, &ctx, &handle, &sc, inBufs, inBufsLen,
			    &inBufsLenReq);
		} else {
			nErr = __QAIC_HEADER(adsp_listener_next2)(
			    ctx, result, outBufs, outBufsLen, &ctx, &handle,
			    &sc, inBufs, inBufsLen, &inBufsLenReq);
		}
		if(nErr) {
			...
			/* For any other error, retry once and
			 * exit if error seen again */
			if(me->adsp_listener1_handle != INVALID_HANDLE) {
				nErr = __QAIC_HEADER(adsp_listener1_next2)(
				    me->adsp_listener1_handle, ctx, nErr, 0, 0,
				    &ctx, &handle, &sc, inBufs, inBufsLen,
				    &inBufsLenReq);
			} else {
				nErr = __QAIC_HEADER(adsp_listener_next2)(
				    ctx, nErr, 0, 0, &ctx, &handle, &sc,
		...
		if(__builtin_smul_overflow(inBufsLenReq, 2, &bufs_len)) {
			FARF(ERROR,
			     "Error: %s: overflow occurred "
			     "while multiplying input buffer "
			     "size: "
			     "%d * 2 = %d for handle 0x%x, "
			     "sc 0x%x",
		...
		if(ALIGNB(bufs_len) < inBufsLen && inBufsLen > cache_size) {
			...
			if(NULL
			   == (buf = rpcmem_realloc(heapid, flags, inBufs,
			                            inBufsLen, size))) {
				result = AEE_ENORPCMEMORY;
				FARF(RUNTIME_RPC_HIGH, "rpcmem_realloc "
				                       "shrink failed");
				goto invoke;
			}
			inBufs = buf;
		...
		if(inBufsLenReq > inBufsLen) {
			...
			inBufsLen = size;
			if(me->adsp_listener1_handle != INVALID_HANDLE) {
				result = __QAIC_HEADER(
				    adsp_listener1_get_in_bufs2)(
				    me->adsp_listener1_handle, ctx, oldLen,
				    inBufs + oldLen, inBufsLen - oldLen, &req);
			} else {
				result = __QAIC_HEADER(
				    adsp_listener_get_in_bufs2)(
				    ctx, oldLen, inBufs + oldLen,
		...
		if(__builtin_smul_overflow(outBufsLen, 2, &bufs_len)) {
			FARF(ERROR,
			     "%s: Overflow occured while "
			     "multiplying output buffer "
			     "size: %d * 2 "
			     "= %d",
		...
		if(ALIGNB(bufs_len) < outBufsCapacity
		   && outBufsCapacity > cache_size) {
			void *buf;
			int size = ALIGNB(bufs_len);
			if(NULL
			   == (buf = rpcmem_realloc(heapid, flags, outBufs,
			                            outBufsCapacity, size))) {
				result = AEE_ENORPCMEMORY;
				FARF(RUNTIME_RPC_HIGH, "listener "
				                       "rpcmem_realloc "
				                       "shrink failed");
				goto invoke;
			}
		...
		if(result && is_process_exiting(domain))
			result = AEE_EBADSTATE; // override result as
			                        // process is exiting
	} while(1);
bail:
	...
	if(nErr != AEE_SUCCESS) {
		if(!is_process_exiting(domain)) {
			FARF(ERROR,
			     "Error 0x%x: %s response with "
			     "result 0x%x for ctx 0x%x, "
			     "handle 0x%x, "
			     "sc 0x%x failed : listener "
			     "thread exited (errno %s)",
			     nErr, __func__, result, ctx, handle, sc,
			     strerror(errno));
		}
	}
	...
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x : Writing to listener "
		               "event_fd %d failed (errno %s)",
		               nErr, me->eventfd, strerror(errno));
	}
	dlerror();
}
...
static void *
listener_start_thread(void *arg)
{
	int nErr = AEE_SUCCESS;
	...
	if((adsp_listener1_handle = get_adsp_listener1_handle(domain))
	   != INVALID_HANDLE) {
		nErr = __QAIC_HEADER(adsp_listener1_init2)(
		    adsp_listener1_handle);
		if((nErr == DSP_AEE_EOFFSET + AEE_ERPC)
		   || nErr == DSP_AEE_EOFFSET + AEE_ENOSUCHMOD) {
			FARF(ERROR,
			     "Error 0x%x: %s domains support "
			     "not available in listener",
			     nErr, __func__);
	...
	if(me->update_requested) {
		/* Update parameters on DSP and signal main thread to
		 * proceed */
...
listener_android_init(void)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = mod_table_register_const_handle(
		       0, "apps_remotectl", apps_remotectl_skel_invoke)));
	VERIFY(AEE_SUCCESS
	       == (nErr = mod_table_register_static("apps_std",
	                                            apps_std_skel_invoke)));
	VERIFY(AEE_SUCCESS
	       == (nErr = mod_table_register_static("apps_mem",
	                                            apps_mem_skel_invoke)));
	VERIFY(AEE_SUCCESS
	       == (nErr = mod_table_register_static(
		       "adspmsgd_apps", adspmsgd_apps_skel_invoke)));
bail:
	if(nErr != AEE_SUCCESS) {
		listener_android_deinit();
		VERIFY_EPRINTF(
		    "Error %x: fastrpc listener initialization error", nErr);
	}
	return nErr;
}

void
listener_android_domain_deinit(int domain)
{
...
int
listener_android_domain_init(int domain, int update_requested, sem_t *r_sem)
{
	listener_config *me = NULL;
	int nErr = AEE_SUCCESS;
	...
	if(me->update_requested) {
		/*
		 * Semaphore initialized to 0. If main thread reaches
		 * wait first, then it will wait for listener to
		 * increment semaphore to 1. If listener posted
		 * semaphore first, then this wait will decrement
...
int
close_reverse_handle(remote_handle64 h, char *dlerr, int dlerrorLen,
                     int *dlErr)
{
...
int
listener_android_geteventfd(int domain, int *fd)
{
	listener_config *me = NULL;
	int nErr = 0;
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error %x: listener android getevent "
