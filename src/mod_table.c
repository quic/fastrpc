...
static __inline HMODULE
DLOPEN(LPCSTR name, int mode)
{
	UNREFERENCED_PARAMETER(mode);
	return LoadLibraryExA(name, 0, 0);
}

static __inline FARPROC
DLSYM(LPVOID handle, LPCSTR name)
{
	return GetProcAddress((HMODULE)handle, name);
}

static __inline int
DLCLOSE(LPVOID handle)
{
...
static int
static_mod_table_ctor(struct static_mod_table *me)
{
	if(me->bInit == 0) {
...
static void
static_mod_table_dtor_imp(struct static_mod_table *me)
{
...
static int
open_mod_table_ctor_imp(void *ctx, void *data)
{
...
static void
open_mod_table_dtor_imp(void *data)
{
	struct open_mod_table *me = (struct open_mod_table *)data;
	...
	if(me->openMods) {
		...
		HASH_ITER(hh, me->openMods, dm, ftmp)
		{
			...
			FARF(ALWAYS,
			     "%s: closed reverse module %s with "
			     "handle 0x%x",
			     __func__, dm->uri, (uint32_t)dm->key);
			dm->key = 0;
		}
		RW_MUTEX_UNLOCK_WRITE(me->mut);
	}
...
static int
open_mod_table_open_static_override(struct open_mod_table *me, const char *uri,
                                    remote_handle *handle)
{
	FARF(RUNTIME_RPC_HIGH, "open_mod_table_open_static_override");
	return open_mod_table_open_from_static(
	    me, &me->smt->staticModOverrides, uri, handle);
}

static int
open_mod_table_open_static(struct open_mod_table *me, const char *uri,
                           remote_handle *handle)
{
	FARF(RUNTIME_RPC_HIGH, "open_mod_table_open_static");
	return open_mod_table_open_from_static(me, &me->smt->staticMods, uri,
	                                       handle);
}

static int
static_mod_add(struct static_mod_table *me, struct static_mod **tbl,
...
static int
static_mod_table_register_static_override(struct static_mod_table *me,
                                          const char *uri,
                                          int (*pfn)(uint32_t sc,
                                                     remote_arg *pra))
{
...
static int
static_mod_table_register_static_override1(
...
static int
static_mod_table_register_static1(struct static_mod_table *me, const char *uri,
    ...
    int (*handle_invoke)(remote_handle64, uint32_t sc, remote_arg *pra))
{
	...
	VERIFYC(NULL
	            != (dm = ((struct const_mod *)calloc(
			    1, sizeof(struct open_mod) + len))),
	        AEE_ENOMEMORY);
	dm->key = local;
	dm->invoke = invoke;
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error %x: failed to register const "
		               "handle in modtable\n",
		               nErr);
		if(dm) {
			free(dm);
...
static int
open_mod_handle_open(struct open_mod *mod, const char *name,
                     remote_handle64 *ph)
{
	int nErr = AEE_SUCCESS;
	remote_arg args[3] = { 0 };
	...
	args[1].buf.nLen = len;
	nErr = mod->handle_invoke(0, REMOTE_SCALARS_MAKEX(0, 0, 2, 0, 0, 1),
	                          args);
	if(!nErr) {
		*ph = args[2].h64;
	}
...
static int
open_mod_handle_close(struct open_mod *mod, remote_handle64 h)
{
	int nErr;
	remote_arg args[1] = { 0 };
	args[0].h64 = h;
	FARF(RUNTIME_RPC_HIGH, "releasing %x", h);
	nErr = mod->handle_invoke(0, REMOTE_SCALARS_MAKEX(0, 1, 0, 0, 1, 0),
	                          args);
	return nErr;
}

static int
notqmark(struct sbuf *buf)
{
	return sbuf_notchar(buf, '?');
}
static int
notandoreq(struct sbuf *buf)
{
	return sbuf_notchars(buf, "&=");
}
static int
notand(struct sbuf *buf)
{
	return sbuf_notchar(buf, '&');
}

static int
parse_uri(const char *uri, int urilen, struct parsed_uri *out)
{
	...
	FARF(RUNTIME_RPC_HIGH, "file:%.*s %d", out->filelen, out->file,
	     out->filelen);
	...
	FARF(RUNTIME_RPC_HIGH, "sym:%.*s %d", out->symlen, out->sym,
	     out->symlen);
...
bail:
	if(out->filelen) {
		FARF(RUNTIME_RPC_HIGH, "parse_uri file: %.*s", out->filelen,
		     out->file);
	}
	if(out->symlen) {
		FARF(RUNTIME_RPC_HIGH, "parse_uri sym: %.*s", out->symlen,
		     out->sym);
	}
	if(out->verlen) {
		FARF(RUNTIME_RPC_HIGH, "parse_uri version: %.*s", out->verlen,
		     out->ver);
	}
	FARF(RUNTIME_RPC_HIGH, "parse_uri done: %s %d err:%x", uri, urilen,
	     nErr);
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error %x: parseuri failed for uri "
		               "%s, urilen %d\n",
		               nErr, uri, urilen);
	}
	return nErr;
...
/*
 * Iterate through the list of reverse handles and search for the next
 * available handle. 'key' is only set when handle have been taken and is set
 * to 0 when handle is free. Returns 0 if it found a free handle, otherwise
 * return -1 if it fails.
 */
static inline int
next_available_rev_handle(uint32_t *handle_idx)
{
	...
	if(nErr)
		FARF(ERROR,
		     "Error 0x%x: %s: max number of reverse RPC handles (%u) "
		     "open already",
...
uint32_t
is_reverse_handle_opened(struct open_mod_table *me, remote_handle *handle,
                         const char *uri)
{
	int ii = 0;
	uint32_t keyfound = 0;
	...
	for(ii = 0; ii < MAX_REV_HANDLES; ++ii) {
		...
		if(dmOld->key != 0) {
			if(!strncmp(dmOld->uri, uri,
			            MAX(strlen(dmOld->uri), strlen(uri)))) {
				keyfound = 1;
				break;
			}
		}
	...
	if(keyfound) {
		*handle = dmOld->key;
		dmOld->refs++;
		FARF(ALWAYS,
		     "%s: reverse module %s already found with "
		     "handle 0x%x (idx %u) refs %d",
...
static int
open_mod_table_open_dynamic(struct open_mod_table *me, const char *uri,
                            remote_handle *handle, char *dlStr, int dlerrorLen,
                            int *pdlErr)
{
	...
	VERIFYC(0 == (nErr = next_available_rev_handle(&handle_idx)),
	        AEE_EINVHANDLE);
	...
	FARF(RUNTIME_RPC_HIGH, "vals %d %d %d", dm->vals.filelen,
	     dm->vals.symlen, dm->vals.verlen);
	if(dm->vals.filelen) {
		int rv = snprintf(tmp, tmplen, "%.*s", dm->vals.filelen,
		                  dm->vals.file);
	...
	if((dlErr = dm->dlhandle == 0 ? AEE_EINVHANDLE : 0)
	   && errno == ENOENT) {
		int str_len, ii, rv;
		...
		for(ii = str_len - 1; ii >= 0; ii--) {
			...
			// Find index of last character before
			// the extension ".so" starts
			if(tmp[ii] == '.') {
				tmp[ii] = '\0';
				break;
			}
		...
		if(dlErr == 0)
			FARF(ALWAYS, "system library %s successfully loaded",
			     tmp);
	}
	...
	if(dm->vals.symlen) {
		int rv = snprintf(tmp, tmplen, "%.*s", dm->vals.symlen,
		                  dm->vals.sym);
		VERIFYC((rv > 0) && (tmplen >= rv), AEE_EBADPARM);
	} else {
	...
	FARF(RUNTIME_RPC_HIGH, "dlsym returned %p %p", dm->invoke,
	     dm->handle_invoke);
	VERIFYC(
	    !(dlErr = dm->invoke || dm->handle_invoke ? 0 : AEE_ENOSUCHSYMBOL),
	    AEE_ENOSUCHSYMBOL);
	...
	if(dm->handle_invoke) {
		VERIFY(AEE_SUCCESS
		       == (nErr = open_mod_handle_open(dm, uri, &dm->h64)));
	}
...
bail:
	...
	if(nErr == AEE_SUCCESS) {
		FARF(ALWAYS,
		     "%s: dynamic reverse module %s opened with "
		     "handle 0x%x (idx %u)",
		...
		if(dlErr) {
			...
			FARF(RUNTIME_RPC_HIGH, "dlerror:0x%x:%s", dlErr,
			     dlerr == 0 ? "" : dlerr);
			nErr = 0;
		}
		if(pdlErr) {
		...
		VERIFY_EPRINTF("Error 0x%x: %s failed for %s, dlerr 0x%x",
		               nErr, __func__, uri, dlErr);
	}

	FARF(RUNTIME_RPC_HIGH,
	     "done open_mod_table_open_dynamic for %s rv %x handle: %p %x",
	     uri, nErr, *handle, dlErr);
	if(tmp) {
		free(tmp);
		tmp = NULL;
	}
...
static int
open_mod_table_open_from_static(struct open_mod_table *me,
                                struct static_mod **tbl, const char *uri,
                                remote_handle *handle)
{
	int nErr = AEE_SUCCESS;
	struct static_mod *sm = 0;
	...
	VERIFYC((REV_HANDLE_SIZE >= sizeof(struct open_mod) + sz),
	        AEE_ENOMEMORY);
	...
	if(sm == 0) {
		...
		FARF(RUNTIME_RPC_HIGH, "file %.*s %d", dm->vals.filelen,
		     dm->vals.file, dm->vals.filelen);
		FARF(RUNTIME_RPC_HIGH, "sym %.*s %d", dm->vals.symlen,
		     dm->vals.sym, dm->vals.symlen);
		FARF(RUNTIME_RPC_HIGH, "version %.*s %d", dm->vals.verlen,
		     dm->vals.ver, dm->vals.verlen);
		if(dm->vals.verlen) {
			int rv = snprintf(
			    dm->uri, sz, "file:///%.*s?%.*s&_modver=%.*s",
			    dm->vals.filelen, dm->vals.file, dm->vals.symlen,
			    dm->vals.sym, dm->vals.verlen, dm->vals.ver);
			VERIFYC((rv > 0) && (sz >= rv), AEE_EBADPARM);
		} else {
			int rv = snprintf(dm->uri, sz, "file://%.*s?%.*s",
	...
	VERIFYM(0 != sm, AEE_ENOSUCHMOD,
	        "Error %x: static mod is not initialized\n", nErr);
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: %s failed for %s", nErr, __func__,
		               uri);
	} else {
		FARF(RUNTIME_RPC_HIGH,
		     "%s: reverse module %s opened with handle 0x%x "
		     "(idx %u)",
		     __func__, uri, *handle, handle_idx);
	}
	if(nErr && dm) {
...
static int
open_mod_table_open(struct open_mod_table *me, const char *uri,
                    remote_handle *handle, char *dlerr, int dlerrorLen,
                    int *pdlErr)
{
	int nErr = AEE_SUCCESS, dlErr = 0;
	if(pdlErr) {
	...
	if(0 != open_mod_table_open_static_override(me, uri, handle)) {
		VERIFY(AEE_SUCCESS
		       == (nErr = open_mod_table_open_dynamic(
			       me, uri, handle, dlerr, dlerrorLen, &dlErr)));
		if(dlErr != 0) {
			FARF(RUNTIME_RPC_HIGH, "dynammic open failed, trying "
			                       "static");
...
static void
open_mod_close(struct open_mod_table *me, struct open_mod *dm)
{
	RW_MUTEX_LOCK_WRITE(me->mut);
	FARF(RUNTIME_RPC_HIGH, "%s: uri:%s, refs:%d", __func__, dm->uri,
	     dm->refs);
	dm->refs--;
	if(dm->refs == 0) {
		HASH_DEL(me->openMods, dm);
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s : module %s has pending invokes ref count %d",
	...
	if(dm) {
		...
		FARF(ALWAYS, "%s: closed reverse module %s with handle 0x%x",
		     __func__, dm->uri, (uint32_t)dm->key);
		dm->key = 0;
		dm = NULL;
	}
...
static int
open_mod_table_close(struct open_mod_table *me, remote_handle64 handle,
                     char *errStr, int errStrLen, int *pdlErr)
{
	int nErr = AEE_SUCCESS;
	...
	if(dm) {
		...
		if(dm->refs == 0) {
			del = dm;
			FARF(RUNTIME_RPC_HIGH, "deleting %s %p %d", del->uri,
			     del, dm->refs);
			HASH_DEL(me->openMods, dm);
			del->key = 0;
		} else {
			FARF(RUNTIME_RPC_HIGH,
			     "%s: pending ref: dm->refs %d, "
			     "for uri: %s",
	...
	if(del) {
		...
		FARF(ALWAYS, "%s: closed reverse module %s with handle 0x%x",
		     __func__, del->uri, (uint32_t)handle);
		del = NULL;
	}
bail:
	if(locked) {
	...
	if(dlErr) {
		...
		VERIFY_EPRINTF(
		    "Error %x: open modtable close failed. dlerr %s\n", nErr,
		    error);
	}
	if(pdlErr) {
		*pdlErr = dlErr;
...
static struct open_mod *
open_mod_table_get_open(struct open_mod_table *me, remote_handle handle)
{
	...
	if(0 != om) {
		om->refs++;
		FARF(RUNTIME_RPC_HIGH, "%s: module %s, increament refs %d",
		     __func__, om->uri, om->refs);
	}
	RW_MUTEX_UNLOCK_WRITE(me->mut);
	return om;
}
static struct const_mod *
open_mod_table_get_const(struct open_mod_table *me, remote_handle handle)
{
...
static int
open_mod_table_handle_invoke(struct open_mod_table *me, remote_handle handle,
                             uint32_t sc, remote_arg *pra)
{
	...
	if(cm) {
		...
		VERIFYC(0 != (om = open_mod_table_get_open(me, handle)),
		        AEE_ENOSUCHMOD);
		invoke = om->invoke;
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for handle:0x%x, sc:0x%x",
		     nErr, __func__, handle, sc);
	}
	FASTRPC_ATRACE_END();
	return nErr;
}
 ...
 */
int
mod_table_register_static_override(const char *name,
                                   int (*pfn)(uint32_t sc, remote_arg *pra))
{
	if(0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_static_override(
		    &static_mod_table_obj, name, pfn);
	}
	return AEE_EUNKNOWN;
}

int
mod_table_register_static_override1(const char *name,
                                    int (*pfn)(remote_handle64, uint32_t sc,
                                               remote_arg *pra))
{
	if(0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_static_override1(
		    &static_mod_table_obj, name, pfn);
	}
	return AEE_EUNKNOWN;
}
 ...
 */
int
mod_table_register_static(const char *name,
                          int (*pfn)(uint32_t sc, remote_arg *pra))
{
	if(0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_static(&static_mod_table_obj,
		                                        name, pfn);
	}
	return AEE_EUNKNOWN;
}
...
int
mod_table_register_static1(const char *name,
                           int (*pfn)(remote_handle64, uint32_t sc,
                                      remote_arg *pra))
{
	if(0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_static1(&static_mod_table_obj,
		                                         name, pfn);
	}
	return AEE_EUNKNOWN;
}
 ...
 */
int
mod_table_open(const char *uri, remote_handle *handle, char *dlerr,
               int dlerrorLen, int *pdlErr)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = HAP_pls_add_lookup(
		       (uintptr_t)open_mod_table_ctor_imp, 0, sizeof(*pomt),
		       open_mod_table_ctor_imp, (void *)&static_mod_table_obj,
		       open_mod_table_dtor_imp, (void **)&pomt)));
	VERIFY(AEE_SUCCESS
	       == (nErr = open_mod_table_open(pomt, uri, handle, dlerr,
	                                      dlerrorLen, pdlErr)));
bail:
 ...
 */
int
mod_table_invoke(remote_handle handle, uint32_t sc, remote_arg *pra)
{
	int nErr = AEE_SUCCESS;
	struct open_mod_table *pomt = 0;
	VERIFY(AEE_SUCCESS
 ...
 */
int
mod_table_close(remote_handle handle, char *errStr, int errStrLen, int *pdlErr)
{
	int nErr = AEE_SUCCESS;
	struct open_mod_table *pomt = 0;

	FASTRPC_ATRACE_BEGIN_L("%s called with handle 0x%x", __func__,
	                       (int)handle);
	VERIFY(AEE_SUCCESS
 ...
 */
int
mod_table_register_const_handle(remote_handle remote, const char *uri,
                                int (*pfn)(uint32_t sc, remote_arg *pra))
{
	if(0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_const_handle(
		    &static_mod_table_obj, remote, 0, uri, pfn, 0);
	}
	return AEE_EUNKNOWN;
}
int
mod_table_register_const_handle1(remote_handle remote, remote_handle64 local,
                                 const char *uri,
                                 int (*pfn)(remote_handle64, uint32_t sc,
                                            remote_arg *pra))
{
	if(0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_const_handle(
		    &static_mod_table_obj, remote, local, uri, 0, pfn);
	}
	return AEE_EUNKNOWN;
}
