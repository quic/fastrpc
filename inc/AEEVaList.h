...
#define AEEVALIST_H
    ...
    (defined(__ARMCC_VERSION) || (defined(__GNUC__) && defined(__arm__)))

#if (defined(__ARMCC_VERSION) && __ARMCC_VERSION >= 200000 &&                  \
     !defined(__APCS_ADSABI)) ||                                               \
...
#define AEEVA_START(va, v)                                                     \
  ((va) = (char *)&(v) + __AEEVA_ARGSIZE(v) + __AEEVA_ATPCS)
#define AEEVA_ARG(va, v, t)                                                    \
  ((void)((va) =                                                               \
              __AEEVa_Arg(va, &v, sizeof(v), sizeof(t), __AEEVA_ARGALIGN(t))))
#define AEEVA_END(va) ((va) = (AEEVaList)0)
#define AEEVA_COPY(dest, src) ((void)((dest) = (src)))

#else /* !defined(__clang__) && (defined(__ARMCC_VERSION) ||                   \
