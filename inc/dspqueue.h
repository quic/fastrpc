...
enum dspqueue_packet_flags {
  ...
  DSPQUEUE_PACKET_FLAG_DRIVER_READY =
      0x0008, /**< Packet is ready for driver consumption. Currently unused. */
...
enum dspqueue_buffer_flags {
  ...
  DSPQUEUE_BUFFER_FLAG_DEREF =
      0x00000008, /**< Remove a reference from a previously mapped buffer */
...
enum dspqueue_stat {
  ...
  DSPQUEUE_STAT_EARLY_WAKEUP_WAIT_TIME, /**< Total accumulated early wakeup wait
                                           time in microseconds */
  DSPQUEUE_STAT_EARLY_WAKEUP_MISSES, /**< Number accumulated of packets missed
                                        in the early wakeup loop */
  DSPQUEUE_STAT_SIGNALING_PERF       /**< Signaling performance; 0 or undefined
                                        indicates the first implementation,
                                          DSPQUEUE_SIGNALING_PERF_REDUCED_SIGNALING or
                                        higher a version if reduced       signaling
                                        for       polling clients. */
};
...
struct dspqueue_buffer {
  ...
  uint32_t size;   /**< Buffer size in bytes. The client can set this field
                        ...
                        buffer as mapped. */
  uint32_t offset; /**< Offset within the buffer in bytes as allocated and
                      mapped. The virtual address #ptr includes the offset */
  uint32_t flags;  /**< Buffer flags, see enum #dspqueue_buffer_flags */
  union {
    void *ptr; /**< Buffer virtual address; NULL if not mapped in the local
                  context */
    uint64_t address;
  };
};
 ...
 *
 * @param [in] domain DSP to communicate with (CDSP_DOMAIN_ID in remote.h for
 * cDSP)
 * @param [in] flags Queue creation flags
 * @param [in] req_queue_size Total request queue memory size in bytes; use 0
 * for system default
 * @param [in] resp_queue_size Total response queue memory size in bytes; use 0
 * for system default
 * @param [in] packet_callback Callback function called when there are new
 * packets to read. The call will be done in a different thread's context. NULL
 * to disable the callback. Clients cannot use blocking read calls if a packet
 * callback has been set.
 * @param [in] error_callback Callback function called on unrecoverable errors.
 * NULL to disable.
 ...
 *
 * @return 0 on success, error code on failure.
 *         - AEE_ERPC: Internal RPC error, e.g. The queue is open on the DSP
 * when attempting to close it on the host CPU
 */
 ...
 *         - AEE_EQURTTHREADCREATE: Unable to create callback thread; the system
 * may have reached its thread limit.
 ...
 *			- AEE_EBADPARM     : Bad parameters, e.g. invalid
 *context
 ...
 *         - AEE_EBADPARM: Bad parameters, e.g. buffers is NULL when num_buffers
 * > 0
 *         - AEE_ENOSUCHMAP: Attempt to refer to an unmapped buffer. Buffers
 * must be mapped to the DSP with fastrpc_mmap() before they can be used in
 * queue packets.
 ...
 * @param [in] max_buffers The maximum number of buffer references that can fit
 ...
 * @param [in] max_message_length Maximum message length that can fit in the
 * "message" parameter
 * @param [out] message_length Message length in bytes
 * @param [out] message Packet message
 *
 * @return 0 on success, error code on failure.
 *         - AEE_ENOSUCHMAP: The packet refers to an unmapped buffer. Buffers
 * must be mapped to the DSP with fastrpc_mmap() before they can be used in
 * queue packets.
 *         - AEE_EWOULDBLOCK: The queue is empty; try again later
 *         - AEE_EBADITEM: The queue contains a corrupted packet. Internal
 * error.
 */
 ...
 *                     the real message packet should be available. Zero if not
 * known. The recipient can use this information to determine how to wait for
 * the packet.
