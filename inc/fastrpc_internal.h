...
// URI string to choose a particular session. Session ID needs to be appended to
// Session uri.
...
struct fastrpc_proc_sess_info {
  ...
  uint32_t
      sharedcb; /* Unused, Session can share context bank with other sessions */
};
 ...
 *more than one process created on same DSP kmem_support - Stores whether kernel
 *can allocate memory for signed process dev - file descriptor returned by
 *open(<fastrpc_device_node>) cphandle, msghandle, lsitenerhandle,
 ...
 *cap_info - stores information needed for DSP process. these are used by remote
...
struct handle_list {
  ...
  int trace_marker_fd;
  uint64_t jobid;
  /* Capability flag to check if mapping DMA handle through reverse RPC is
   * supported */
...
#define PRINT_WARN_USE_DOMAINS()                                               \
  VERIFY_WPRINTF("Warning: %s: Non-domain usage of FastRPC will be "           \
                 "deprecated, use domains to offload to DSP using FastRPC",    \
                 __func__)
#else
 ...
 *
 **/
int fastrpc_update_module_list(uint32_t req, int domain, remote_handle64 handle,
...
int ioctl_init(int dev, uint32_t flags, int attr, byte *shell, int shelllen,
               int shellfd, char *initmem, int initmemlen, int initmemfd,
               int tessiglen);
