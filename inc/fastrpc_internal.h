...
// URI string to choose a particular session. Session ID needs to be appended
// to Session uri.
 ...
 * Two extra characters for session ID > 9 and domain name more than 4
 * characters.
 */
#define FASTRPC_URI_BUF_LEN                                                   \
	(strlen(CDSP1_DOMAIN) + strlen(FASTRPC_SESSION1_URI) + 2)
...
#define GET_SESSION_ID_FROM_DOMAIN_ID(domain_id)                              \
	((int)(domain_id / NUM_DOMAINS))

/* From actual domain ID (0-3) and session ID, get effective domain ID */
#define GET_EFFECTIVE_DOMAIN_ID(domain, session)                              \
	(domain + (NUM_DOMAINS * session))

/* From effective domain ID, get actual domain ID */
#define GET_DOMAIN_FROM_EFFEC_DOMAIN_ID(effec_dom_id)                         \
	(effec_dom_id & DOMAIN_ID_MASK)
...
#define IS_VALID_EFFECTIVE_DOMAIN_ID(domain)                                  \
	((domain >= 0) && (domain < NUM_DOMAINS_EXTEND))

/* Check if given effective domain ID is in extended range */
#define IS_EXTENDED_DOMAIN_ID(domain)                                         \
	((domain >= NUM_DOMAINS) && (domain < NUM_DOMAINS_EXTEND))
...
struct fastrpc_proc_sess_info {
	uint32_t
	    domain_id; /* Set the remote subsystem, Domain ID of the session */
	uint32_t
	    session_id;    /* Unused, Set the Session ID on remote subsystem */
	uint32_t pd_type;  /* Set the process type on remote subsystem */
	uint32_t sharedcb; /* Unused, Session can share context bank with other
	                      sessions */
};
 ...
 *    dsppd - Type of the process that should be created on DSP (specific
 *domain) dsppdname - Name of the process sessionname - Name of the session, if
 *more than one process created on same DSP kmem_support - Stores whether
 *kernel can allocate memory for signed process dev - file descriptor returned
 *by open(<fastrpc_device_node>) cphandle, msghandle, lsitenerhandle,
 *remotectlhandle, adspperfhandle - static handles created for the stub files
 *loaded by fastRPC. procattrs - Attributes for the DSP Process. Stores
 *information like debug process, trace enablement, etc. qos, th_params,
 *cap_info - stores information needed for DSP process. these are used by
 *remote calls whenever needed.
 **/
struct handle_list {
	...
	int trace_marker_fd;
	uint64_t jobid;
	/* Capability flag to check if mapping DMA handle through reverse RPC
	 * is supported */
	int dma_handle_reverse_rpc_map_capability;
	/* Mutex to synchronize ASync init and deinit */
	pthread_mutex_t async_init_deinit_mut;
	uint32_t
	    pd_initmem_size; /** Initial memory allocated for remote userPD */
 ...
 * @get_path : get_path will be updated with the path stored in DSP_SEARCH_PATH
 *locally.
 **/
 ...
 * @param[in] attributeID: One of the DSP/kernel attributes from enum
 *fastrpc_internal_attributes.
 * @param[out] capability: Result of the DSP/kernel capability.
 * @return Integer value. Zero for success and non-zero for failure.
 **/
int fastrpc_get_cap(uint32_t domain, uint32_t attributeID,
                    uint32_t *capability);
...
/**
 * @brief Notify the FastRPC QoS logic of activity outside of the invoke code
 *path that should still be considered in QoS timeout calculations. Note that
 *the function will silently fail on errors such as the domain not having a
 *valid QoS mode.
 *
 * @param[in] domain DSP domain
 **/
...
static __inline int
convert_kernel_to_user_error(int nErr, int err_no)
{
	if(!(nErr == AEE_EUNKNOWN && err_no
	     && (err_no >= MIN_KERNEL_ERRNO && err_no <= MAX_KERNEL_ERRNO))) {
		return nErr;
	}

	switch(err_no) {
	case EIO:           /* EIO 5 I/O error */
	case ETOOMANYREFS:  /* ETOOMANYREFS 109 Too many references:
	                       cannot splice */
	case EADDRNOTAVAIL: /* EADDRNOTAVAIL 99 Cannot assign requested
	                       address */
	...
	case EOVERFLOW: /* EOVERFLOW 75 Value too large for defined
	                   data type */
	...
	case ESHUTDOWN: /* ESHUTDOWN 108 Cannot send after transport
	                   endpoint shutdown */
		nErr = AEE_EEXPIRED;
		break;
	case ENOTCONN:     /* ENOTCONN 107 Transport endpoint is not
	                      connected */
	...
	case EPIPE:      /* EPIPE 32 Broken pipe */
		nErr = AEE_ECONNRESET;
		break;
	case EPROTONOSUPPORT: /* EPROTONOSUPPORT 93 Protocol not
	                         supported */
...
#define PRINT_WARN_USE_DOMAINS()                                              \
	VERIFY_WPRINTF(                                                       \
	    "Warning: %s: Non-domain usage of FastRPC will be deprecated, "   \
	    "use domains to offload to DSP using FastRPC",                    \
	    __func__)
#else
...
/**
 * @brief APIs to return the newly allocated handles for all the static modules
 *loaded
 **/
 ...
 * @domain: domain id
 * Prepend and dequeue operations can be performed on reverse and non-domain
 *handle list
 * @returns: 0 on success, valid non-zero error code on failure
 *
 **/
int fastrpc_update_module_list(uint32_t req, int domain,
                               remote_handle64 handle, remote_handle64 *local,
                               const char *name);

/**
 * @brief functions to wrap ioctl syscalls for downstream and upstream kernel
 **/
int ioctl_init(int dev, uint32_t flags, int attr, unsigned char *shell,
               int shelllen, int shellfd, char *initmem, int initmemlen,
               int initmemfd, int tessiglen);
int ioctl_invoke(int dev, int req, remote_handle handle, uint32_t sc,
                 void *pra, int *fds, unsigned int *attrs, void *job,
                 unsigned int *crc, uint64_t *perf_kernel, uint64_t *perf_dsp);
...
int ioctl_mmap(int dev, int req, uint32_t flags, int attr, int fd, int offset,
               size_t len, uintptr_t vaddrin, uint64_t *vaddr_out);
