...
/**
 *  @file rpcmem.h
 *  @brief APIs used to manage memory allocated by the application processor
 * and shared with the DSP.
 */
 ...
 * remote process of all current and new FastRPC sessions. In case of failure
 * to map, the FastRPC library ignores the error and continues to open the
 * session without pre-mapping the buffer. In case of success, buffers
 * allocated with this flag will be pre-mapped to reduce the latency of
 * upcoming FastRPC calls. This flag is recommended only for buffers that are
 * used with latency-critical FastRPC methods. Pre-mapped buffers will be
 * unmapped during either buffer free or session close.
 */
...
enum rpc_heap_ids {
	 ...
	 *  * Recommended for all use cases that do not require using a
	 * specific heap
 ...
 *
 * This API is mandatory on pre-Lahaina targets IF the client has
 * linked to the rpcmem.a static library. If the client has only linked
 * libadsprpc.so, libcdsprpc.so, or libsdsprpc.so, then the rpcmem_init
 * call is not required on any target and other rpcmem APIs such as
 * rpcmem_alloc can be called directly.
 *
 ...
 *
 * Only call this function once when the RPCMEM Library is no longer
 * required.
 *
 * This API is mandatory on pre-Lahaina targets IF the client has
 * linked to the rpcmem.a static library. If the client has only linked
 * libadsprpc.so, libcdsprpc.so, or libsdsprpc.so, then the
 * rpcmem_deinit call is not required on any target.
 *
 * NOTE: This function is not thread safe.
 */
...
/**
 * Allocate a zero-copy buffer for size upto 2 GB with the FastRPC
 * framework. Buffers larger than 2 GB must be allocated with
 * rpcmem_alloc2
 ...
 * @return            Pointer to the buffer on success; NULL on
 * failure.
 *
 * Examples:
 *
