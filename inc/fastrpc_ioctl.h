...
#define FASTRPC_IOCTL_INIT_CREATE_STATIC                                      \
	_IOWR('R', 9, struct fastrpc_ioctl_init_create_static)
...
#define FASTRPC_IOCTL_GET_DSP_INFO                                            \
	_IOWR('R', 13, struct fastrpc_ioctl_capability)
...
#define set_args(i, pra, len, filedesc, attrs)                                \
	args[i].ptr = (uint64_t)pra;                                          \
	args[i].length = len;                                                 \
	args[i].fd = filedesc;                                                \
	args[i].attr = attrs;
...
#define FASTRPC_INVOKE2_STATUS_NOTIF                                          \
	2 // TODO: Temporary change (Bharath to fix)
#define FASTRPC_INVOKE2_KERNEL_OPTIMIZATIONS                                  \
	1 // TODO: Temporary change (Bharath to fix)
