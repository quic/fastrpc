 ...
 */
static __inline void sbuf_init(struct sbuf *buf, int offset, void *data,
                               int dataLen) {
 ...
 *             from the buf's current position.  Only the valid portion of data
 * will be written.
...
static __inline void sbuf_write(struct sbuf *buf, void *psrc, int srcLen) {
  ...
  if (buf->bufCur + srcLen > buf->buf) {
    ...
    if (writeLen > 0) {
      std_memsmove((void *)buf->bufCur, buf->bufEnd - buf->bufCur, (void *)src,
                   writeLen);
    }
  }
