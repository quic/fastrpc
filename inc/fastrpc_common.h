...
#define IS_REVERSE_RPC_CALL(handle, sc)                                        \
  ((handle == FASTRPC_STATIC_HANDLE_LISTENER) && (sc == REVERSE_RPC_SCALAR))
...
typedef enum {
  ...
  FASTRPC_DEBUG_PDDUMP = 7, // to enable pd dump debug data collection on rooted
                            // device for signed/unsigned pd
  FASTRPC_PROCESS_ATTRS_PERSISTENT = 8, // to set proc attr as persistent
  FASTRPC_BUILD_TYPE = 9 // Fetch build type of firmware image. It gives the
                         // details if its debug or prod build
} fastrpc_properties;
...
enum fastrpc_internal_attributes {
  ...
  FASTRPC_MAX_ATTRIBUTES =
      DSPSIGNAL_DRIVER_SUPPORT + 1, /**<  Max DSP/Kernel attributes supported */
};
 ...
 *
 */
int fastrpc_get_property_string(fastrpc_properties UserPropertyKey, char *value,
                                char *defValue);
