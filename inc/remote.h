 ...
 * pra, contains the arguments in the following order, inbufs, outbufs,
 * inhandles, outhandles. implementors should ignore and pass values asis that
 * the transport doesn't understand.
 */
 ...
 * @param datalen, length of data
 * @retval, 0 on success
 * remote_session_control with FASTRPC_REMOTE_PROCESS_KILL req ID, possible
 * error codes are AEE_ENOSUCH, AEE_EBADPARM, AEE_EINVALIDDOMAIN. Other than
 * this errors codes treated as retuned from fastRPC framework.
 */
 ...
 *
 * desc, descriptor contains type of asyncjob. context and call back function(if
 * any) see retrive macro's on dwScalars format pra, contains the arguments in
 * the following order, inbufs, outbufs, inhandles, outhandles. all outbufs need
 * to be either allocated using rpcmem_alloc or registered ION buffers using
 * register_buf implementors should ignore and pass values as is that the
 * transport doesn't understand.
 */
 ...
 *                    timeout > 0, waits for specified time and then returns
 * with status/result timeout < 0. waits indefinetely until job completes
 ...
 * @param size, size to unmap.  Unmapping a range partially may  not be
 * supported.
 ...
 *          Default domain is selected based on library lib(a/m/s/c)dsprpc.so
 * library linked to application.
 ...
 *
 * @domain: DSP domain ID. Use -1 for using default domain. Get domain from
 * multi-domain handle if required.
...
/**
 * fastrpc_mmap
 * Creates a mapping on remote process for an ION buffer with file descriptor.
 * New fastrpc session will be opened if not already opened for the domain.
 *
 ...
 * @param flags, controls mapping functionality on DSP. Refer fastrpc_map_flags
 * enum definition for more information.
 *
 * @return, 0 on success, error code on failure.
 *          AEE_EALREADY Buffer already mapped. Multiple mappings for the same
 * buffer are not supported. AEE_EBADPARM Bad parameters AEE_EFAILED Failed to
 * map buffer AEE_ENOMEMORY Out of memory (internal error) AEE_EUNSUPPORTED
 * Unsupported API on the target
 */
 ...
 *		  refer to "Attributes for
 *remote_register_buf_attr/remote_register_buf_attr2" to set the required
 *attribute value.
 */
