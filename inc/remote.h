...
enum remote_rpc_latency_flags {
  ...
  /** DSP driver predicts completion time of a method and send CPU wake up
   * signal to reduce wake up latency. Recommended for moderate latency
   * sensitive use cases. It is more power efficient compared to pm_qos control.
   */
  RPC_ADAPTIVE_QOS,
...
struct remote_rpc_control_latency {
  /** Enable latency optimization techniques to meet requested latency. Use
   ...
   * of method on DSP. CPU will poll for a DSP response for that specified
   * duration after which it will timeout and fall back to waiting for a glink
   * response. Max value that can be passed is 10000 (10 ms)
   */
  uint32_t latency;
};
...
typedef struct remote_dsp_capability {
  uint32_t domain; /** @param[in]: DSP domain ADSP_DOMAIN_ID, SDSP_DOMAIN_ID, or
                      CDSP_DOMAIN_ID */
  uint32_t attribute_ID; /** @param[in]: One of the DSP/kernel attributes from
                            enum remote_dsp_attributes */
  uint32_t capability;   /** @param[out]: Result of the DSP/kernel capability
                            query based on attribute_ID */
} fastrpc_capability;
 ...
 * DSPRPC_GET_DSP_INFO should only be used with remote_handle_control() as a
 * handle is not needed to query DSP capabilities. To query DSP capabilities
 * fill out 'domain' and 'attribute_ID' from structure remote_dsp_capability.
 * DSP capability will be returned on variable 'capability'.
 */
enum remote_dsp_attributes {
 ...
 * in remote handle control interface.
 * Get domain ID associated with an opened handle to remote interface of type
 * remote_handle64. remote_handle64_control() returns domain for a given handle
...
struct remote_control_custom_path {
  ...
  char *value;        /** value which will be used for file operations when the
                         corresponding key is specified in the file URI */
};
...
enum handle_control_req_id {
  ...
  DSPRPC_GET_DSP_INFO,     /** Request ID to get dsp capabilites from kernel and
                              Hexagon */
  DSPRPC_CONTROL_WAKELOCK, /** Request ID to enable wakelock for the given
                              domain */
  DSPRPC_GET_DOMAIN,       /** Request ID to get the default domain or domain
                              associated to an exisiting handle */
...
struct remote_rpc_thread_params {
  int domain;     /** Remote subsystem domain ID, pass -1 to set params for all
                     domains */
...
struct remote_rpc_relative_thread_priority {
  int domain; /** Remote subsystem domain ID, pass -1 to update priority for all
                 domains */
  int relative_thread_priority; /** the value by which the default thread
                                 * priority needs to increase/decrease DSP
                                 * thread priorities run from 1 to 255 with 1
                                 * being the highest thread priority. So a
                                 * negative relative thread priority value will
                                 * 'increase' the thread priority, a positive
                                 * value will 'decrease' the thread priority.
                                 */
};
 ...
 * libcdsprpc.so/libadsprpc.so/libmdsprpc.so/libsdsprpc.so in case of
 * non-domains.
 */
...
struct remote_rpc_session_close {
  int domain; /** Remote subsystem domain ID, -1 to close all handles for all
                 domains */
};
 ...
 *remote_handle_control Return values denote the type of process on remote
 *subsystem
 **/
 ...
 * Status flags for the user PD on the DSP returned by the status notification
 *function
 *
 **/
typedef enum remote_rpc_status_flags {
  ...
  FASTRPC_USER_PD_FORCE_KILL, /** DSP user process forcefully killed. Happens
                                 when DSP resources needs to be freed. */
  FASTRPC_USER_PD_EXCEPTION,  /** Exception in the user process of DSP. */
  FASTRPC_DSP_SSR, /** Subsystem restart of the DSP, where user process is
                      running. */
...
typedef struct remote_rpc_notif_register {
  ...
  int domain;    /** @param[in]: DSP domain ADSP_DOMAIN_ID, SDSP_DOMAIN_ID, or
                    CDSP_DOMAIN_ID */
  fastrpc_notif_fn_t
      notifier_fn; /** @param[in]: Notification function pointer */
...
struct remote_rpc_pd_initmem_size {
  ...
  uint32_t pd_initmem_size; /** Initial memory allocated for remote userpd,
                               minimum value : 3MB, maximum value 200MB **/
  /** Unsupported for unsigned user PD, for unsigned user PD init mem size is
   * fixed at 5MB **/
};
 ...
 * Default sesion is always 0 and remains available for any module opened
 *without Session ID. New session reservation starts with session ID 1.
 **/
typedef struct remote_rpc_reserve_new_session {
  char *domain_name; /** @param[in]: Domain name of DSP, on which session need
                        to be reserved */
  uint32_t domain_name_len; /** @param[in]: Domain name length, without NULL
                               character */
  char *session_name; /** @param[in]: Session name of the reserved sesssion */
  uint32_t session_name_len; /** @param[in]: Session name length, without NULL
                                character */
  uint32_t effective_domain_id; /** @param[out]: Effective Domain ID is the
                                 * identifier of the session. Effective Domain
                                 * ID is the unique identifier representing the
                                 * session(PD) on DSP. Effective Domain ID needs
                                 * to be used in place of Domain ID when
                                 * application has multiple sessions.
                                 */
  uint32_t session_id; /** @param[out]: Session ID of the reserved session.
                        * An application can have multiple sessions(PDs) created
                        * on DSP. session_id 0 is the default session. Clients
                        * can reserve session starting from 1. Currently only 2
                        * sessions are supported session_id 0 and session_id 1.
                        */
...
typedef struct remote_rpc_effective_domain_id {
  ...
  uint32_t domain_name_len; /** @param[in]: Domain name length, without NULL
                               character */
  uint32_t session_id; /** @param[in]: Session ID of the reserved session. 0 can
                          be used for Default session */
  uint32_t
      effective_domain_id; /** @param[out]: Effective Domain ID of session */
...
typedef struct remote_rpc_get_uri {
  ...
  uint32_t session_id; /** @param[in]: Session ID of the reserved session. 0 can
                          be used for Default session */
  char *module_uri;    /** @param[in]: URI of the module, found in the
                          auto-generated header file*/
  uint32_t module_uri_len; /** @param[in]: Module URI length, without NULL
                              character */
  char *uri;        /** @param[out]: URI containing module, domain and session.
                     * Memory for uri need to be pre-allocated with session_uri_len
                     * size.        Typically session_uri_len is 30 characters more
                     * than        Module URI length.        If size of uri is beyond
                     * session_uri_len,        remote_session_control fails with
                     * AEE_EBADSIZE
                     */
...
enum session_control_req_id {
  ...
  FASTRPC_THREAD_PARAMS, /** Set thread parameters like priority and stack size
                          */
  DSPRPC_CONTROL_UNSIGNED_MODULE,   /** Handle the unsigned module offload
                                       request, to be called before
                                       remote_handle_open() */
  FASTRPC_RESERVED_2,               /** Reserved */
  FASTRPC_RELATIVE_THREAD_PRIORITY, /** To increase/decrease default thread
                                       priority */
  ...
  FASTRPC_REMOTE_PROCESS_EXCEPTION, /** Trigger Exception in the remote process
                                     */
  FASTRPC_REMOTE_PROCESS_TYPE,      /** Query type of process defined by enum
                                       fastrpc_process_type */
  FASTRPC_REGISTER_STATUS_NOTIFICATIONS, /** Enable DSP User process status
                                            notifications */
  FASTRPC_PD_INITMEM_SIZE,     /** Set signed userpd initial memory size  **/
  FASTRPC_RESERVE_NEW_SESSION, /** Reserve new FastRPC session **/
  FASTRPC_GET_EFFECTIVE_DOMAIN_ID, /** Get effective domain ID of a FastRPC
                                      session */
  FASTRPC_GET_URI, /** Creates the URI needed to load a module in the DSP User
                      PD */
  FASTRPC_MAX_THREAD_PARAM, /** Set max thread value for unsigned PD */
  FASTRPC_CONTEXT_CREATE,  /** Create or attaches to remote session(s) on one or
                              more domains */
...
enum remote_mem_map_flags {
  /**
   * Create static memory map on remote process with default cache configuration
   * (writeback). Same remoteVirtAddr will be assigned on remote process when
   * fastrpc call made with local virtual address.
   * @Map lifetime
   * Life time of this mapping is until user unmap using remote_mem_unmap or
   * session close. No reference counts are used. Behavior of mapping multiple
   * times without unmap is undefined.
   * @Cache maintenance
   * Driver clean caches when virtual address passed through RPC calls defined
   * in IDL as a pointer. User is responsible for cleaning cache when
   * remoteVirtAddr shared to DSP and accessed out of fastrpc method invocations
   * on DSP.
   * @recommended usage
   * Map buffers which are reused for long time or until session close. This
   * helps to reduce fastrpc latency. Memory shared with remote process and
   * accessed only by DSP.
   */
...
#define FASTRPC_ATTR_COHERENT                                                  \
  4 /** Attribute to map a buffer as dma coherent,                             \
       ...
       It will be ignored if a device is marked as dma-coherent in device      \
       tree.*/
...
#define FASTRPC_ATTR_FORCE_NOFLUSH                                             \
  32 /** Attribute to map buffer such that flush by driver is skipped for that \
        particular buffer client has to perform cache maintenance*/
#define FASTRPC_ATTR_FORCE_NOINVALIDATE                                        \
  64 /** Attribute to map buffer such that invalidate by driver is skipped for \
        that particular buffer client has to perform cache maintenance */
 ...
 *
 * NOTE: This function should not be called directly from applications. It is
 * automatically called by the stub functions generated by the QAIC compiler
 * from IDL files. Applications should use the generated stub functions instead.
 *
 * This function creates a handle to communicate with a module running on the
 * DSP. The handle can be used to invoke remote functions defined in the
 * module's IDL interface.
 *
 * @param name [in] URI of the module to open, found in the auto-generated
 * header file. Format: "uri:module[;option1=value1][;option2=value2]..."
 *
 ...
 *
 * @param ph [out] Pointer to store the opened remote handle. This handle should
 * be used in subsequent remote_handle_invoke() calls and must be closed using
 ...
 * from IDL files. Applications should use the generated stub functions instead.
 *
 * This function invokes a remote function defined in the module's IDL interface
 * using the handle opened via remote_handle_open().
 *
 ...
 *                       - REMOTE_SCALARS_OUTHANDLES(sc): Number of output
 * handles
 *
 * @param pra [in] Array of remote_arg structures containing the arguments in
 * order:
 *                 1. Input buffers
 *                 2. Output buffers
 *                 3. Input handles
 ...
 *
 * @note This API is thread-safe and can be called concurrently from multiple
 * threads using the same handle.
 *
 * @note For output buffers, the caller must ensure sufficient buffer size is
 * allocated before making the RPC call.
 */
 ...
 *
 * This function closes the remote handle and frees any associated resources.
 ...
 *
 * @note This API is thread-safe but should not be called while other threads
 * are using the same handle for RPC operations.
 *
 * @note All open handles should be closed when no longer needed to avoid
 * resource leaks. Handles are not automatically closed when the process exits.
 */
 ...
 *
 * This function allows configuring various control parameters for remote handle
 * operations like latency requirements, wake lock control, domain info etc.
 *
 * @param req [in] Request ID specifying the control parameter to set, defined
 * in enum handle_control_req_id:
 ...
 *
 * @param data [in] Pointer to request-specific data structure containing
 * parameters:
 ...
 *
 * @return 0 on success, otherwise error code:
 *         - AEE_EBADPARM: Invalid parameters (NULL data pointer, invalid
 * datalen)
 ...
 *
 * This function allows configuring various control parameters for remote
 * sessions like process lifecycle, thread parameters, session management etc.
 *
 * @param req [in] Request ID specifying the control parameter to set, defined
 * in enum session_control_req_id:
 ...
 *
 * This function allows asynchronous invocation of remote methods, providing
 * non-blocking execution and callback mechanisms.
 *
 * @param h [in] Remote handle obtained from remote_handle_open()
 *
 ...
 * FASTRPC_ASYNC_CALLBACK type) See fastrpc_async_descriptor_t for details
 *
 ...
 * @note Resources must be released using fastrpc_release_async_job() after
 * completion
 */
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int
    __QAIC_REMOTE(remote_handle64_invoke_async)(
        __QAIC_IN remote_handle64 h, __QAIC_IN fastrpc_async_descriptor_t *desc,
        __QAIC_IN uint32_t dwScalars,
        __QAIC_IN remote_arg *pra) __QAIC_REMOTE_ATTRIBUTE;
 ...
 *
 * This function allows checking the completion status of an asynchronous
 * FastRPC job and retrieving its result. It can be configured to wait for job
 * completion with different timeout behaviors.
 *
 * @param jobid [in] Job ID returned by remote_handle_invoke_async() when
 * submitting the asynchronous job
 *
 * @param timeout_us [in] Timeout value in microseconds:
 *                       - 0: Returns immediately with current status/result
 *                       - Positive value: Waits up to specified microseconds
 * for completion
 ...
 * @note This function is thread-safe and can be called concurrently from
 * multiple threads
 */
__QAIC_REMOTE_EXPORT __QAIC_RETURN int
    __QAIC_REMOTE(fastrpc_async_get_status)(__QAIC_IN fastrpc_async_jobid jobid,
 ...
 *
 * This function must be called after an asynchronous job completes to free
 * associated resources and cleanup internal state. It should only be called
 * after receiving job completion status either through:
 ...
 * @note Resources must be released exactly once per async job to avoid memory
 * leaks
 */
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(fastrpc_release_async_job)(
    __QAIC_IN fastrpc_async_jobid jobid);
 ...
 *
 * @param fd [in] File descriptor associated with the memory to be mapped. Must
 * be a valid DMA buffer file descriptor.
 * @param flags [in] Mapping flags. Currently only REMOTE_MAP_MEM_STATIC is
 * supported.
 * @param vaddrin [in] Input virtual address on CPU side. Must be the address
 * returned by mmap() when mapping the DMA fd.
 ...
 * @param size [in] Size of buffer to unmap in bytes. Must match the size used
 * in remote_mmap(). Partial unmapping is not supported.
 *
 * @return 0 on success, error code on failure:
 *         AEE_EBADPARM - Invalid parameters
 ...
 * DEPRECATED: Use fastrpc_mmap() instead.
 *
 * Maps memory to a specific remote domain process. This function provides more
 * control over domain selection compared to remote_mmap().
 *
 * @param domain [in] DSP domain ID to map memory to. Use -1 for default domain
 * based on linked library (lib(a/m/s/c)dsprpc.so). Valid domains:
 * ADSP_DOMAIN_ID, MDSP_DOMAIN_ID, SDSP_DOMAIN_ID, CDSP_DOMAIN_ID
 ...
 * @param remoteVirtAddr [out] Pointer to store the mapped address on remote
 * domain
 *
 * @return 0 on success, error code on failure:
 *         AEE_EBADPARM - Invalid parameters
 ...
 *
 * @note This API is deprecated and will be removed in a future release.
 *       Use fastrpc_mmap() instead which provides better cache configuration
 * control.
 */
 ...
 * DEPRECATED: Use fastrpc_munmap() instead.
 *
 * Unmaps memory previously mapped using remote_mem_map() from a specific remote
 * domain process.
 *
 * @param domain [in] DSP domain ID to unmap memory from. Use -1 for default
 * domain. Must match domain used in remote_mem_map().
 * @param remoteVirtAddr [in] Remote virtual address returned by
 * remote_mem_map()
 * @param size [in] Size of buffer in bytes to unmap. Must match size used in
 * remote_mem_map().
 *
 * @return 0 on success, error code on failure:
 *         AEE_EBADPARM - Invalid parameters
 ...
 *
 * @note This API is deprecated and will be removed in a future release.
 *       Use fastrpc_munmap() instead which provides better integration with the
 * FastRPC framework.
 */
 ...
 * DEPRECATED: Use fastrpc_mmap() instead.
 *
 * Maps memory to the remote domain with 64-bit address support. This is the
 * 64-bit version of remote_mmap().
 *
 ...
 * @param size [in] Size of buffer to unmap in bytes. Must match size used in
 * remote_mmap64().
 *
 * @return 0 on success, error code on failure:
 *         AEE_EBADPARM - Invalid parameters
...
/**
 * fastrpc_mmap
 * Creates a mapping on remote process for a DMA buffer with file descriptor.
 * New fastrpc session will be opened if not already opened for the domain. This
 * API maps the buffer with RW- permission and CACHE WRITEBACK configuration.
 * Driver will clean cache when buffer is passed in a FastRPC call.
 *
 * @param domain [in] DSP domain ID of a fastrpc session. Use -1 for default
 * domain based on linked library. Valid domains are ADSP_DOMAIN_ID,
 * MDSP_DOMAIN_ID, SDSP_DOMAIN_ID, or CDSP_DOMAIN_ID.
 * @param fd [in] DMA memory file descriptor obtained from dma_alloc_fd() or
 * similar DMA allocation APIs.
 * @param addr [in] Virtual address of the buffer on CPU side. Must be the same
 * address returned by mmap() when mapping the DMA fd.
 * @param offset [in] Offset from the beginning of the buffer. Must be page
 * aligned (4KB).
 * @param length [in] Size of buffer in bytes to map. Must be page aligned
 * (4KB).
 * @param flags [in] Controls mapping functionality on DSP. See enum
 * fastrpc_map_flags for valid flags: FASTRPC_MAP_CACHE_WRITEBACK - Map with
 * writeback cache configuration (default) FASTRPC_MAP_CACHE_WRITETHROUGH - Map
 * with writethrough cache configuration FASTRPC_MAP_CACHE_UNCACHED - Map as
 * uncached memory FASTRPC_MAP_CACHE_NONCACHED - Map as non-cached memory
 *
 * @return 0 on success, error code on failure:
 *         AEE_EALREADY - Buffer already mapped. Multiple mappings for same
 * buffer not supported. AEE_EBADPARM - Invalid parameters (null pointers,
 * unaligned sizes, etc) AEE_EFAILED - Failed to map buffer (internal driver
 * error) AEE_ENOMEMORY - Out of memory in driver AEE_EUNSUPPORTED - API not
 * supported on target DSP
 *
...
/**
 * fastrpc_munmap
 * Removes a mapping created by fastrpc_mmap() for a DMA buffer on the remote
 * process. The mapping must be removed before closing the DMA file descriptor.
 *
 * @param domain [in] DSP domain ID of a fastrpc session. Use -1 for default
 * domain based on linked library. Valid domains are ADSP_DOMAIN_ID,
 * MDSP_DOMAIN_ID, SDSP_DOMAIN_ID, or CDSP_DOMAIN_ID.
 * @param fd [in] DMA memory file descriptor that was used to create the
 * mapping.
 * @param addr [in] Virtual address of the buffer on CPU side. Must match the
 * address used in fastrpc_mmap().
 * @param length [in] Size of buffer in bytes to unmap. Must match the length
 * used in fastrpc_mmap().
 *
 * @return 0 on success, error code on failure:
 *         AEE_EBADPARM - Invalid parameters (null pointers, unaligned sizes,
 * etc) AEE_EINVALIDFD - No mapping found for the specified file descriptor
 *         AEE_EFAILED - Failed to unmap buffer (internal driver error)
 *         AEE_EUNSUPPORTED - API not supported on target DSP
 *
 * @note This API must be called to cleanup mappings before closing DMA file
 * descriptors. Failing to unmap can lead to resource leaks in the driver.
 */
 ...
 * Register a file descriptor for a buffer to enable zero-copy sharing with DSP
 * via SMMU.
 *
 * These functions are thread-safe and can be called from multiple threads
 * concurrently. However, registering/deregistering the same buffer from
 * different threads simultaneously is not supported and will lead to undefined
 * behavior.
 *
 * @note These APIs are limited to buffers < 2GB in size. For larger buffers,
 * use remote_register_buf_attr2 which supports 64-bit sizes.
 *
 * @note Some versions of libcdsprpc.so lack these functions, so users should
 * set these symbols as weak: #pragma weak remote_register_buf #pragma weak
 * remote_register_buf_attr
 *
 * @param buf [in] Virtual address of the buffer to register. Must be a valid
 * mapped address.
 * @param size [in] Size of the buffer in bytes. Must be > 0 and < 2GB.
 * @param fd [in] File descriptor for the buffer. Use -1 to deregister a
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN void __QAIC_REMOTE(remote_register_buf_attr)(
    __QAIC_IN_LEN(size) void *buf, __QAIC_IN int size, __QAIC_IN int fd,
    __QAIC_IN int attr) __QAIC_REMOTE_ATTRIBUTE;

/**
 * remote_register_buf_attr2
 * Register a file descriptor for a buffer to enable zero-copy sharing with DSP
 * via SMMU. This version supports 64-bit buffer sizes, unlike
 * remote_register_buf/remote_register_buf_attr.
 *
 ...
 * behavior.
 *
 * Some older versions of libcdsprpc.so lack this function, so users should set
 * this symbol as weak: #pragma weak remote_register_buf_attr2
 *
 * @param buf [in] Virtual address of the buffer to register. Must be a valid
 * mapped address.
 ...
 * Register a DMA handle with FastRPC to enable zero-copy sharing of ION memory
 * with DSP via SMMU. This API is only valid on Android systems with
 * ION-allocated memory.
 *
 * This function is thread-safe and can be called from multiple threads
 * concurrently. However, registering/deregistering the same buffer from
 * different threads simultaneously is not supported and will lead to undefined
 * behavior.
 *
 * Some versions of libadsprpc.so lack this function, so users should set these
 * symbols as weak: #pragma weak remote_register_dma_handle #pragma weak
 * remote_register_dma_handle_attr
 *
 * @param fd [in] File descriptor for the ION buffer. Use -1 to deregister a
 * previously registered buffer.
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(
    remote_register_dma_handle)(__QAIC_IN int fd,
                                __QAIC_IN uint32_t len) __QAIC_REMOTE_ATTRIBUTE;
__QAIC_REMOTE_EXPORT __QAIC_RETURN int
...
/**
 * remote_register_fd
 * Register a file descriptor with FastRPC to enable zero-copy sharing of memory
 * with DSP. This API is useful when users have a file descriptor but no virtual
 * address mapping.
 *
 * The function creates a PROT_NONE mapping that cannot be accessed directly,
 * but serves as an identifier for the buffer in FastRPC calls. The mapping is
 * used internally by the RPC layer to share the buffer with DSP.
 *
 ...
 * behavior.
 *
 * Some versions of libadsprpc.so lack this function, so users should set this
 * symbol as weak: #pragma weak remote_register_fd
 *
 * @param fd [in] File descriptor for the buffer. Must be a valid file
 * descriptor.
 * @param size [in] Size of the buffer in bytes. Must be > 0 and < 2GB.
 *
 * @return On success, returns a virtual address that can be used in FastRPC
 * calls. On failure, returns (void*)-1. Check errno for error details: EINVAL -
 * Invalid parameters (fd < 0, size = 0 or size >= 2GB) ENOMEM - Out of memory
 * in driver EBADF - Invalid file descriptor EBUSY - Buffer already registered
 *         ENOSYS - API not supported on this platform
 *
 * To deregister the buffer, call remote_register_buf(addr, size, -1) with the
 * returned address.
 */
 ...
 * Register a file descriptor with FastRPC to enable zero-copy sharing of memory
 * with DSP. This API is useful when users have a file descriptor but no virtual
 * address mapping. Unlike remote_register_fd(), this function supports buffers
 * larger than 2GB.
 *
 * The function creates a PROT_NONE mapping that cannot be accessed directly,
 * but serves as an identifier for the buffer in FastRPC calls. The mapping is
 ...
 *
 * Some versions of libadsprpc.so lack this function, so users should set this
 * symbol as weak: #pragma weak remote_register_fd2
 *
 * @param fd [in] File descriptor for the buffer. Must be a valid file
 * descriptor.
 * @param size [in] Size of the buffer in bytes. Must be > 0.
 ...
 * @return On success, returns a virtual address that can be used in FastRPC
 * calls. On failure, returns (void*)-1. Check errno for error details: EINVAL -
 * Invalid parameters (fd < 0, size = 0) ENOMEM - Out of memory in driver EBADF
 * - Invalid file descriptor EBUSY - Buffer already registered ENOSYS - API not
