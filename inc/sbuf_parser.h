...
//! init
static __inline void sbuf_parser_init(struct sbuf *buf, const char *data,
                                      int dataLen) {
...
static __inline int sbuf_end(struct sbuf *buf) { return sbuf_left(buf) == 0; }
 ...
 */
static __inline int sbuf_try(struct sbuf *buf,
                             int (*parser)(struct sbuf *buf)) {
  struct sbuf tryp;
